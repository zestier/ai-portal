...
describe("resolvePermissionShortcut", () => {
  ...
  const ev = (
    over: Partial<{ key: string; shiftKey: boolean; repeat: boolean }> = {},
  ) => ({
    key: "Enter",
    shiftKey: false,
    repeat: false,
  ...
  it("persists on armed Shift+Enter when a persistent decision is offered", () => {
    expect(resolvePermissionShortcut(ev({ shiftKey: true }), ready)).toBe(
      "allow-always",
    );
  });
  ...
  it("ignores Enter until the approve shortcuts are armed", () => {
    expect(
      resolvePermissionShortcut(ev(), { ...ready, approveArmed: false }),
    ).toBe("none");
  });

  it("ignores Enter while focus is on an inner control (e.g. feedback textarea)", () => {
    expect(
      resolvePermissionShortcut(ev(), { ...ready, focusOnDialog: false }),
    ).toBe("none");
  });

  it("ignores every key while a response is in flight", () => {
    expect(
      resolvePermissionShortcut(ev({ key: "Escape" }), {
        ...ready,
        busy: true,
      }),
    ).toBe("none");
    expect(resolvePermissionShortcut(ev(), { ...ready, busy: true })).toBe(
      "none",
    );
  });
  ...
  it("downgrades Shift+Enter to none when persistence is unavailable", () => {
    expect(
      resolvePermissionShortcut(ev({ shiftKey: true }), {
        ...ready,
  ...
  it("always denies on Escape immediately — even unarmed, repeating, or mid-typing", () => {
    ...
    expect(
      resolvePermissionShortcut(esc, { ...ready, approveArmed: false }),
    ).toBe("deny");
    expect(resolvePermissionShortcut({ ...esc, repeat: true }, ready)).toBe(
      "deny",
    );
    expect(
      resolvePermissionShortcut(esc, { ...ready, focusOnDialog: false }),
    ).toBe("deny");
  });
