...
describe("templatePermissionPreview", () => {
  ...
  it("builds a create preview with default type and metadata fields", () => {
    ...
    expect(preview?.fields.find((f) => f.label === "Pinned")?.value).toBe(
      "yes",
    );
  });
  ...
  it("has a null merged view for create and for update without a before snapshot", () => {
    expect(
      templatePermissionPreview("template_create", { title: "T", prompt: "p" })
        ?.merged,
    ).toBeNull();
    expect(
      templatePermissionPreview("template_update", { id: "x", title: "New" })
        ?.merged,
    ).toBeNull();
...
describe("summarizeTemplatePermission", () => {
  it("returns null for non-template tools / non-records", () => {
    expect(
      summarizeTemplatePermission("git_commit", { title: "x" }),
    ).toBeNull();
