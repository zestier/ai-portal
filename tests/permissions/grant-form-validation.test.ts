...
describe("GrantInputSchema — valid shapes", () => {
  ...
  it("shell with session-workspace-paths positionals", () => {
    const parsed = GrantInputSchema.parse({
      ...
      scope: {
        kind: "shell",
        rule: shell("cat", {
          positionals: { kind: "session-workspace-paths" },
        }),
      },
    });
    expect(parsed.scope.kind).toBe("shell");
  ...
  it("lowercases url host-suffix scopes at store time", () => {
    ...
    expect(parsed.scope.kind).toBe("url");
    if (
      parsed.scope.kind === "url" &&
      parsed.scope.rule.kind === "host-suffix"
    ) {
...
describe("GrantInputSchema — rejections", () => {
  ...
  it("rejects tool/scope-kind mismatch (shell tool, fs scope)", () => {
    ...
    if (!r.success) {
      expect(
        r.error.issues.some((i) => i.message.includes("requires scope.kind")),
      ).toBe(true);
    }
  });
...
describe("refineScopeToolAlignment — shared by GrantInputSchema and request_permission_grant", () => {
  ...
  it("rejects a scope kind that does not match the tool, citing scope.kind", () => {
    ...
    if (!r.success) {
      expect(
        r.error.issues.some((i) => i.path.join(".") === "scope.kind"),
      ).toBe(true);
    }
  });
  ...
  it("rejects fs perms that exclude the chosen tool, citing scope.perms", () => {
    ...
    if (!r.success) {
      expect(
        r.error.issues.some((i) => i.path.join(".") === "scope.perms"),
      ).toBe(true);
    }
  });
...
describe("grant form metadata helpers", () => {
  it("builds shell, fs, and url scope JSON through shared form helpers", () => {
    ...
    expect(
      JSON.parse(buildGrantScopeJson("shell", shellFields).json ?? ""),
    ).toEqual({
      kind: "shell",
      rule: {
        command: [
    ...
    expect(
      JSON.parse(buildGrantScopeJson("read", fsFields).json ?? ""),
    ).toEqual({
      kind: "fs",
      perms: ["read"],
      rule: {
    ...
    expect(
      JSON.parse(buildGrantScopeJson("url", urlFields).json ?? ""),
    ).toEqual({
      kind: "url",
      rule: { kind: "host-suffix", suffix: "github.com" },
    });
  ...
  it("rejects malformed positional count bounds", () => {
    ...
    expect(
      buildGrantScopeJson("shell", { ...base, shellPositionalMax: "-1" }).error,
    ).toMatch(/non-negative integer/);
    expect(
      buildGrantScopeJson("shell", { ...base, shellPositionalMin: "1.5" })
        .error,
    ).toMatch(/non-negative integer/);
    ...
    expect(
      buildGrantScopeJson("shell", {
        ...base,
        shellPositionalMin: "2",
        shellPositionalMax: "1",
      }).error,
    ).toMatch(/must not exceed/);
    // Blank on both ends omits the rule entirely.
    expect(
      JSON.parse(buildGrantScopeJson("shell", base).json ?? "").rule
        .positionalCount,
    ).toBe(undefined);
  });
  ...
  it("initializes edit form fields from existing grant scopes", () => {
    ...
    expect(originalShellCommand?.map((step) => step.token)).toEqual([
      "git",
      "log",
    ]);
  });
  ...
  it("shares grant scope descriptions between settings and capability surfaces", () => {
    ...
    expect(describeGrantScope({ scope, scopePattern: null })).toBe(
      "[read] <workspace>/src/**",
    );
...
describe("GrantInputSchema — custom-tool grants", () => {
  ...
  it("trims surrounding whitespace off the tool name", () => {
    const parsed = GrantInputSchema.parse(
      customTool({ toolName: "  worktree_remove  " }),
    );
  ...
  it("requires a tool name, citing toolName", () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success) {
      expect(r.error.issues.some((i) => i.path.join(".") === "toolName")).toBe(
        true,
      );
    }
  });
  ...
  it.each(["*", "worktree *", "worktree/create", "a b"])(
    "rejects %j as a tool name",
    (name) => {
      const r = GrantInputSchema.safeParse(customTool({ toolName: name }));
      expect(r.success).toBe(false);
      if (!r.success) {
        expect(
          r.error.issues.some((i) => i.path.join(".") === "toolName"),
        ).toBe(true);
      }
    },
  );
  ...
  it("offers custom-tool in the form tool list without widening the scoped kinds", () => {
    ...
    expect(grantToolLabel("custom-tool")).toBe(
      "custom-tool (a portal tool, by name)",
    );
  });
