...
async function seed(conversationTitle = 'projection') {
	...
	const conv = convs.create(user.id, { title: conversationTitle, workdir: '/tmp', model: null });
...
async function emptyConversation() {
	...
	const convs = await import('../src/lib/server/db/repos/conversations');
	const user = users.ensureLocalUser();
	const conv = convs.create(user.id, { title: 'empty', workdir: '/tmp', model: null });
	return { users, conv, user };
}

describe('previewCut', () => {
	it('strips markdown to plain text and cuts on a word boundary', () => {
		const p = previewCut('one two three **four** five six seven eight nine ten', 20);
...
describe('projectTranscript', () => {
	...
	it('hydrates the newest tail and indexes the rest with descriptors', async () => {
		...
		expect(task.meta).toMatchObject({ agent_type: 'memory-extractor', model: 'some/model' });
	...
	it('splits a large conversation into tail + bounded index', async () => {
		...
		const messages = await import('../src/lib/server/db/repos/messages');
		const user = users.ensureLocalUser();
		const conv = convs.create(user.id, { title: 'big', workdir: '/tmp', model: null });
		for (let i = 0; i < TRANSCRIPT_HYDRATED_TAIL + TRANSCRIPT_INDEX_COUNT + 30; i++) {
			messages.append(conv.id, { role: i % 2 ? 'assistant' : 'user', content: `m ${i}` });
		}
		const t = projectTranscript(conv.id);
		expect(t.tail.length).toBe(TRANSCRIPT_HYDRATED_TAIL);
...
describe('projectIndexPage', () => {
	...
	it('pages older-than-beforeId in ascending order with hasMore', async () => {
		...
		const messages = await import('../src/lib/server/db/repos/messages');
		const user = users.ensureLocalUser();
		const conv = convs.create(user.id, { title: 'paging', workdir: '/tmp', model: null });
		for (let i = 0; i < 40; i++) {
			messages.append(conv.id, { role: i % 2 ? 'assistant' : 'user', content: `m ${i}` });
		}
