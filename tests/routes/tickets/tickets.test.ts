...
import {
  conversationId as conversationIdCodec,
...
describe("workspace tickets", () => {
  ...
  it("repo scopes tickets by user and workspace", async () => {
    ...
    const a = tickets.create(user.id, {
    ...
    expect(
      tickets.list(user.id, workspace, { status: "done" }).map((t) => t.id),
    ).toEqual([a.id]);
    expect(tickets.count(user.id, workspace, "done")).toBe(1);
    expect(() =>
      tickets.create(user.id, { workspaceKey: workspace, title: "   " }),
  ...
  it("API creates, lists, updates, and archives tickets for the current user", async () => {
    ...
    const otherWorkspace = mkdtempSync(
      join(tmpdir(), "portal-ticket-other-workspace-"),
    );
    const { POST, GET } =
      await import("../../../src/routes/api/tickets/+server");
    const { PATCH, DELETE } =
      await import("../../../src/routes/api/tickets/[id]/+server");
    const user = users.ensureLocalUser();
    ...
    expect(listed.tickets.map((t: { id: number }) => t.id)).toEqual([
      created.ticket.id,
    ]);
    ...
    expect(
      tickets.list(user.id, workspace, { status: "archived" }).map((t) => t.id),
    ).toEqual([created.ticket.id]);
  ...
  it("API purge permanently deletes a ticket and cascades its edges", async () => {
    ...
    const blocker = tickets.create(user.id, {
      workspaceKey: workspace,
      title: "Prerequisite",
    });
    ...
    expect(
      tickets.list(user.id, workspace, { status: "all" }).map((t) => t.id),
    ).toEqual([blocker.id]);
  ...
  it("agent ticket tools are scoped to the active user and workspace", async () => {
    ...
    const tickets = await import("../../../src/lib/server/db/repos/tickets");
    const { buildTicketTools } =
      await import("../../../src/lib/server/tools/tickets");
    const user = users.ensureLocalUser();
    ...
    const listedFields = await list.handler({
      status: "all",
      fields: ["id", "status"],
    });
    const listedData =
      listedFields.ok && (listedFields.result as { tickets: unknown[] });
    expect(Array.isArray(listedData && listedData.tickets)).toBe(true);
    expect(
      (listedData as { tickets: Record<string, unknown>[] }).tickets[0],
    ).toEqual({
      id: ticket.id,
      status: "done",
    });
    ...
    const compactData =
      compact.ok && (compact.result as Record<string, unknown>);
    expect(compactData).toMatchObject({
      id: String(ticket.id),
      title: "Remember this",
    });
    expect(compactData).not.toHaveProperty("createdAt");
    expect((compactData as { _omitted?: string[] })._omitted).toContain(
      "createdAt",
    );

    const picked = await get.handler({
      id: String(ticket.id),
      fields: ["createdAt"],
    });
    ...
    const workspaceOnly = await get.handler({
      id: String(ticket.id),
      fields: ["workspaceKey"],
    });
    const workspaceData =
      workspaceOnly.ok && (workspaceOnly.result as Record<string, unknown>);
    ...
    const planOnly = await get.handler({
      id: String(ticket.id),
      fields: ["plan"],
    });
    const planData =
      planOnly.ok && (planOnly.result as Record<string, unknown>);
  ...
  it("models ticket dependencies, blocks cycles, and surfaces blockers", async () => {
    ...
    const otherWorkspace = mkdtempSync(
      join(tmpdir(), "portal-ticket-deps-other-"),
    );
    const tools = buildTicketTools({
      userId: user.id,
      workspaceKey: workspace,
    ...
    const foreign = tickets.create(user.id, {
      workspaceKey: otherWorkspace,
      title: "Elsewhere",
    });
    ...
    // an unchanged set).
    const again = await update.handler({
      id: String(ui.id),
      blockedBy: [String(api.id)],
    });
    ...
    expect(
      (await update.handler({ id: String(ui.id), blockedBy: [String(ui.id)] }))
        .ok,
    ).toBe(false);
    const cycle = await update.handler({
      id: String(api.id),
      blockedBy: [String(ui.id)],
    });
    ...
    expect(listed.ok && (listed.result as string)).toContain(
      `(blocked by: ${api.id})`,
    );

    // ticket_get exposes blockedBy/blocks via the fields selector.
    const uiBlockers = await get.handler({
      id: String(ui.id),
      fields: ["blockedBy"],
    });
    expect(
      uiBlockers.ok && (uiBlockers.result as { blockedBy: string[] }).blockedBy,
    ).toEqual([api.id]);
    ...
    expect(
      uiCompact.ok && (uiCompact.result as { blockedBy?: string[] }).blockedBy,
    ).toEqual([api.id]);
    const apiCompact = await get.handler({ id: String(api.id) });
    expect(
      apiCompact.ok && (apiCompact.result as { blocks?: string[] }).blocks,
    ).toEqual([ui.id]);
    // A ticket with no open blockers omits the empty list rather than showing [].
    expect(
      apiCompact.ok && (apiCompact.result as Record<string, unknown>),
    ...
    expect(readyList.ok && (readyList.result as string)).not.toContain(
      "blocked by",
    );
    // UI now renders without a blockedBy entry in the compact view.
    const uiReady = await get.handler({ id: String(ui.id) });
    expect(
      uiReady.ok && (uiReady.result as Record<string, unknown>),
    ).not.toHaveProperty("blockedBy");
    ...
    expect(
      (await update.handler({ id: String(ui.id), blockedBy: [] })).ok,
    ).toBe(true);
  ...
  it("ticket_add and ticket_update accept edges and surface bad ones as errors", async () => {
    ...
    expect(
      tickets
        .list(user.id, workspace, { status: "all" })
        .some((t) => t.title === "Broken"),
    ).toBe(false);
  });
  ...
  it("create and update set blocking edges from either side, atomically", async () => {
    ...
    const infra = tickets.create(user.id, {
      workspaceKey: workspace,
      title: "Infra",
    });
    ...
    expect(tickets.dependencyRefs(ui.id, user.id).map((r) => r.id)).toEqual([
      api.id,
    ]);
    expect(tickets.dependentRefs(ui.id, user.id).map((r) => r.id)).toEqual([
      ship.id,
    ]);
    ...
    expect(tickets.list(user.id, workspace, { status: "all" }).length).toBe(
      before,
    );

    // A bad edge id on update throws and leaves edges unchanged.
    expect(() =>
      tickets.update(ui.id, user.id, { blockedBy: [999999] }),
    ).toThrow(/not found/i);
    expect(
      tickets
        .dependencyRefs(ui.id, user.id)
  ...
  it("ticket detail page load returns the ticket with its dependency refs", async () => {
    ...
    const tickets = await import("../../../src/lib/server/db/repos/tickets");
    const { load } =
      await import("../../../src/routes/tickets/[id]/+page.server");
    const user = users.ensureLocalUser();
    ...
    expect(apiData.dependents).toEqual([
      { id: ui.id, title: "Build UI", status: "open" },
    ]);
  ...
  it("repo list paginates with limit + offset", async () => {
    ...
    const full = tickets.list(user.id, workspace, {
      status: "open",
      limit: 100,
      offset: 0,
    });
    ...
    expect(
      tickets
        .list(user.id, workspace, { limit: 2, offset: 0 })
        .map((t) => t.id),
    ).toEqual(full.slice(0, 2).map((t) => t.id));
    expect(
      tickets
        .list(user.id, workspace, { limit: 2, offset: 2 })
        .map((t) => t.id),
    ).toEqual(full.slice(2, 4).map((t) => t.id));
    expect(
      tickets
        .list(user.id, workspace, { limit: 2, offset: 4 })
        .map((t) => t.id),
    ).toEqual(full.slice(4).map((t) => t.id));
    ...
    const fullAll = tickets.list(user.id, workspace, {
      status: "all",
      limit: 100,
      offset: 0,
    });
    expect(
      tickets
        .list(user.id, workspace, { status: "all", limit: 3, offset: 0 })
        .map((t) => t.id),
  ...
  it("repo list filters by priority and sorts highest-first across the full set", async () => {
    ...
    expect(
      tickets
        .list(user.id, workspace, { sort: "priority", limit: 3, offset: 0 })
        .map((t) => t.id),
    ).toEqual(sorted.slice(0, 3).map((t) => t.id));
    expect(
      tickets
        .list(user.id, workspace, { sort: "priority", limit: 3, offset: 3 })
        .map((t) => t.id),
    ...
    const p0 = tickets.list(user.id, workspace, {
      priority: "P0",
      limit: 100,
      offset: 0,
    });
    ...
    expect(
      tickets
        .list(user.id, workspace, { priority: "P0", limit: 1, offset: 1 })
        .map((t) => t.id),
    ).toEqual([p0[1].id]);
    ...
    expect(
      tickets.list(user.id, workspace, {
        status: "open",
        limit: 100,
        offset: 0,
      }).length,
  ...
  it("GET /api/tickets honors and validates the sort + priority params", async () => {
    ...
    const sorted = (await sortedRes.json()) as {
  ...
  it("listForSidebar surfaces ready tickets even when the newest are all blocked", async () => {
    ...
    const ready = tickets.create(user.id, {
      workspaceKey: workspace,
      title: "Ready",
    });
    ...
    for (let i = 0; i < 10; i++) {
      const b = tickets.create(user.id, {
        workspaceKey: workspace,
        title: `Blocked ${i}`,
      });
    ...
    const sidebar = tickets
      .listForSidebar(user.id, workspace, 10)
      .map((t) => t.id);
  ...
  it("GET /api/tickets honors the offset query param", async () => {
    ...
    expect(body.tickets.map((t) => t.id)).toEqual(
      full.tickets.slice(1, 3).map((t) => t.id),
    );
  });
  ...
  it("/tickets page load returns the first page for the current workspace", async () => {
    ...
    const { TICKETS_PAGE_SIZE } =
      await import("../../../src/lib/client/tickets-list");
    const user = users.ensureLocalUser();
  ...
  it("defaults priority to P2, persists a chosen one, and enforces the CHECK constraint", async () => {
    ...
    const def = tickets.create(user.id, {
      workspaceKey: workspace,
    ...
    const renamed = tickets.update(def.id, user.id, {
      title: "Default priority (renamed)",
    });
  ...
  it("agent ticket tools accept, persist, and surface priority", async () => {
    ...
    expect(added.ok).toBe(true);
    expect(added.ok && (added.result as { priority?: string }).priority).toBe(
      "P0",
    );
    ...
    await expect(
      add.handler({ title: "Bad", priority: "P9" }),
    ).rejects.toThrow();
    ...
    expect(
      updated.ok && (updated.result as { priority?: string }).priority,
    ).toBe("P2");
  ...
  it("REST API accepts and returns priority on create, patch, and get", async () => {
    ...
    const defResponse = await POST(
      event({
        userId: user.id,
        body: { workspace, title: "API default" },
      }) as never,
    );
    ...
    try {
      await POST(
        event({
          userId: user.id,
          body: { workspace, title: "API bad", priority: "P9" },
        }) as never,
      );
    } catch (e) {
    ...
    const listed = await listResponse.json();
    expect(
      listed.tickets.every(
        (t: { priority?: string }) => typeof t.priority === "string",
      ),
    ).toBe(true);
  });
});
