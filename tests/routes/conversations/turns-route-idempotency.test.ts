...
async function freshImports() {
  ...
  const route =
    await import("../../../src/routes/api/conversations/[id]/turns/+server");
...
describe("turns POST idempotency", () => {
  ...
  it("replays the original ids for a repeated Idempotency-Key header", async () => {
    ...
    startTurnMock.mockImplementation(() =>
      Promise.resolve({ id: `turn-${++turnSeq}` }),
    );
  ...
  it("dedupes via a body requestId too", async () => {
    ...
    const first = await post(route, conv.id, u.id, {
      content: "hi",
      requestId: "req-1",
    });
    const second = await post(route, conv.id, u.id, {
      content: "hi",
      requestId: "req-1",
    });
    ...
    expect(startTurnMock).toHaveBeenCalledTimes(1);
    expect(
      messages.listByConversation(conv.id).filter((m) => m.role === "user"),
    ).toHaveLength(1);
  });
