...
describe("conversation field endpoint", () => {
  ...
  it("404s an unknown field kind rather than guessing", async () => {
    ...
    expect(await status(callGet(conv.id, "tool-secrets", "1", user.id))).toBe(
      404,
    );
  });
  ...
  it("404s a field that has no stored value", async () => {
    const { conv, user } = await seed();
    expect(await status(callGet(conv.id, "tool-result", "2", user.id))).toBe(
      404,
    );
  });
  ...
  it("404s (not 403) for another user, so ids stay non-enumerable", async () => {
    ...
    expect(await status(callGet(conv.id, "tool-args", "1", other.id))).toBe(
      404,
    );
  });
  ...
  it("404s a real tool call requested through the wrong conversation", async () => {
    ...
    const sibling = convs.create(user.id, {
      title: "other",
      workdir: "/tmp",
      model: null,
    });
    expect(await status(callGet(sibling.id, "tool-args", "1", user.id))).toBe(
      404,
    );
  });
});
