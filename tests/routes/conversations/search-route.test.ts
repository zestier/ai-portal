...
async function seed() {
  ...
  messages.append(conv.id, {
    role: "assistant",
    content: "Stubbed reply to: hello",
  });
  return { conv, m, user };
}
...
function makeEvent(convId: string, q: string | null, userId: number | null) {
  const url = new URL(
    `http://127.0.0.1/api/conversations/${convId}/search?q=${q ?? ""}`,
  );
  return {
    params: { id: convId },
    locals: { userId },
...
async function callGet(
  ...
  const { GET } =
    await import("../../../src/routes/api/conversations/[id]/search/+server");
...
describe("conversation search endpoint", () => {
  ...
  it("returns jump targets for an FTS hit", async () => {
    ...
    const hit = body.results.find(
      (r: { messageId: string }) => r.messageId === m.id,
    );
