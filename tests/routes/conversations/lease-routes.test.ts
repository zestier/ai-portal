...
describe("lease-scoped conversation routes", () => {
  ...
  function ctx(query = "", extraParams: Record<string, string | number> = {}) {
    return {
      ...
      url: new URL(
        `http://localhost/api/conversations/${conversationId}/x${query}`,
      ),
  ...
  it("lists held worktrees with dirty counts", async () => {
    ...
    const { GET } =
      await import("../../../src/routes/api/conversations/[id]/worktrees/+server");
  ...
  it("reads the file tree of a lease rather than the conversation workspace", async () => {
    ...
    const { GET } =
      await import("../../../src/routes/api/conversations/[id]/fs/tree/+server");
    const inLease = await (await GET(ctx(`?worktree=${lease.id}`))).json();
    const inPrimary = await (await GET(ctx())).json();

    expect(inLease.entries.map((e: { name: string }) => e.name)).toContain(
      "only-in-lease.txt",
    );
    // Omitting the selector must still resolve the conversation's own tree.
    expect(
      inPrimary.entries.map((e: { name: string }) => e.name),
    ).not.toContain("only-in-lease.txt");
  });
  ...
  it("reads a file from the selected lease", async () => {
    ...
    const { GET } =
      await import("../../../src/routes/api/conversations/[id]/fs/file/+server");
    const body = await (
      await GET(ctx(`?path=README.md&worktree=${lease.id}`))
    ).json();
  ...
  it("reports git changes for the selected lease", async () => {
    ...
    const { GET } =
      await import("../../../src/routes/api/conversations/[id]/git/changes/+server");
    const body = await (await GET(ctx(`?worktree=${lease.id}`))).json();

    expect(body.entries.map((c: { path: string }) => c.path)).toContain(
      "changed.txt",
    );
  });
  ...
  describe("ownership", () => {
    it("404s a lease held by a different conversation", async () => {
      ...
      await expect(GET(ctx(`?worktree=${foreign.id}`))).rejects.toMatchObject({
        status: 404,
      });
    });
    ...
    it("404s an unknown lease id", async () => {
      const { GET } =
        await import("../../../src/routes/api/conversations/[id]/fs/tree/+server");
      await expect(GET(ctx("?worktree=999999"))).rejects.toMatchObject({
        status: 404,
      });
    });
  ...
  describe("conversation delete", () => {
    function convCtx(query = "") {
      return {
        ...
        url: new URL(
          `http://localhost/api/conversations/${conversationId}${query}`,
        ),
        getClientAddress: () => "127.0.0.1",
      } as never;
    }
    ...
    async function expectDeleteRejection(query = ""): Promise<{
      ...
      const { DELETE } =
        await import("../../../src/routes/api/conversations/[id]/+server");
      try {
    ...
    it("refuses when a lease holds unmerged commits, naming the reason", async () => {
      ...
      expect(err.body.leases[0]).toMatchObject({
        id: lease.id,
        reason: "unmerged",
        ahead: 1,
      });
    ...
    it("leads with the dirty reason when both kinds are present", async () => {
      ...
      const reasons = Object.fromEntries(
        err.body.leases.map((l) => [l.id, l.reason]),
      );
