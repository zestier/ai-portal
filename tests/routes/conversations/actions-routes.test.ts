...
describe("conversation actions routes (local mode)", () => {
  ...
  it("GET lists actions from the conversation .zap/actions.json", async () => {
    ...
    const { GET } =
      await import("../../../src/routes/api/conversations/[id]/actions/+server");
  ...
  it("POST runs an action and streams a successful done event", async () => {
      ...
      actions: [
        {
          ...
          steps: [
            {
              command: execPath,
              args: ["-e", 'process.stdout.write("hello-action")'],
            },
          ],
        },
      ],
    ...
    expect(logs.some((e) => String(e.text).includes("hello-action"))).toBe(
      true,
    );
  ...
  it("POST writes a start audit row and a terminal success/failure row", async () => {
    ...
    try {
      ...
      const badEnd = records.find(
        (r) => r.detail?.actionId === "bad" && r.detail?.phase === "end",
      );
  ...
  it("POST rejects a concurrent run of the same action with 409", async () => {
    ...
    const firstReq = new Request(`http://localhost/x`, {
      method: "POST",
      signal: ctrl.signal,
    });
