...
describe("managed worktree conversation routes", () => {
  ...
  it("creates, archives, protects, and force-removes a managed worktree", async () => {
    ...
    const deleteResponse = await DELETE({
      ...
      locals: { userId },
      url: new URL(
        `http://localhost/api/conversations/${conversation.id}?forceWorktree=1`,
      ),
      getClientAddress: () => "127.0.0.1",
    } as never);
    ...
    expect(
      git(source, ["branch", "--list", conversation.worktreeBranch]),
    ).toContain(conversation.worktreeBranch);
  });
  ...
  describe("integration endpoints", () => {
    ...
    it("reports and then integrates a worktree session", async () => {
      ...
      const { GET } =
        await import("../../../src/routes/api/conversations/[id]/worktree/+server");
      const before = await (
        await GET({
          params: { id: conversation.id },
          locals: { userId },
    ...
    it("lists unmerged work for the sidebar and drops it once integrated", async () => {
      ...
      const { GET: BULK } =
        await import("../../../src/routes/api/worktrees/status/+server");
      const listed = await (
        await BULK({
          locals: { userId },
    ...
    it("serves the sidebar a cached status by default and a fresh one on request", async () => {
      ...
      const read = async (fresh: boolean) => {
        const response = await BULK({
          locals: { userId },
          url: new URL(
            `http://localhost/api/worktrees/status${fresh ? "?fresh=1" : ""}`,
          ),
        } as never);
        const body = await response.json();
        return body.worktrees.find(
          (w: { conversationId: string }) =>
