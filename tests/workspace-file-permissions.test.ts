...
describe('parser — workspace permissions file', () => {
	it('parses a realistic document into one grant per entry', () => {
		...
		expect(rm.scope).toMatchObject({ kind: 'shell', rule: { command: [{ token: 'rm' }] } });
		...
		expect(rule?.command[0].options?.allow).toEqual([{ name: '--short', kind: 'flag' }]);
		expect(rule?.command[1].options?.deny).toEqual(['--porcelain']);
		expect(rule?.positionalCount).toEqual({ min: 0, max: 0 });

		expect(readNotes).toMatchObject({ tool: 'read', permissionKind: 'read', decision: 'allow' });
		expect(readNotes.scope).toMatchObject({
			kind: 'fs',
			perms: ['read'],
			rule: { kind: 'path', root: 'absolute', behavior: 'prefix', value: '/tmp/wf-notes' }
		});
	...
	it('rejects unknown sections and malformed entries with a line-referencing error', () => {
		...
		expect(parseWorkspaceGrantFile(`not toml at all = [unclosed`)).toMatchObject({ ok: false });
		...
		expect(
			...
			ok: false,
			error: 'entry 0: shell.options has 2 entries but command has 1 tokens; provide one per token'
		});
		...
		expect(
			parseWorkspaceGrantFile(`[[url]]\nrule = "hostt"\nvalue = "example.com"\n`)
		).toMatchObject({
			ok: false,
			error: 'entry 0: url.rule must be one of exact | host | host-suffix, got "hostt"'
		});
		...
		expect(parseWorkspaceGrantFile(`[[shell]]\ncommand = ["head"]\npipeline = 5\n`)).toMatchObject({
			ok: false,
...
describe('gate — fail-closed hash-gated import', () => {
	...
	it('raises a review when an approved file changes, keeping the old state active', () => {
		...
		expect(getWorkspaceFileStatus(userId, root)).toMatchObject({ accepted: true, drift: false });
	...
	it('raises a review on a whitespace-only drift (conservative hash gate)', () => {
		...
		expect(approve(workspaceFileRequests(driveGate(root).events)[0].requestId)).toBe(true);
		writeFile(root, `${original}\n`);
		const emitter = driveGate(root);
		expect(workspaceFileRequests(emitter.events).length).toBe(1);
	});
...
describe('apply — workspace permissions', () => {
	it('materializes file grants that the matcher resolves, scoped to the file root', () => {
		...
		const rows = settings.listGrantsForUser(userId).filter((g) => g.source === 'workspace-file');
	...
	it('apply is idempotent for the same file content', () => {
		const root = makeWorkspace();
		writeFile(root, REALISTIC_TOML);
		expect(applyWorkspaceFile({ userId, workspaceRoot: root })).toEqual({ ok: true, applied: 5 });
		expect(applyWorkspaceFile({ userId, workspaceRoot: root })).toEqual({ ok: true, applied: 5 });
	...
	it('refuses to edit or individually revoke a workspace-file row via the settings path', () => {
		...
		writeFile(root, `[[tool]]\nname = "git_commit"\ndecision = "deny"\n`);
		expect(applyWorkspaceFile({ userId, workspaceRoot: root })).toEqual({ ok: true, applied: 1 });
		const row = settings.listGrantsForUser(userId).find((g) => g.source === 'workspace-file');
		...
		const after = settings.listGrantsForUser(userId).find((g) => g.id === row.id);
