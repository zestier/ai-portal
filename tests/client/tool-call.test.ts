...
describe("summarizeToolCall", () => {
  it("uses description over command for bash", () => {
    expect(
      summarizeToolCall(
        "bash",
        JSON.stringify({ command: "echo hi", description: "Greet" }),
      ),
    ).toBe("Greet");
  });
  ...
  it("falls back to command when no description", () => {
    expect(
      summarizeToolCall("bash", JSON.stringify({ command: "ls -la" })),
    ).toBe("ls -la");
  });
  ...
  it("falls back to the path for the legacy view tool", () => {
    ...
    expect(
      summarizeToolCall(
        "view",
        JSON.stringify({ path: "src/foo.ts", view_range: [1, 30] }),
      ),
    ).toBe("src/foo.ts");
  });
  ...
  it("falls back to first string arg for unknown tools", () => {
    expect(
      summarizeToolCall("unknown_tool", JSON.stringify({ x: "hello" })),
    ).toBe("hello");
  });
  ...
  it("summarizes structured git tool options", () => {
    expect(
      summarizeToolCall(
        ...
        JSON.stringify({
          output: "name-status",
          target: "worktree-vs-head",
          path: "src/a.ts",
        }),
      ),
    ).toBe("name-status · worktree-vs-head · src/a.ts");
    expect(
      summarizeToolCall("git_log", JSON.stringify({ path: "src/a.ts" })),
    ).toBe("src/a.ts");
    expect(
      summarizeToolCall(
        "git_show_commit",
        JSON.stringify({ sha: "abc1234", includePatch: true }),
      ),
    ).toBe("abc1234 · patch");
    expect(
      summarizeToolCall(
        "git_commit",
        JSON.stringify({ subject: "Add feature", paths: "all" }),
      ),
    ).toBe("Add feature · all changes");
    expect(
      summarizeToolCall(
        "git_worktree_merge",
        JSON.stringify({ direction: "to-source" }),
      ),
    ).toBe("integrate into source branch");
    expect(
      summarizeToolCall(
        "git_worktree_merge",
        JSON.stringify({ direction: "from-source" }),
      ),
  ...
  it("summarizes portal memory tool options", () => {
    expect(
      summarizeToolCall(
        "memory_search",
        JSON.stringify({ query: "blue candle" }),
      ),
    ).toBe("blue candle");
    expect(
      summarizeToolCall(
        "memory_get_entity",
        JSON.stringify({ id: "character.elias" }),
      ),
    ).toBe("character.elias");
    expect(
      summarizeToolCall(
  ...
  it("summarizes portal ticket and permission tool options", () => {
    expect(
      summarizeToolCall(
        "ticket_add",
        JSON.stringify({ title: "Remember this" }),
      ),
    ).toBe("Remember this");
    expect(
      summarizeToolCall(
        "ticket_update",
        JSON.stringify({ id: "t1", status: "done" }),
      ),
    ).toBe("t1 · done");
    expect(
      summarizeToolCall("ticket_list", JSON.stringify({ status: "open" })),
    ).toBe("open");
    expect(
      summarizeToolCall(
    ...
    expect(
      parseGitToolResult(
        ...
        JSON.stringify({
          ...
          fileStats: [
            { path: "src/a.ts", origPath: null, added: 1, removed: 0 },
          ],
...
describe("decodeToolResult envelope", () => {
  it("decodes a success envelope with an object result as pretty JSON", () => {
    const r = decodeToolResult(
      JSON.stringify({ ok: true, result: { commits: [] } }),
    );
    expect(r.blocks).toEqual([
      { kind: "text", text: JSON.stringify({ commits: [] }, null, 2) },
    ]);
  ...
  it("decodes a success envelope with a string result", () => {
    const r = decodeToolResult(
      JSON.stringify({ ok: true, result: "plain text" }),
    );
  ...
  it("falls back to summary for a result-less success envelope", () => {
    const r = decodeToolResult(
      JSON.stringify({ ok: true, summary: "Added ticket t1" }),
    );
...
describe("decodeToolResult views", () => {
  ...
  it("falls back to JSON when only unknown view types are present", () => {
    ...
    expect(r.blocks).toEqual([
      { kind: "text", text: JSON.stringify({ foo: 1 }, null, 2) },
    ]);
...
describe("decodeToolResult modelText", () => {
  ...
  it("leaves modelText undefined for non-envelope records", () => {
    const r = decodeToolResult(
      JSON.stringify({ content: "x", detailedContent: "y" }),
    );
...
describe("getSubagentDisplayState", () => {
  ...
  it("extracts background agent ids from tolerated result shapes", () => {
    expect(
      getBackgroundAgentId(JSON.stringify({ agentId: "agent-camel" })),
    ).toBe("agent-camel");
    expect(getBackgroundAgentId(JSON.stringify({ id: "agent-id" }))).toBe(
      "agent-id",
    );
    expect(
      getBackgroundAgentId(
        JSON.stringify({
          content: [{ type: "text", text: "agent_id: agent-array" }],
        }),
      ),
    ).toBe("agent-array");
    expect(
      getBackgroundAgentId(
        JSON.stringify("Started. Use read_agent with agent-text."),
      ),
    ).toBe("agent-text");
  });
});
