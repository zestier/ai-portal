...
import {
  fetchTicketsPage,
  ticketsPageUrl,
} from "../../src/lib/client/tickets-list";
import {
  resolveInitialSidebarOpen,
  orderSidebarTickets,
} from "../../src/lib/client/sidebar";
...
describe("resolveInitialSidebarOpen", () => {
  ...
  it("treats unrecognized stored values as missing", () => {
    ...
    expect(
      resolveInitialSidebarOpen({ getStored: () => "", isDesktop: () => true }),
    ).toBe(true);
  });
});
...
describe("ticket action helpers", () => {
  ...
  it("uses the ticket title as the chat title", () => {
    expect(ticketActionChatTitle({ title: "Fix sidebar actions" })).toBe(
      "Fix sidebar actions",
    );
  });
  ...
  it("returns the action conversation-mode override or undefined for default", () => {
    expect(
      ticketActionConversationMode({ conversationMode: "interactive" }),
    ).toBe("interactive");
    expect(
      ticketActionConversationMode({ conversationMode: null }),
    ).toBeUndefined();
  });
...
describe("ticket list pagination helper", () => {
  it("builds an /api/tickets URL with status, workspace, limit and offset", () => {
    expect(
      ticketsPageUrl({
        status: "done",
        workspace: "/ws with spaces",
        limit: 20,
        offset: 40,
      }),
    ).toBe(
      "/api/tickets?status=done&workspace=%2Fws+with+spaces&limit=20&offset=40",
    );
  });

  it("omits the workspace param when there is no workspace", () => {
    expect(
      ticketsPageUrl({ status: "open", workspace: null, limit: 10, offset: 0 }),
    ).toBe("/api/tickets?status=open&limit=10&offset=0");
  });
  ...
  it("reports hasMore when a full page is returned", async () => {
    const page = await fetchTicketsPage({
      ...
      fetcher: async (url) => {
        expect(url).toBe(
          "/api/tickets?status=open&workspace=%2Fws&limit=2&offset=0",
        );
