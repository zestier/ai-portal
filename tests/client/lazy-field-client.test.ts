...
describe("lazy field client", () => {
  ...
  it("fetches once and memoizes, so a remount does not re-download", async () => {
    const fetchMock = vi.fn(
      async () => new Response("the-result", { status: 200 }),
    );
    vi.stubGlobal("fetch", fetchMock);

    expect(peekLazyField(C1, "tool-result", toolCallId.encode(1))).toBeNull();
    expect(await fetchLazyField(C1, "tool-result", toolCallId.encode(1))).toBe(
      "the-result",
    );
    expect(await fetchLazyField(C1, "tool-result", toolCallId.encode(1))).toBe(
      "the-result",
    );
    expect(peekLazyField(C1, "tool-result", toolCallId.encode(1))).toBe(
      "the-result",
    );
  ...
  it("makes a second concurrent caller genuinely await the first fetch", async () => {
    ...
    const a = fetchLazyField(C1, "tool-args", toolCallId.encode(1)).then(
      (v) => {
        firstDone = true;
        return v;
      },
    );
    const b = fetchLazyField(C1, "tool-args", toolCallId.encode(1)).then(
      (v) => {
        secondDone = true;
        return v;
      },
  ...
  it("throws a retryable error and caches nothing on failure", async () => {
    ...
    await expect(fetchLazyField(C1, "file-diff", 3)).rejects.toThrow(
      /Could not load/,
    );
  ...
  it("reports a missing field distinctly from a transport failure", async () => {
    ...
    await expect(
      fetchLazyField(C1, "tool-result", toolCallId.encode(9)),
