...
describe("languageForPath", () => {
  it("detects the curated languages used by file and diff views", () => {
    ...
    expect(languageForPath("src/lib/client/syntax-highlight.ts")).toBe(
      "typescript",
    );
...
describe("highlightCodeLines", () => {
  it("escapes unsupported languages without injecting markup", async () => {
    const result = await highlightCodeLines(
...
describe("canHighlight", () => {
  it("enforces line and byte caps", () => {
    expect(
      canHighlight(Array.from({ length: MAX_HIGHLIGHT_LINES }, () => "x")),
    ).toBe(true);
    expect(
      canHighlight(Array.from({ length: MAX_HIGHLIGHT_LINES + 1 }, () => "x")),
    ).toBe(false);
...
describe("escapeHtml", () => {
  it("escapes all HTML-sensitive characters", () => {
    expect(escapeHtml(`<tag attr="a&b">'`)).toBe(
      "&lt;tag attr=&quot;a&amp;b&quot;&gt;&#39;",
    );
  });
});
