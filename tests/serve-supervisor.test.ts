...
import {
  restartDelayMs,
  shouldRollback,
} from "../scripts/lib/serve-supervisor.mjs";
...
describe("shouldRollback", () => {
  ...
  it("rolls back on an early non-zero exit when a previous tree exists", () => {
    expect(
      shouldRollback({ code: 1, uptimeMs: 800, hasPrev: true, thresholdMs }),
    ).toBe(true);
  });

  it("does not roll back without a previous tree to restore", () => {
    expect(
      shouldRollback({ code: 1, uptimeMs: 800, hasPrev: false, thresholdMs }),
    ).toBe(false);
  });

  it("does not roll back a clean exit (the redeploy self-restart)", () => {
    expect(
      shouldRollback({ code: 0, uptimeMs: 10, hasPrev: true, thresholdMs }),
    ).toBe(false);
  });
  ...
  it("does not roll back a signal kill (code === null)", () => {
    ...
    expect(
      shouldRollback({ code: null, uptimeMs: 5, hasPrev: true, thresholdMs }),
    ).toBe(false);
  });
  ...
  it("does not roll back a non-zero exit that happened after the threshold", () => {
    ...
    expect(
      shouldRollback({ code: 1, uptimeMs: 60_000, hasPrev: true, thresholdMs }),
    ).toBe(false);
  });
