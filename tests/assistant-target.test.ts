...
type Msg = { id: number; role: Role };

const m = (id: number, role: Role): Msg => ({ id, role });
...
describe('resolveAssistantTarget', () => {
	it('targets the assistant message matching the event messageId', () => {
		...
		expect(resolveAssistantTarget(messages, 2)).toEqual({ kind: 'found', index: 1 });
	});
	...
	it('refreshes when the messageId is not present locally (reconnect gap)', () => {
		...
		expect(resolveAssistantTarget(messages, 99)).toEqual({ kind: 'refresh' });
	});

	it('refreshes when the messageId resolves to a non-assistant message', () => {
		const messages = [m(1, 'user'), m(2, 'assistant')];
		expect(resolveAssistantTarget(messages, 1)).toEqual({ kind: 'refresh' });
	});
	...
	it('refreshes on an empty thread', () => {
		...
		expect(resolveAssistantTarget([], 2)).toEqual({ kind: 'refresh' });
	});
});
