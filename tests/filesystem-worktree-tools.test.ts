...
describe('filesystem tools inside a worktree lease', () => {
	...
	it('move renames inside the lease', async () => {
		...
		const res = await tools
			.get('move')!
			.handler({ source: 'a.txt', destination: 'nested/b.txt', worktree: leaseId });

		expect(expectOk(res)).toMatchObject({ source: 'a.txt', destination: 'nested/b.txt' });
		expect(existsSync(join(leasePath, 'a.txt'))).toBe(false);
		expect(readFileSync(join(leasePath, 'nested/b.txt'), 'utf-8')).toBe('hello');
	});
	...
	it('trash buries the file in the lease’s own trash store', async () => {
		...
		const res = await tools.get('trash')!.handler({ path: 'doomed.txt', worktree: leaseId });
		...
		expect(readFileSync(join(leasePath, payload.trashPath), 'utf-8')).toBe('bye');
		const meta = JSON.parse(
			readFileSync(join(leasePath, TRASH_DIR, payload.entryId, 'meta.json'), 'utf-8')
		);
		expect(meta).toMatchObject({ originalPath: 'doomed.txt', name: 'doomed.txt', type: 'file' });
	...
	it('trash still refuses to bury the lease’s trash store itself', async () => {
		...
		const res = await tools.get('trash')!.handler({ path: TRASH_DIR, worktree: leaseId });

		expect(expectErr(res).message).toBe('refusing to trash the trash store itself');
	});
	...
	describe('derived permission request', () => {
		it('names the path inside the lease', () => {
			...
			expect(req).toEqual({ permissionKind: 'write', path: join(leasePath, 'pkg') });
		});
		...
		it('still derives against the workspace when no lease is selected', () => {
			const req = tools.get('trash')!.derivePermissionRequest!({ path: 'README.md' });

			expect(req).toEqual({ permissionKind: 'write', path: join(source, 'README.md') });
		});
	});
});
