...
describe("placeholder registry", () => {
  ...
  it("extracts distinct placeholder names in first-seen order", () => {
    expect(
      extractPlaceholders("{{ticket.title}} {{ ticket.id }} {{ticket.title}}"),
    ).toEqual(["ticket.title", "ticket.id"]);
  });
  ...
  it("flags unknown placeholders per type", () => {
    ...
    expect(findUnknownPlaceholders("{{ticket.body}}", "chat")).toEqual([
      "ticket.body",
    ]);
    expect(findUnknownPlaceholders("{{ticket.nope}}", "ticket-action")).toEqual(
      ["ticket.nope"],
    );
  });
  ...
  it("builds type-aware unknown-placeholder messages", () => {
    ...
    const ticketMsg = unknownPlaceholderMessage("ticket-action", [
      "ticket.nope",
    ]);
...
describe("interpolatePrompt", () => {
  ...
  it("renders an empty plan as (none) and trims a non-empty plan", () => {
    expect(
      ticketPlaceholderValues({ id: "T1", title: "x", body: "b", plan: "   " })[
        "ticket.plan"
      ],
    ).toBe("(none)");
    expect(
...
describe("ticket action defaults", () => {
  it("seeds Do, Draft, and Refine", () => {
    expect(TICKET_ACTION_DEFAULTS.map((d) => d.key)).toEqual([
      "do",
      "draft",
      "refine",
    ]);
  ...
  it("forces interactive mode only for refine", () => {
    const byKey = Object.fromEntries(
      TICKET_ACTION_DEFAULTS.map((d) => [d.key, d]),
    );
