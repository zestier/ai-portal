...
import type { ProviderOpenOptions, ProviderSession } from '../src/lib/server/pi/session-contract';
...
function userText(m: { role?: string; content?: unknown }): string {
	...
	if (Array.isArray(m.content)) {
		return m.content
			.filter((b): b is { text: string } => !!b && typeof b === 'object' && 'text' in b)
			.map((b) => b.text)
			.join('');
	}
	return '';
...
describe('pi session persistence', () => {
	...
	it(
		...
		async () => {
			...
			const { create, get } = await import('../src/lib/server/db/repos/conversations');
			const { append } = await import('../src/lib/server/db/repos/messages');
			const { startTurnFromUserMessage } = await import('../src/lib/server/turn-start');
			...
			expect(activeUserMessages(file, wd)).toEqual(['first turn', 'second turn']);
		},
		T
	);
	...
	it(
		...
		async () => {
			...
			const current = append(conv.id, { role: 'user', content: 'current prompt' });

			const turn = await startTurnFromUserMessage(conv, current, { rerun: true });
