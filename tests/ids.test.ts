...
describe("makeId", () => {
  it("round-trips encode -> parse", () => {
    ...
    expect(ticketId.parse(ticketId.encode(Number.MAX_SAFE_INTEGER))).toBe(
  ...
  it("rejects numeric-only input", () => {
    expect(() => ticketId.parse("10")).toThrow(
      "not a ticket id: 10 — expected T<number>",
    );
    expect(ticketId.is("10")).toBe(false);
  });

  it("rejects a wrong prefix", () => {
    expect(() => ticketId.parse("C10")).toThrow(
      "not a ticket id: C10 — expected T<number>",
    );
    expect(() => ticketId.parse("X10")).toThrow(
      "not a ticket id: X10 — expected T<number>",
    );
  });

  it("rejects leading zeros", () => {
    expect(() => ticketId.parse("T007")).toThrow(
      "not a ticket id: T007 — expected T<number>",
    );
    expect(() => ticketId.parse("T00001")).toThrow();
  });

  it("rejects zero and negatives", () => {
    expect(() => ticketId.parse("T0")).toThrow(
      "not a ticket id: T0 — expected T<number>",
    );
    expect(() => ticketId.parse("T-1")).toThrow(
      "not a ticket id: T-1 — expected T<number>",
    );
  });

  it("rejects non-safe integers", () => {
    // 9e15 > MAX_SAFE_INTEGER (9007199254740991) — parses to a float / unsafe int.
    expect(() => ticketId.parse("T9007199254740992")).toThrow(
      "not a ticket id: T9007199254740992",
    );
  ...
  it("error() produces a precise, greppable message", () => {
    expect(ticketId.error("10")).toBe(
      "not a ticket id: 10 — expected T<number>",
    );
    expect(ticketId.error("T007")).toBe(
      "not a ticket id: T007 — expected T<number>",
    );
    expect(ticketId.error(10)).toBe("not a ticket id: 10 — expected T<number>");
    expect(ticketId.error(null)).toBe(
      "not a ticket id: null — expected T<number>",
    );
  });
  ...
  it("multi-letter prefixes work (PT, PC)", () => {
    ...
    expect(promptTemplateId.parse("pt3")).toBe(3);
    expect(() => promptTemplateId.parse("P3")).toThrow(
      "not a prompt template id: P3",
    );
...
describe("prefixed handle instances", () => {
  ...
  it("conversation/message/tool-call ids do not cross-parse", () => {
    ...
    expect(() => conversationId.parse("M5")).toThrow(
      "not a conversation id: M5",
    );
