...
function multiEditTool(workspace: string) {
	return buildMultiEditTools(workspace).find((candidate) => candidate.name === 'multi_edit')!;
}
...
describe('multi_edit', () => {
	it('applies edits across multiple files, reporting per-edit FileEditOutput entries', async () => {
		await withWorkspace(async (workspace) => {
			...
			expect(result).toMatchObject({ ok: true, summary: 'Applied 2 edit(s) across 2 file(s).' });
	...
	it('applies edits sequentially within a file, matching later edits against prior results', async () => {
		await withWorkspace(async (workspace) => {
			...
			const result = await multiEditTool(workspace).handler({
				edits: [
					{ file_path: 'sample.txt', old_string: 'gamma three', new_string: 'gamma THREE' },
					{ file_path: 'sample.txt', old_string: 'beta two', new_string: 'beta TWO' }
				]
			});
			...
			if (result.ok) {
				const output = result.result as { edits: Array<{ originalFile: string }> };
				// The second edit matched against the content AFTER the first edit.
				expect(output.edits[1]?.originalFile).toBe('gamma THREE\nbeta two\ngamma three\n');
			}
		});
	});
	...
	it('replace_all replaces every occurrence', async () => {
		await withWorkspace(async (workspace) => {
			...
			const result = await multiEditTool(workspace).handler({
				edits: [
					{ file_path: 'sample.txt', old_string: 'twenty', new_string: 'TWENTY', replace_all: true }
				]
			});
			...
			if (result.ok) {
				const output = result.result as { edits: Array<{ replaceAll: boolean }> };
		...
		await withWorkspace(async (workspace) => {
			...
			const result = await multiEditTool(workspace).handler({
				edits: [
					...
					{ file_path: 'b.txt', old_string: 'does not exist', new_string: 'edited-b' }
				]
			});
	...
	it('names the failing edit index, path, and unmatched string', async () => {
		await withWorkspace(async (workspace) => {
			...
			const result = await multiEditTool(workspace).handler({
				edits: [
					{ file_path: 'sample.txt', old_string: 'hello', new_string: 'goodbye' },
	...
	it('reports a missing file with the failing edit index', async () => {
		await withWorkspace(async (workspace) => {
			...
			expect(result).toMatchObject({
				ok: false,
				error: { code: 'edit_failed', message: 'edits[0] (missing.txt): file not found' }
			});
		});
	});
	...
	it('dryRun validates without writing anything', async () => {
		await withWorkspace(async (workspace) => {
			...
			const result = await multiEditTool(workspace).handler({
				edits: [{ file_path: 'sample.txt', old_string: 'before', new_string: 'after' }],
				dryRun: true
			});
	...
	it('reports a gitDiff per edit when the targets are in a git repo', async () => {
		await withWorkspace(async (workspace) => {
			...
			if (result.ok) {
				...
				expect(output.edits[0]?.gitDiff).toMatchObject({ filename: 'app.ts', status: 'modified' });
			}
		});
	});
	...
	it('derives an edit-kind permission request covering every target', async () => {
		await withWorkspace(async (workspace) => {
			...
			expect(
				derive?.({ edits: [{ file_path: 'bad\0path', old_string: 'a', new_string: 'b' }] })
			).toBeNull();
		});
	});
