...
import type {
  ActionInput,
  ActionStep,
} from "../../../src/lib/server/actions/config";
...
describe("tokensIn", () => {
  it("extracts distinct input names, tolerating inner whitespace", () => {
    expect(tokensIn("--ref={{ ref }} --to={{target}} {{ref}}")).toEqual([
      "ref",
      "target",
    ]);
  });
...
describe("resolveInputValues", () => {
  ...
  it("falls back to a default and enforces required", () => {
    const withDefault = resolveInputValues(
      [input({ name: "env", default: "staging" })],
      {},
    );
    ...
    if (!missingRequired.ok)
      expect(missingRequired.error).toContain("required");
  });

  it("resolves an optional unset input to the empty string", () => {
    const res = resolveInputValues(
  ...
  it("validates enum membership", () => {
    const inputs = [
      input({ name: "env", type: "enum", options: ["staging", "prod"] }),
    ];
  ...
  it("coerces and validates numbers", () => {
    ...
    expect(resolveInputValues(inputs, { count: 5 })).toEqual({
      ok: true,
      values: { count: "5" },
    });
  ...
  it("rejects an over-long string value", () => {
    const res = resolveInputValues([input({ name: "blob" })], {
      blob: "x".repeat(8193),
    });
...
describe("substituteArg / substituteSteps", () => {
  it("replaces tokens with their resolved values as whole literals", () => {
    expect(substituteArg("--ref={{ref}}", { ref: "feature/x y" })).toBe(
      "--ref=feature/x y",
    );
  });
  ...
  it("leaves a value with shell metacharacters intact (no shell interpretation)", () => {
    ...
    expect(substituteArg("{{msg}}", { msg: "$(rm -rf /)" })).toBe(
      "$(rm -rf /)",
    );
  });
