...
describe("parseActionsConfig — fail closed", () => {
  ...
  it("rejects unknown action keys", () => {
    const res = parseActionsConfig({
      version: 1,
      actions: [
        { id: "x", label: "X", steps: [{ command: "ls" }], surprise: 1 },
      ],
    });
    expect(res.ok).toBe(false);
  });
  ...
  it("rejects a cwd field with a message naming the reserved capability", () => {
    const res = parseActionsConfig({
      version: 1,
      actions: [
        { id: "x", label: "X", steps: [{ command: "ls", cwd: "/etc" }] },
      ],
    });
    expect(res.ok).toBe(false);
    if (res.ok) return;
  ...
  it("rejects a rollover/restarting field (built-in-only)", () => {
    for (const key of ["rollover", "restarting", "restart", "inheritEnv"]) {
      const res = parseActionsConfig({
        version: 1,
        actions: [
          { id: "x", label: "X", [key]: true, steps: [{ command: "ls" }] },
        ],
      });
  ...
  it("rejects an env entry that names one of the portal\u2019s own secrets", () => {
    for (const name of ["ENCRYPTION_KEY"]) {
      const res = parseActionsConfig({
        version: 1,
        actions: [
          { id: "x", label: "X", env: [name], steps: [{ command: "ls" }] },
        ],
      });
  ...
  it("rejects lowercase / malformed env names", () => {
    const res = parseActionsConfig({
      version: 1,
      actions: [
        { id: "x", label: "X", env: ["vercel"], steps: [{ command: "ls" }] },
      ],
    });
    expect(res.ok).toBe(false);
  });
      ...
      actions: [
        {
          ...
          steps: [
            {
              command: "pnpm",
              args: ["run", "deploy", "--env={{env}}", "--note={{note}}"],
            },
          ],
        },
      ],
      ...
      actions: [
        {
          ...
          label: "X",
          inputs: [
            { name: "env", label: "Env", type: "string", options: ["a"] },
          ],
          steps: [{ command: "ls" }],
        },
      ],
