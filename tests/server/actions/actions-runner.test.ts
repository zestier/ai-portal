...
function logsOf(events: ActionEvent[]) {
  return events.filter(
    (ev): ev is Extract<ActionEvent, { type: "log" }> => ev.type === "log",
  );
}
...
describe("runStep env model", () => {
  it("default-inherits process.env (built-in behavior) and merges step.env", async () => {
    ...
    try {
      const code = await runStep(
        nodeStep(
          "inherit",
          'process.stdout.write(process.env.ZAP_RUNNER_PROBE ?? "unset")',
        ),
        (ev) => events.push(ev),
      );
      expect(code).toBe(0);
      expect(logsOf(events).some((ev) => ev.text.includes("inherited"))).toBe(
        true,
      );
    } finally {
  ...
  it("inheritEnv:false gives the child EXACTLY step.env — a secret is not visible", async () => {
    ...
    try {
      ...
      expect(code).toBe(0);
      expect(logsOf(events).some((ev) => ev.text.includes("[absent]"))).toBe(
        true,
      );
      expect(logsOf(events).every((ev) => !ev.text.includes("leak-me"))).toBe(
        true,
      );
    } finally {
...
describe("runStep cwd selection", () => {
  ...
  it("defaults cwd to process.cwd() (built-in behavior)", async () => {
    const events: ActionEvent[] = [];
    await runStep(
      nodeStep("defaultcwd", "process.stdout.write(process.cwd())"),
      (ev) => events.push(ev),
    );
    const out = logsOf(events)
      .map((ev) => ev.text)
...
describe("runStep spawn error", () => {
  it("emits a single step-done with a non-zero code when the command does not exist", async () => {
    ...
    expect(logsOf(events).some((ev) => ev.text.includes("spawn error"))).toBe(
      true,
    );
  });
});
...
describe("runSequence rollover", () => {
  ...
  it("emits a plain ok done (no restart) when rollover is false", async () => {
    const events = await drain([nodeStep("ok", "process.exit(0)")], {
      rollover: false,
    });
...
describe("scrubLog", () => {
  it("redacts sensitive env values and token-shaped strings", () => {
    const scrubbed = scrubLog(
      "SECRET=super-secret-value ghp_abcdefghijklmnopqrstuvwxyz",
      {
