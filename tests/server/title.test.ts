...
describe("deriveTitle", () => {
  ...
  it("uses the first sentence when short enough", () => {
    expect(deriveTitle("Fix the login bug. Then add tests.")).toBe(
      "Fix the login bug",
    );
  });
  ...
  it("keeps inline code text but drops backticks", () => {
    expect(deriveTitle("Refactor `parseUrl` helper")).toBe(
      "Refactor parseUrl helper",
    );
  });

  it("keeps link labels and drops URLs", () => {
    expect(
      deriveTitle("See [the docs](https://example.com/x) for details."),
