...
describe('BoundedTtlCache', () => {
	...
	it('caps growth even when many distinct keys are inserted', () => {
		const cache = new BoundedTtlCache<number, number>({ ttlMs: 1_000, maxEntries: 1000 });
	...
	it('supports delete and clear', () => {
		const cache = new BoundedTtlCache<string, number>({ ttlMs: 1_000, maxEntries: 10 });
		cache.set('a', 1);
		cache.set('b', 2);
		cache.delete('a');
		expect(cache.get('a')).toBeUndefined();
	...
	it('rejects a non-positive cap', () => {
		expect(() => new BoundedTtlCache<string, number>({ ttlMs: 1, maxEntries: 0 })).toThrow();
	});
});
