...
describe("log", () => {
  ...
  it("rejects invalid refs and path escapes", async () => {
    await expect(git.log(repo, { ref: "--all" })).rejects.toThrow(
      "invalid ref",
    );
    await expect(git.log(repo, { path: "../escape" })).rejects.toThrow(
      "invalid path",
    );
  });

  it("rejects stash/reflog refs", async () => {
    await expect(git.log(repo, { ref: "stash@{0}" })).rejects.toThrow(
      "invalid ref",
    );
    await expect(git.log(repo, { ref: "@{upstream}" })).rejects.toThrow(
      "invalid ref",
    );
    await expect(git.log(repo, { ref: "refs/stash" })).rejects.toThrow(
      "invalid ref",
    );
  });
...
describe("diff", () => {
  ...
  it("rejects path escape", async () => {
    await expect(
      git.diff(repo, { kind: "worktree-vs-head" }, "../escape"),
    ).rejects.toThrow();
  });
  ...
  it("returns structured stat and path-filtered name outputs", async () => {
    ...
    await expect(
      git.nameOnly(repo, { kind: "worktree-vs-head" }, "sub/b.txt"),
    ).resolves.toEqual([]);
    await expect(
      git.nameOnly(repo, { kind: "worktree-vs-head" }),
    ).resolves.toEqual(["a.txt"]);
    await expect(
      git.nameStatus(repo, { kind: "worktree-vs-head" }),
    ).resolves.toEqual([
...
describe("showCommit / showFile", () => {
  ...
  it("rejects stash/reflog refs", async () => {
    await expect(git.showFile(repo, "stash@{0}", "a.txt")).rejects.toThrow(
      "invalid ref",
    );
    await expect(git.showFile(repo, "refs/stash", "a.txt")).rejects.toThrow(
      "invalid ref",
    );
    await expect(git.showFile(repo, "@{-1}", "a.txt")).rejects.toThrow(
      "invalid ref",
    );
  });
...
describe("structured git tools", () => {
  ...
  it("rejects unknown structured git tool properties", async () => {
    ...
    await expect(tool!.handler({ limit: 1, flags: ["--all"] })).rejects.toThrow(
      "Unrecognized key",
    );
  });
  ...
  it("wires git_commit as an always-prompt structured tool", async () => {
    ...
    await expect(
      tool!.handler({ paths: [], subject: "empty" }),
    ).rejects.toThrow();
    await expect(
      tool!.handler({ paths: "all", subject: "bad\nsubject" }),
    ).rejects.toThrow();
    await expect(
      tool!.handler({
        paths: "all",
...
describe("commitChanges", () => {
  ...
  it("commits all tracked, deleted, and untracked changes", async () => {
    ...
    try {
      ...
      expect(
        execFileSync("git", ["status", "--porcelain"], { cwd: tmp }).toString(),
      ).toBe("");
      expect(
        execFileSync("git", ["log", "-1", "--pretty=%s"], { cwd: tmp })
          .toString()
          .trim(),
      ).toBe("commit all");
  ...
  it("rejects selected commits when unrelated staged changes exist", async () => {
    ...
    try {
      ...
      writeFileSync(join(tmp, "b.txt"), "two\nstaged\n");
      run(["add", "b.txt"]);
      const before = execFileSync("git", ["rev-parse", "HEAD"], { cwd: tmp })
        .toString()
        .trim();
  ...
  it("formats body and structured trailers deterministically", async () => {
    ...
    try {
      ...
      expect(result.trailers).toEqual([
        { token: "Reviewed-by", value: "Tester <t@example.com>" },
      ]);
      expect(
  ...
  it("rejects C1 control characters and ANSI escapes in the subject", () => {
    ...
    expect(() =>
      git.formatCommitMessage({ subject: "line\u0085forged" }),
    ).toThrow("control characters");
    expect(() =>
      git.formatCommitMessage({ subject: "clear\u001b[2Jscreen" }),
    ).toThrow("control characters");
  });

  it("rejects control characters in the body but allows ordinary whitespace", () => {
    expect(() =>
      git.formatCommitMessage({ subject: "ok", body: "nel\u0085here" }),
    ).toThrow("control characters");
    expect(() =>
      git.formatCommitMessage({ subject: "ok", body: "ansi\u001b[31mred" }),
    ).toThrow("control characters");
    expect(() =>
      git.formatCommitMessage({ subject: "ok", body: "NUL\u0000here" }),
    ).toThrow("control characters");
...
describe("commitChanges streaming context", () => {
  ...
  it("bounds the error message from a verbose failing pre-commit hook", async () => {
    ...
    try {
      ...
      writeFileSync(join(tmp, "a.txt"), "one\nchanged\n");
      const err = await git
        .commitChanges(tmp, { paths: "all", subject: "x" })
        .catch((e) => e);
