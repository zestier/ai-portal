...
function conflictedRepo() {
	...
	const preMergeSha = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: tmp }).toString().trim();
...
function porcelain(tmp: string) {
	return execFileSync('git', ['status', '--porcelain'], { cwd: tmp }).toString();
}
...
describe('commitChanges on a conflicted merge', () => {
	it('concludes the merge once the conflict is resolved', async () => {
		...
		try {
			...
			const parents = execFileSync('git', ['rev-list', '-1', '--parents', result.sha], { cwd: tmp })
				.toString()
				.trim()
				.split(/\s+/);
			expect(parents).toHaveLength(3);
			// The merge is over and the tree is clean, so it is mergeable again.
			expect(await git.mergeState(tmp)).toMatchObject({ inProgress: false, conflictedPaths: [] });
			...
			expect(result.files.map((f) => f.path).sort()).toEqual(['a.txt', 'c.txt']);
	...
	it('stages only the resolved conflicts, not unrelated work in the tree', async () => {
		...
		try {
			...
			const result = await git.commitChanges(tmp, { paths: 'all', subject: 'resolve only' });
			...
			const after = await git.commitChanges(tmp, { paths: 'all', subject: 'the rest' });
			expect(after.mergeCommit).toBe(false);
			expect(after.files.map((f) => f.path).sort()).toEqual(['b.txt', 'scratch.txt']);
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	});
	...
	it('refuses to commit a file that still has conflict markers', async () => {
		...
		try {
			...
			resolve(tmp);
			const result = await git.commitChanges(tmp, { paths: 'all', subject: 'resolved' });
			expect(result.mergeCommit).toBe(true);
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	...
	it('still commits when the resolution reproduces the pre-merge content', async () => {
		...
		try {
			...
			const result = await git.commitChanges(tmp, { paths: 'all', subject: 'take ours' });
			expect(result.mergeCommit).toBe(true);
	...
	it('leaves the merge finishable when a hook rejects the commit', async () => {
		...
		try {
			...
			await expect(git.commitChanges(tmp, { paths: 'all', subject: 'rejected' })).rejects.toThrow();
			expect(await git.mergeState(tmp)).toMatchObject({
				inProgress: true,
				conflictedPaths: ['a.txt']
			});
			...
			writeFileSync(hook, '#!/bin/sh\nexit 0\n');
			const result = await git.commitChanges(tmp, { paths: 'all', subject: 'accepted' });
			expect(result.mergeCommit).toBe(true);
			expect(await git.mergeState(tmp)).toMatchObject({ inProgress: false });
		} finally {
			rmSync(tmp, { recursive: true, force: true });
...
describe('abortMerge', () => {
	...
	it('refuses when no merge is in progress', async () => {
		const { tmp } = initRepo();
		try {
			await expect(git.abortMerge(tmp)).rejects.toThrow('no merge is in progress');
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	});
...
describe('unmerged paths with no merge in progress', () => {
	...
	function cherryPickConflict() {
		...
		const featureSha = execFileSync('git', ['rev-parse', 'HEAD'], { cwd: tmp }).toString().trim();
		run(['checkout', '-q', 'main']);
		writeFileSync(join(tmp, 'a.txt'), 'one\nmain\n');
		run(['commit', '-q', '-am', 'main change']);
		let conflicted = false;
	...
	it('names only the commit path when there is nothing to abort or continue', async () => {
		...
		try {
			...
			expect(out.ok && out.followUpHint).toContain('git_merge_abort does not apply');
	...
	it('admits a cherry-pick cannot be continued through the portal', async () => {
		...
		try {
			...
			const out = await gitTool(tmp, 'git_status').handler({});
			expect(out.ok && out.followUpHint).toBe(sequencerFollowUpHint('cherry-pick'));
			// The ticket forbids claiming a capability the surface lacks: the hint
			// has to say the commit does not advance the operation.
			expect(out.ok && out.followUpHint).toContain('does NOT advance the cherry-pick');
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	});
	...
	it('commits the resolution as an ordinary commit', async () => {
		...
		try {
			...
			resolve(tmp);
			const result = await git.commitChanges(tmp, { paths: 'all', subject: 'resolve cherry-pick' });
			...
			expect(await git.mergeState(tmp)).toMatchObject({ conflictedPaths: [], sequencer: null });
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	});
...
describe('merge-aware git tools', () => {
	it('git_status reports the merge and points at the way out', async () => {
		...
		try {
			...
			expect(out.ok && (out.result as { merge: git.MergeState }).merge).toMatchObject({
				inProgress: true,
				conflictedPaths: ['a.txt']
			});
	...
	it('git_status omits the hint on a clean tree', async () => {
		...
		try {
			...
			expect(out.ok && (out.result as { merge: git.MergeState }).merge.inProgress).toBe(false);
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
	});
	...
	it('git_merge_abort is an always-prompt tool that clears the merge', async () => {
		...
		try {
			...
			const out = await tool!.handler({});
			expect(out.ok).toBe(true);
			expect(out.ok && (out.result as { headSha: string }).headSha).toBe(preMergeSha);
			expect(await git.mergeState(tmp)).toMatchObject({ inProgress: false });
	...
	it('git_merge_abort rejects unknown properties', async () => {
		const tool = buildGitTools(mkdtempSync(join(tmpdir(), 'gitmerge-args-'))).find(
			(t) => t.name === 'git_merge_abort'
		);
		await expect(tool!.handler({ hard: true })).rejects.toThrow('Unrecognized key');
	});
	...
	it('git_commit concludes a merge through the tool surface', async () => {
		...
		try {
			...
			resolve(tmp);
			const out = await tool!.handler({ paths: 'all', subject: 'resolve conflict' });
			expect(out.ok).toBe(true);
			expect(out.ok && (out.result as { mergeCommit: boolean }).mergeCommit).toBe(true);
			expect(await git.mergeState(tmp)).toMatchObject({ inProgress: false });
		} finally {
			rmSync(tmp, { recursive: true, force: true });
		}
