...
describe("fork.forkAtMessage", () => {
  ...
  it("clones prior history + the edited message and shares the source workdir", async () => {
    ...
    const u1 = messages.append(sourceConv.id, {
      role: "user",
      content: "first",
    });
    ...
    const u2 = messages.append(sourceConv.id, {
      role: "user",
      content: "second",
    });
    ...
    const sourceIds = new Set(
      messages.listByConversation(sourceConv.id).map((m) => m.id),
    );
  ...
  it("carries auto-deny into a fork but never auto-approve", async () => {
    ...
    const denyUser = messages.append(denySource.id, {
      role: "user",
      content: "go",
    });
    ...
    expect(denyFork.conversation).toMatchObject({
      mode: "autopilot",
      approvalMode: "auto-deny",
    });
    ...
    const approveUser = messages.append(approveSource.id, {
      role: "user",
      content: "go",
    });
  ...
  it("creates an isolated worktree at the historical snapshot state", async () => {
    ...
    const target = messages.append(source.id, {
      role: "user",
      content: "change it",
    });
    ...
    expect(
      readFileSync(join(result.conversation.workdir, "state.txt"), "utf8"),
    ).toBe("historical\n");
    expect(
      readFileSync(join(result.conversation.workdir, "untracked.txt"), "utf8"),
    ).toBe("captured\n");
    expect(readFileSync(join(source.workdir, "state.txt"), "utf8")).toBe(
      "current\n",
    );
  ...
  it("creates an isolated worktree from a legacy snapshot without a recorded base commit", async () => {
    const { users, convs, messages, snapshots, fork, db } =
      await freshImports();
    const u = users.ensureLocalUser();
    ...
    db.getDb()
      .prepare(
        "UPDATE turn_snapshots SET base_commit_sha = NULL WHERE message_id = ?",
      )
  ...
  it("gives a managed-source fork an independent worktree with a stable repository source", async () => {
    ...
    execFileSync("git", ["config", "user.name", "Portal Test"], {
      cwd: repository,
    });
    execFileSync("git", ["config", "user.email", "portal-test@localhost"], {
      cwd: repository,
    ...
    const target = messages.append(parent.id, {
      role: "user",
      content: "change it",
    });
    ...
    const childWorktree = convs.getManagedWorktree(
    ...
    await expect(
      removeManagedWorktree(childWorktree!, { force: true }),
  ...
  it("retries from an assistant message: clones up to and including it, no new user msg", async () => {
    ...
    const a1 = messages.append(sourceConv.id, {
      role: "assistant",
      content: "reply 1",
    });
  ...
  it("carries prefix session memory into the fork but leaves rewound-suffix memory behind", async () => {
    const { users, convs, messages, memory, engine, db, fork } =
      await freshImports();
    const u = users.ensureLocalUser();
    ...
    const prefixCommit = engine.commitPatch({
      ...
      patch: {
        ...
        facts: [
          ...
          {
            entityKey: "object.key",
            predicate: "stale_plan",
            value: "Use the discarded plan.",
          },
        ],
        openLoops: [
          {
            loopType: "task",
            title: "Inspect the study",
            relatedEntityKeys: ["object.key"],
          },
        ],
      },
    });
    ...
    const entityPatchItem = memory
      .listPatchItems(sourceConv.id, {
        patchId: prefixCommit.patch.id,
        limit: 20,
      })
    ...
    const a2 = messages.append(sourceConv.id, {
      role: "assistant",
      content: "reply 2",
    });
    engine.commitPatch({
      ...
      patch: {
        ...
        facts: [
          { entityKey: "object.door", predicate: "state", value: "locked" },
        ],
      },
    });
    ...
    for (const table of [
      ...
      conn
        .prepare(
          `UPDATE ${table} SET created_at = ? WHERE source_message_id = ?`,
        )
    ...
    conn
      .prepare(
        `UPDATE memory_entities SET created_at = ? WHERE entity_key = 'object.key'`,
      )
      .run(boundary - 100);
    conn
      .prepare(
        `UPDATE memory_entities SET created_at = ? WHERE entity_key = 'object.door'`,
      )
      .run(boundary + 100);
    ...
    expect(memory.listEntities(forkId).map((e) => e.entityKey)).toEqual([
      "object.key",
    ]);
    ...
    expect(memory.listEntities(forkId).map((e) => e.entityKey)).not.toContain(
      "object.door",
    );
    ...
    expect(
      memory.search(forkId, { query: "study key" }).length,
    ).toBeGreaterThan(0);
    expect(
      memory.listPatchItems(forkId).find((item) => item.itemType === "entity")
        ?.reviewStatus,
    ).toBe("approved");
  ...
  it("inherits pinned per-session directives into the fork", async () => {
    const { users, convs, messages, memory, engine, fork } =
      await freshImports();
    const u = users.ensureLocalUser();
    ...
    const inherited = memory.listFacts(forkId, {
      predicate: "directive",
      limit: 10,
    });
