...
vi.mock("../../../src/lib/server/runtime/turn-runner", () => ({
  ...
  startExtractionRetryTurn: (...args: unknown[]) =>
    startExtractionRetryTurnMock(...args),
}));
...
async function loadPost() {
  const { POST } =
    await import("../../../src/routes/api/conversations/[id]/memory/+server");
  return POST;
}
...
describe("memory retry-extraction endpoint", () => {
  ...
  it("defers the patch undo to the retry turn and starts an extraction-only retry", async () => {
    ...
    const userMsg = messages.append(conv.id, {
      role: "user",
      content: "remember the attic key",
    });
    ...
    const patch = memory
      .listPatches(conv.id)
      .find((p) => p.id === committed.patch.id);
