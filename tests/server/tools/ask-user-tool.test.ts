...
async function makeHarness() {
	...
	const { buildAskUserTool } = await import('../../../src/lib/server/tools/ask-user');
...
async function driveAndAnswer(
	...
	const resolved = harness.interactive.resolve(view.requestId, harness.user.id, answer);
...
describe('ask_user', () => {
	...
	it('raises a user_input view carrying the question and resolves to the answer', async () => {
		...
		const { result, view } = await driveAndAnswer(
			harness,
			{ question: 'Which database do you prefer?', choices: ['sqlite', 'postgres'] },
		...
		expect(harness.events.some((ev) => ev.type === 'interactive.request')).toBe(true);
	...
	it('echoes wasFreeform true for a typed answer', async () => {
		...
		expect(result.ok).toBe(true);
		if (result.ok) {
			expect(result.result).toMatchObject({ wasFreeform: true, answer: 'something custom' });
		}
	});
