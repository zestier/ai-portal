...
function lsText(result: { ok: boolean; views?: { type: string; text?: string }[] }): string {
...
describe('ls', () => {
	...
	it('caps entries at the requested limit', async () => {
		await withWorkspace(async (workspace) => {
			for (const name of ['a.ts', 'b.ts', 'c.ts']) await writeFile(join(workspace, name), '');
			...
			const view = lsText(result);
			expect(view.split('\n').filter(Boolean).slice(0, 2)).toEqual(['a.ts', 'b.ts']);
