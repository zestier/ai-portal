...
describe("prompt-template tools", () => {
  ...
  it("exposes the five expected tools with correct permission behavior", async () => {
    ...
    for (const w of writes)
      expect(tools.find((t) => t.name === w)!.permissionBehavior).toBe(
        "always-prompt",
      );
    for (const r of reads)
      expect(tools.find((t) => t.name === r)!.permissionBehavior).toBe(
        "never-prompt",
      );
  });
  ...
  it("CRUD: create, list, get, update, archive a chat template", async () => {
    ...
    const rows =
      listed.ok && (listed.result as { templates: { id: number }[] }).templates;
    ...
    expect(updated.ok && (updated.result as { title: string }).title).toBe(
      "Renamed preset",
    );
    ...
    expect(archived.ok && (archived.result as { status: string }).status).toBe(
      "archived",
    );
    const openList = await byName("template_list").handler({});
    const openRows =
      openList.ok &&
      (openList.result as { templates: { id: number }[] }).templates;
    expect((openRows as { id: number }[]).some((t) => t.id === id)).toBe(false);

    // Archive is reversible via template_update status -> open.
    const reopened = await byName("template_update").handler({
      id: String(id),
      status: "open",
    });
    expect(reopened.ok && (reopened.result as { status: string }).status).toBe(
      "open",
    );
  });
  ...
  it("lists ticket-action defaults (lazily seeded) and built-ins", async () => {
    ...
    const rows =
      listed.ok &&
      (listed.result as { templates: { title: string }[] }).templates;
    ...
    const data =
      builtins.ok &&
      (builtins.result as { chat: unknown[]; ticketAction: unknown[] });
  ...
  it("scopes reads/writes to the authed user", async () => {
    ...
    expect(stillMine.ok && (stillMine.result as { title: string }).title).toBe(
      "mine",
    );
  });
});
