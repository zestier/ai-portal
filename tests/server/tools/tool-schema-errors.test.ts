...
describe("validatePortalToolArgs", () => {
  it("returns ok:true when args match the schema", () => {
    ...
    expect(
      validatePortalToolArgs(gitCommit, {
        paths: "all",
        subject: "fix: thing",
      }),
    ).toEqual({
  ...
  it("returns ok:false with schema feedback for missing fields", () => {
    ...
    expect(result.feedback).toMatch(
      /Expected JSON Schema for "git_commit" parameters:/,
    );
  });

  it("reports per-field issue paths for empty subject", () => {
    const gitCommit = getTool("git_commit");
    const result = validatePortalToolArgs(gitCommit, {
      paths: "all",
      subject: "",
    });
    expect(result.ok).toBe(false);
  ...
  it("rejects a JSON-encoded ticket fields array with copyable corrected arguments", () => {
    ...
    expect(
      validatePortalToolArgs(ticketList, { fields: ["id", "title"] }),
    ).toEqual({ ok: true });

    const result = validatePortalToolArgs(ticketList, {
      fields: '["id","title"]',
    });
    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("unreachable");
