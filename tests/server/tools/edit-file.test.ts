...
function tool(workspace: string, name: "edit" | "write") {
  return buildEditFileTools(workspace).find(
...
describe("cappedLevenshtein", () => {
  ...
  it("returns the sentinel (maxDist + 1) when the true distance exceeds the cap", () => {
    ...
    expect(
      cappedLevenshtein(Array.from("kitten"), Array.from("sitting"), 2),
    ).toBe(3);
  });
  ...
  it("matches the reference DP on random strings within a tight cap", () => {
    ...
    for (let trial = 0; trial < 200; trial++) {
      ...
      const a = Array.from(
        { length: lenA },
        () => alphabet[(trial + lenA) % 4]!,
      );
      const b = Array.from(
        { length: lenB },
        () => alphabet[(trial * 2 + lenB) % 4]!,
      );
...
describe("findClosestMatch", () => {
  ...
  it("returns null for files over the file size guard", () => {
    expect(
      findClosestMatch(
        "x".repeat(MAX_SUGGEST_FILE_BYTES + 1) + "\n",
        "gamma tree",
      ),
    ).toBeNull();
  });
...
describe("write", () => {
    ...
    await withWorkspace(async (workspace) => {
      ...
      expect(result).toMatchObject({
        ...
        result: {
          type: "create",
          originalFile: null,
          filePath: join(workspace, "nested/file.txt"),
        },
      });
      expect(
        await readFile(join(workspace, "nested", "file.txt"), "utf8"),
      ).toBe("hello\n");
      if (result.ok) {
  ...
  it("writes outside the workspace after authorization", async () => {
    await withWorkspace(async (workspace) => {
      const outside = await mkdtemp(join(tmpdir(), "portal-write-outside-"));
...
describe("edit", () => {
  ...
  it("suggests the closest matching region for a one-word typo (ACB-1)", async () => {
    await withWorkspace(async (workspace) => {
      ...
      if (!result.ok) {
        ...
        const suggestion = (
          result.error.details as { suggestion: { similarity: number } }
        ).suggestion;
  ...
  it("accepts an absolute file_path inside the workspace", async () => {
    await withWorkspace(async (workspace) => {
      ...
      expect(result).toMatchObject({ ok: true });
      expect(await readFile(join(workspace, "abs.txt"), "utf8")).toBe(
        "after\n",
      );
    });
  });

  it("edits outside the workspace after authorization", async () => {
    await withWorkspace(async (workspace) => {
      const outside = await mkdtemp(join(tmpdir(), "portal-edit-outside-"));
      const filePath = join(outside, "escape.txt");
  ...
  it("reports a gitDiff with status modified when the target is in a git repo", async () => {
    await withWorkspace(async (workspace) => {
      ...
      expect(result).toMatchObject({ ok: true });
      if (result.ok) {
        const gitDiff = (result.result as { gitDiff?: { status: string } })
          .gitDiff;
