...
async function makeHarness(approvalMode: ApprovalMode = "ask") {
  ...
  const tool = tools.find(
    (t) => t.name === "request_permission_grant",
  ) as PortalTool;
...
describe("request_permission_grant", () => {
  ...
  it("raises a grant-request dialog carrying the proposed scope and reason", async () => {
    ...
    const { result, view } = await driveAndResolve(
      ...
      "allow-always",
      {
        scope: { permissionKind: "shell", scope: SHELL_SCOPE },
    ...
    expect(grants[0].conversationId).toBe(
      convCodec.encode(harness.conversationId),
    );
  ...
  it("reports the human-edited (narrowed) scope, not the agent proposal", async () => {
    ...
    const { result } = await driveAndResolve(
      ...
      {
        tool: "shell",
        reason: "Scaffolding needs to run pnpm install.",
        scope: SHELL_SCOPE,
      },
      "allow-always",
      {
        scope: { permissionKind: "shell", scope: narrowed },
        applyToAllConversations: false,
      },
    );
