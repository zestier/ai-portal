...
describe("project", () => {
  ...
  it("treats array entries literally, including former sentinel names", () => {
    ...
    expect(() => project(row, { fields: ["*"], keep })).toThrow(
      /Unknown field/,
    );
    expect(() => project(row, { fields: ["id", "all"], keep })).toThrow(
      /Unknown field/,
    );
  });
  ...
  it('selects a field actually named "all" or "default" when present', () => {
    ...
    expect(project(row, { fields: ["default"], keep }).value).toEqual({
      default: "D",
    });
  });
  ...
  it("rejects a bare non-sentinel string instead of treating it as a one-field selector", () => {
    ...
    expect(() => project(row, { fields: "plan", keep })).toThrow(
      /use an array like \["plan"\]/,
    );
  });
  ...
  it("throws on requested field names that exist on no record", () => {
    ...
    expect(() => project(row, { fields: ["nope"], keep })).toThrow(
      /Unknown field\(s\) requested/,
    );
  ...
  it("validates against the keep allowlist even when there are no records", () => {
    ...
    expect(() => project([], { fields: ["whatever"], keep })).toThrow(
      /Unknown field/,
    );
  });
  ...
  it("preserves allowlisted empty containers but drops allowlisted absent values", () => {
    ...
    const { value, omitted } = project(row, {
      keep: ["id", "title", "status", "value"],
    });
  ...
  it("passes non-object values through unchanged", () => {
    ...
    expect(project(["plain", "strings"], { keep }).value).toEqual([
      "plain",
      "strings",
    ]);
  });
...
describe("normalizeFieldSelector", () => {
  it("maps undefined to the compact view and rejects an empty array", () => {
    ...
    expect(() => normalizeFieldSelector([])).toThrow(
      /no fields were requested/,
    );
  });
  ...
  it("rejects a bare field name, guiding toward an array", () => {
    expect(() => normalizeFieldSelector("plan")).toThrow(
      /must be an array of field names/,
    );
    expect(() => normalizeFieldSelector("plan")).toThrow(
  ...
  it("rejects a stringified array, flagging the JSON-string mistake", () => {
    ...
    expect(() => normalizeFieldSelector(stringified)).toThrow(
      /JSON-encoded string/,
    );
    // The whole blob must NOT leak into an unknown-field list.
    expect(() => normalizeFieldSelector(stringified)).not.toThrow(
      /Unknown field/,
    );
  });
  ...
  it("takes every array entry literally", () => {
    ...
    expect(normalizeFieldSelector(["default", "plan"])).toEqual([
      "default",
      "plan",
    ]);
  });
