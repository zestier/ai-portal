...
describe("customToolGrantCaveat", () => {
  ...
  it("warns that always-prompt tools cannot be auto-approved by a grant", () => {
    ...
    expect(customToolGrantCaveat(byName("worktree_remove"))).toMatch(
      /always prompts/,
    );
    expect(customToolGrantCaveat(byName("git_commit"))).toMatch(
      /always prompts/,
    );
  });

  it("warns that never-prompt tools are already approved", () => {
    expect(customToolGrantCaveat(byName("memory_search"))).toMatch(
      /never prompts/,
    );
  });
