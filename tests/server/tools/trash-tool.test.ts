...
describe("trash tool", () => {
  ...
  it("never clobbers a pre-existing .zap/.gitignore", async () => {
    ...
    writeFileSync(join(root, "a.txt"), "x");
    await trash.handler({ path: "a.txt" });
    expect(readFileSync(join(root, zapDir(), ".gitignore"), "utf-8")).toBe(
      "# custom\n/scratch/\n",
    );
  });
  ...
  it("writes a restorable meta.json describing the original path", async () => {
    ...
    const meta = JSON.parse(
      readFileSync(join(root, TRASH_DIR, entryId, "meta.json"), "utf-8"),
    );
  ...
  it("errors when the path does not exist", async () => {
    expect(expectErr(await trash.handler({ path: "missing.txt" }))).toMatch(
      /does not exist/i,
    );
  });

  it("refuses to trash the workspace root", async () => {
    expect(expectErr(await trash.handler({ path: "." }))).toMatch(
      /workspace root/i,
    );
  });
  ...
  it("refuses to trash the trash store itself", async () => {
    ...
    expect(expectErr(await trash.handler({ path: TRASH_DIR }))).toMatch(
      /trash/i,
    );
  });
  ...
  it("refuses to trash a path nested inside the trash store", async () => {
    ...
    expect(
      expectErr(await trash.handler({ path: `${TRASH_DIR}/old/x` })),
    ).toMatch(/trash/i);
  });
  ...
  it("rejects a `..` escape", async () => {
    expect(expectErr(await trash.handler({ path: "../escape" }))).toMatch(
