...
describe("move tool", () => {
  ...
  it("creates missing destination parent directories", async () => {
    ...
    const res = await move.handler({
      source: "a.txt",
      destination: "nested/deep/b.txt",
    });
  ...
  it("errors when the source does not exist", async () => {
    expect(
      expectErr(
        await move.handler({ source: "missing.txt", destination: "b.txt" }),
      ),
  ...
  it("errors when source and destination are the same path", async () => {
    ...
    expect(
      expectErr(
        await move.handler({ source: "a.txt", destination: "./a.txt" }),
      ),
    ).toMatch(/same path/i);
  });
