...
describe("zap-dir", () => {
  ...
  it("builds config paths directly under the dir", () => {
    ...
    expect(zapSubdir("config.json", { ZAP_DIR: ".portal" })).toBe(
      ".portal/config.json",
    );
  });
  ...
  it("builds ephemeral paths under the scratch tree", () => {
    ...
    expect(scratchSubdir("notes", { ZAP_DIR: ".portal" })).toBe(
      ".portal/scratch/notes",
    );
  });
});
...
describe("ensureZapGitignore", () => {
  ...
  it("never clobbers an existing file", async () => {
    ...
    await ensureZapGitignore(root);
    expect(readFileSync(join(root, ".zap", ".gitignore"), "utf-8")).toBe(
      "# mine\n",
    );
  });
