...
describe("find", () => {
  ...
  it("respects .gitignore", async () => {
    await withWorkspace(async (workspace) => {
      ...
      const view = findText(
        await tool(workspace).handler({ pattern: "**/*.ts" }),
      );
      expect(view.split("\n")).toEqual(["kept.ts"]);
    });
  });
  ...
  it("excludes node_modules even when not gitignored", async () => {
    await withWorkspace(async (workspace) => {
      ...
      const view = findText(
        await tool(workspace).handler({ pattern: "**/*.js" }),
      );
      expect(view.split("\n")).toEqual(["own.js"]);
    });
  });
  ...
  it("scopes to the given path and rejects paths outside the workspace", async () => {
    await withWorkspace(async (workspace) => {
      ...
      const scoped = await tool(workspace).handler({
        pattern: "**/*.ts",
        path: "src",
      });
  ...
  it("caps results at the requested limit", async () => {
    await withWorkspace(async (workspace) => {
      for (let i = 0; i < 20; i++)
        await writeFile(
          join(workspace, `f${String(i).padStart(2, "0")}.ts`),
          "",
        );
