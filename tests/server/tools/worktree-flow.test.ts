...
describe("orchestrator worktree flow (acceptance)", () => {
  ...
  beforeEach(async () => {
    ...
    gitTools = new Map(
      buildGitTools(source, { userId, conversationId }).map((t) => [t.name, t]),
    );
    ({ decideByPolicy: decide } =
      await import("../../../src/lib/server/runtime/interactive-requests"));
    ({ workspaceRootsFor: rootsFor } =
  ...
  it("lets a sub-agent write in a fresh worktree without prompting, then revokes on removal", async () => {
    ...
    const created = payload(
      await tools.get("worktree_create")!.handler({ label: "api" }),
    );
    ...
    expect(writeDecision(join(leasePath, "nested", "deep", "feature.ts"))).toBe(
      "approved",
    );

    // Unrelated locations are still gated.
    expect(
      writeDecision(join(makeTmpDir("portal-wt-e2e-outside-"), "x.ts")),
    ).toBe("ask");
    ...
    const removed = payload(
      await tools.get("worktree_remove")!.handler({ leaseId }),
    );
    ...
    expect(
      git(source, ["branch", "--list", removed.branch as string]),
    ).toContain(removed.branch as string);
  ...
  it("refuses to strand uncommitted sub-agent work on removal", async () => {
    const created = payload(
      await tools.get("worktree_create")!.handler({ label: "wip" }),
    );
    writeFileSync(join(created.path as string, "unsaved.ts"), "half done\n");

    const res = await tools
      .get("worktree_remove")!
      .handler({ leaseId: created.leaseId as string });
  ...
  describe("git read tools targeting a lease", () => {
    it("inspects a lease without disturbing the conversation-local reads", async () => {
      ...
      const inLease = payload(
        await gitTools.get("git_status")!.handler({ worktree: leaseId }),
      );
      ...
      const leaseLog = payload(
        await gitTools.get("git_log")!.handler({ worktree: leaseId }),
      );
      const leaseSubjects = (
        leaseLog.commits as Array<{ subject: string }>
      ).map((c) => c.subject);
      expect(leaseSubjects).toContain("sub-agent work");
      const mainLog = payload(await gitTools.get("git_log")!.handler({}));
      expect(
        (mainLog.commits as Array<{ subject: string }>).map((c) => c.subject),
      ).not.toContain("sub-agent work");
    ...
    it("refuses a lease this conversation does not hold", async () => {
      ...
      const { buildGitTools } =
        await import("../../../src/lib/server/tools/git");
      const otherTools = new Map(
        buildGitTools(source, {
          userId,
          conversationId: convCodec.parse(other.id),
        }).map((t) => [t.name, t]),
      );
      ...
      expect(
        await gitTools.get("git_status")!.handler({ worktree: "L999999" }),
      ).toMatchObject({
        ok: false,
        error: { code: "lease_not_found" },
      });
    ...
    it("rejects the selector when the session context is unavailable", async () => {
      ...
      expect(
        await bare.get("git_status")!.handler({ worktree: "anything" }),
      ).toMatchObject({
        ok: false,
  ...
  describe("git_commit targeting a lease", () => {
    it("commits a sub-agent’s work in the lease and leaves it mergeable", async () => {
      ...
      expect(git(leasePath, ["log", "-1", "--format=%s"])).toBe(
        "feature: add x",
      );
      ...
      expect(res.followUpHint).toContain(
        `worktree_merge (leaseId: "${leaseId}")`,
      );
      ...
      const merged = payload(
        await tools.get("worktree_merge")!.handler({ leaseId }),
      );
    ...
    it("commits explicitly named paths relative to the lease", async () => {
      ...
      expect(res.ok).toBe(true);
      expect(git(leasePath, ["show", "--name-only", "--format=", "HEAD"])).toBe(
        "wanted.ts",
      );
      expect(git(leasePath, ["status", "--porcelain"])).toContain(
        "unwanted.ts",
      );
    });
    ...
    it("refuses to commit into a lease this conversation does not hold", async () => {
      ...
      writeFileSync(
        join(created.path as string, "feature.ts"),
        "export const x = 1;\n",
      );
      const convs =
        await import("../../../src/lib/server/db/repos/conversations");
      ...
      expect(git(created.path as string, ["log", "-1", "--format=%s"])).toBe(
        "initial",
      );
      expect(git(source, ["log", "-1", "--format=%s"])).toBe("initial");
    });
    ...
    it("serializes a lease commit against the repository lock", async () => {
      ...
      const { withRepositoryLock } =
        await import("../../../src/lib/server/repo-lock");
  ...
  describe('a lease left mid-merge by onConflict: "keep"', () => {
    ...
    async function conflictedLease() {
      ...
      expect(
        (
          await gitTools.get("git_commit")!.handler({
            worktree: leaseId,
            paths: "all",
            subject: "sub-agent edit",
          })
        ).ok,
      ).toBe(true);
    ...
    it("can be resolved, committed, and merged back with structured tools alone", async () => {
      ...
      expect(res.error.message).toContain(
        `git_merge_abort { worktree: "${leaseId}" }`,
      );
      ...
      expect(status.merge).toMatchObject({
        inProgress: true,
      ...
      await expect(
        gitTools
          .get("git_commit")!
          .handler({ worktree: leaseId, paths: "all", subject: "resolve" }),
      ...
      const commit = await gitTools.get("git_commit")!.handler({
        worktree: leaseId,
        paths: "all",
        subject: "merge orchestrator edit",
      });
      expect(commit.ok).toBe(true);
      expect(
        commit.ok && (commit.result as { mergeCommit: boolean }).mergeCommit,
      ).toBe(true);

      // The lease is clean again, so it is mergeable — and the merge lands
      // the resolution in the conversation's own workspace.
      const leaseStatus = payload(
        await tools.get("worktree_status")!.handler({ leaseId }),
      );
    ...
    it("can be abandoned with git_merge_abort, leaving the lease mergeable", async () => {
      ...
      const aborted = await gitTools
        .get("git_merge_abort")!
        .handler({ worktree: leaseId });
      expect(aborted.ok).toBe(true);
      // Back to the sub-agent's own commit, with nothing left in the tree.
      expect(readFileSync(join(leasePath, "README.md"), "utf8")).toBe(
        "base\nfrom the sub-agent\n",
      );
      const status = payload(
        await gitTools.get("git_status")!.handler({ worktree: leaseId }),
      ...
      expect(collect.ok || collect.error.message).not.toContain(
        "git_merge_abort",
      );
