...
function runReport(dataDir: string): Record<string, unknown> {
	const out = execFileSync(process.execPath, [scriptPath, '--data-dir', dataDir, '--json'], {
		encoding: 'utf8',
...
describe('adversary shadow report script', () => {
	...
	it('reads every column it uses, including the ones only the skip logic needs', async () => {
		...
		repo.recordVerdict(tp, 'deny', 'dangerous', { denyProbability: 0.9, latencyMs: 120 });
		repo.recordHumanDecision(tp, 'deny');

		// A scored true negative.
		const tn = insert('prompt-policy');
		repo.recordVerdict(tn, 'allow', 'fine', { denyProbability: 0.1, latencyMs: 80 });
		...
		repo.recordVerdict(auto, 'allow', 'fine', { denyProbability: 0.2, latencyMs: 100 });
		...
		const strata = report.strata as Record<string, Record<string, Record<string, unknown>>>;
