...
import {
  isPathInAnyWorkspace,
  isPathInWorkspace,
} from "../../../src/lib/server/permissions/workspace";
...
describe("isPathInAnyWorkspace", () => {
  ...
  it("rejects a path outside every supplied root", () => {
    expect(
      isPathInAnyWorkspace(join(outside, "secret.txt"), [primary, lease]),
    ).toBe(false);
  });
  ...
  it("does not treat a prefix-sharing sibling root as contained", () => {
    ...
    expect(isPathInAnyWorkspace(join(sibling, "c.ts"), [lease, sibling])).toBe(
      true,
    );
  });
  ...
  it("does not let a symlink inside one root escape to a non-root", () => {
    ...
    expect(
      isPathInAnyWorkspace(join(lease, "escape", "secret.txt"), [
        primary,
        lease,
      ]),
    ).toBe(false);
...
describe("decideByPolicy multi-root auto-approval", () => {
  ...
  beforeEach(() => {
    ...
    lease = join(base, "leases", "01LEASE");
    outside = join(base, "elsewhere");
    for (const dir of [primary, lease, outside])
      mkdirSync(dir, { recursive: true });
  });
