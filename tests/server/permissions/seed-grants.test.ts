...
describe("seed grants — installation", () => {
  it("ensureLocalUser seeds the default grant set", async () => {
    ...
    expect(all.every((g) => g.scope !== null || g.scopePattern !== null)).toBe(
      true,
    );
  ...
  it("restore replaces identifiable old hard-deny prompt seeds with current seeds", async () => {
    ...
    reSettings.addGrant({
      ...
      scope: {
        kind: "shell",
        rule: { command: [{ token: "cat" }], pipeline: "forbid" },
      },
      decision: "deny",
      denyReason:
        "Bare `cat` is denied. Use `view` for file reads. Piped `cat` is allowed.",
    });
    ...
    expect(result).toEqual({
      removed: 1,
      inserted: defaultSeedGrants().length,
    });
    expect(all.length).toBe(defaultSeedGrants().length);
    expect(
      all
  ...
  it("restore leaves user-created non-default grants alone", async () => {
    ...
    expect(
      all.some(
        (g) => g.decision === "deny" && g.denyReason === "rm stays blocked",
      ),
    ).toBe(true);
  });
});
...
describe("seed grants — runtime behaviour", () => {
  ...
  it("allows cd within the workspace; prompts cd that leaves it", () => {
    ...
    expect(shellMatchDetailed("cd /etc", "/tmp", "/tmp").feedback).toContain(
      "`cwd` argument",
    );
  ...
  it("auto-approves filesystem requests inside the SDK session workspace by default", () => {
    ...
    expect(fsMatch("write", join(session, "files", "out.txt"), session)).toBe(
      "allow",
    );
  ...
  it("denies shell git commands covered by structured Git tools", () => {
    ...
    expect(shellMatchDetailed("git --no-pager status").feedback).toContain(
      "git_status",
    );
    expect(shellMatchDetailed("git diff HEAD").feedback).toContain("git_diff");
    expect(shellMatchDetailed("git log -n 5").feedback).toContain(
      "git_log tool",
    );
    expect(shellMatchDetailed("git show HEAD").feedback).toContain(
      "git_show_commit",
    );
    expect(shellMatchDetailed("git commit -m x").feedback).toContain(
      "git_commit",
    );
  });
  ...
  it("denies shell wc and steers to the grep tool", () => {
    ...
    expect(shellMatchDetailed("wc -l README.md", "/tmp").feedback).toContain(
      "grep",
    );
  });
  ...
  it("requires prompts for mutating git subcommands instead of auto-approving them", () => {
    ...
    expect(
      shellMatch("git remote set-url origin https://example.com/repo.git"),
    ).toBe("prompt");
  });
  ...
  it("denies risky Git global options with structured-tool feedback", () => {
    ...
    expect(
      shellMatch("git --config-env core.sshCommand=GIT_SSH_COMMAND status"),
    ).toBe("deny");
  ...
  it("bare cat is allowed when it matches the filesystem read allow seed", () => {
    ...
    expect(
      shellMatch("cat /tmp/session/plan.md | grep foo", "/tmp", "/tmp/session"),
    ).toBe("allow");
  ...
  it("explains a refused shell read by the missing read grant, not a workspace boundary", () => {
    const feedback =
      shellMatchDetailed("cat /etc/passwd", "/tmp").feedback ?? "";
  ...
  it("lets a user-added read grant widen the seeded shell readers to match `view`", () => {
    ...
    const base = realpathSync(
      mkdtempSync(join(tmpdir(), "portal-seed-corpus-")),
    );
  ...
  it("does not let a write grant satisfy a seeded shell reader", () => {
    const base = realpathSync(
      mkdtempSync(join(tmpdir(), "portal-seed-writeonly-")),
    );
    const ws = join(base, "ws");
  ...
  it("keeps worktree-lease files readable by `cat` via the workspace floor seed", () => {
    ...
    const base = realpathSync(
      mkdtempSync(join(tmpdir(), "portal-seed-lease-")),
    );
    const ws = join(base, "ws");
  ...
  it("never lets the deferring seed grant on a command with no path to check", () => {
    ...
    for (const g of deferring) {
      const count =
        g.scope?.kind === "shell" ? g.scope.rule.positionalCount : undefined;
  ...
  it("denies the find primaries that write to their FILE operand", () => {
    ...
    expect(shellMatch("find . -files0-from /tmp/list", "/tmp")).not.toBe(
      "allow",
    );
    expect(shellMatch("find . -files0-from=/tmp/list", "/tmp")).not.toBe(
      "allow",
  ...
  it("denies the sort options that execute a program or name an unchecked path", () => {
    ...
    expect(
      shellMatch("sort --compress-program=/bin/sh README.md", "/tmp"),
    ).not.toBe("allow");
    // Reads a file that is not a positional.
    expect(
      shellMatch("sort --random-source=/etc/shadow README.md", "/tmp"),
    ).not.toBe("allow");
    ...
    expect(
      shellMatch("sort --temporary-directory=./sub README.md", "/tmp"),
    ).not.toBe("allow");
  });
  ...
  it("denies the `file` options that compile a magic file or read a file list", () => {
    ...
    expect(
      shellMatch("file --compile --magic-file /tmp/magic", "/tmp"),
    ).not.toBe("allow");
  ...
  it("bounds `uniq` to one operand because its second operand is an OUTPUT file", () => {
    ...
    expect(shellMatch("uniq README.md /root/.bashrc", "/tmp")).not.toBe(
      "allow",
    );
  ...
  it("denies an abbreviated form of a dangerous long option", () => {
    ...
    expect(shellMatch("sort --compress-p=/bin/sh README.md", "/tmp")).not.toBe(
      "allow",
    );
  ...
  it("keeps the seeded cwd-movers in sync with the matcher cwd set", () => {
    ...
    // quietly diverge.
    expect(new Set(CWD_MOVERS.map((m) => m.token))).toEqual(
      CWD_MOVING_BUILTINS,
    );
  });
  ...
  it("carries every audited operand bound onto EVERY seed of that tool", () => {
    ...
    for (const [token, entry] of bounded) {
      const rules = defaultSeedGrants()
        .filter(
          (g) =>
            g.scope?.kind === "shell" &&
            g.scope.rule.command?.[0]?.token === token,
        )
        .map((g) => (g.scope?.kind === "shell" ? g.scope.rule : null));
      expect(rules.length, `seeds for ${token}`).toBeGreaterThan(1);
      for (const rule of rules) {
        expect(rule?.positionalCount?.max, `bound on a ${token} seed`).toBe(
          entry.maxPositionals,
        );
      }
    }
    ...
    const deferringRule =
      deferringUniq?.scope?.kind === "shell" ? deferringUniq.scope.rule : null;
  ...
  it("leaves grep/rg on a count bound rather than a path rule", () => {
    ...
    const grepSeeds = defaultSeedGrants().filter(
      (g) =>
        g.scope?.kind === "shell" &&
        (g.scope.rule.command?.[0]?.token === "grep" ||
          g.scope.rule.command?.[0]?.token === "rg"),
    );
    expect(grepSeeds.length).toBeGreaterThan(0);
    for (const g of grepSeeds) {
      const kind =
        g.scope?.kind === "shell" ? g.scope.rule.positionals?.kind : undefined;
      expect(kind).not.toBe("readable-paths");
      expect(kind).not.toBe("writable-paths");
    }
    expect(shellMatch("cat README.md | grep ../etc/passwd", "/tmp")).toBe(
      "allow",
    );
  });
  ...
  it("auto-approves find searches confined to the session workspace", () => {
    ...
    expect(shellMatch(`find ${session} -name foo`, null, session)).toBe(
      "allow",
    );
  });
  ...
  it("steers grep/rg file searches to the structured grep tool", () => {
    ...
    expect(shellMatchDetailed("rg secret src", "/tmp").feedback).toContain(
      "`grep` tool",
    );
  });
