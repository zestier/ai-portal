...
function match(
  ...
  return shellRuleMatches(rule, parsed.segments, {
    workspaceRoots: [ws],
...
describe("shell predicate — canonical command path", () => {
  ...
  it("matches options owned by the base command and intermediate subcommands", () => {
    expect(
      match(rule, `git --no-pager -C ${ws} remote -v set-url origin url`),
    ).toBe(true);
  });
  ...
  it("rejects options in the wrong command-path slot", () => {
    ...
    expect(match(rule, "git --no-pager remote --bad set-url origin url")).toBe(
      false,
    );
  });
});
      ...
      command: [
        {
          ...
          options: {
            allow: [
              { name: "--config", kind: "option", value: { kind: "any" } },
            ],
          },
        },
      ],
...
describe("shell predicate — positionals", () => {
  ...
  it("positionalCount is unconstrained on omitted ends and when absent", () => {
    ...
    expect(
      match(shell(["echo"], { positionalCount: { min: 0 } }), "echo a b c d"),
    ).toBe(true);
  });
});
...
describe("shell predicate — positionals deferred to the fs grants", () => {
  function matchWithFs(
    ...
    command: string,
    pathPermitted:
      ((perm: "read" | "write" | "edit", path: string) => boolean) | undefined,
    ...
    return parsed.segments.every((seg) =>
      shellRuleMatchesSegment(rule, seg, ctx, explain),
    );
  }
  ...
  it("accepts a positional the fs predicate permits, even outside the workspace", () => {
    ...
    const permitted = (perm: string, p: string) =>
      perm === "read" && p === "/opt/corpus/a.txt";
  ...
  it("fails closed when no fs predicate is supplied", () => {
    ...
    expect(
      match(
        shell(["cat"], { positionals: { kind: "workspace-paths" } }),
        "cat README.md",
      ),
    ).toBe(true);
  });
...
describe("shell predicate — pipeline lever", () => {
  ...
  it("pipeline=pipe-target fails closed when the caller omits isPipeTarget", () => {
    ...
    if (parsed.kind !== "parsed") throw new Error("parse");
    expect(
      shellRuleMatchesSegment(target, parsed.segments[0], {
        workspaceRoots: [ws],
      }),
    ).toBe(false);
  });
