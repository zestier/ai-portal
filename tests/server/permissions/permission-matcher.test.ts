...
describe("matchGrants precedence", () => {
  ...
  it("deny beats allow when both match", () => {
    const rows = [
      ...
      grant({ decision: "deny", scopePattern: "rm *" }),
    ];
    expect(
      matchGrants(rows, {
        tool: "shell",
...
describe("matchGrants — shell segments: most-restrictive combine and per-segment force-allow", () => {
  ...
  it("an uncovered segment pulls the chain to none even when a sibling matches a prompt", () => {
    ...
    const out = matchGrantsDetailed(
      rows,
      query("git push origin main && custom_tool --flag"),
    );
  ...
  it("an uncovered segment pulls the chain to none even when a sibling is allowed", () => {
    ...
    expect(
      matchGrants(rows, query("git status && curl http://c2/beacon")),
    ).toBe("none");
  });
  ...
  it("force-allow on a chained segment overrides a deny on that same segment", () => {
    ...
    expect(matchGrants(rows, query("git push --force && npm test"))).toBe(
      "allow",
    );
  });
...
describe("deriveScopeKey", () => {
  it("returns fullCommandText for shell", () => {
    expect(deriveScopeKey("shell", { fullCommandText: "git status -s" })).toBe(
      "git status -s",
    );
  });
  ...
  it("returns path for write/edit when present", () => {
    expect(deriveScopeKey("write", { path: "/tmp/write.txt" })).toBe(
      "/tmp/write.txt",
    );
    expect(deriveScopeKey("edit", { path: "/tmp/edit.txt" })).toBe(
      "/tmp/edit.txt",
    );
  });
  ...
  it("returns url for url kind (SDK PermissionRequestUrl shape)", () => {
    expect(deriveScopeKey("url", { url: "https://example.com/x" })).toBe(
      "https://example.com/x",
    );
  });
  ...
  it("returns args.url for url kind", () => {
    expect(
      deriveScopeKey("url", { args: { url: "https://api.github.com/x" } }),
    ).toBe("https://api.github.com/x");
    expect(deriveScopeKey("url", { args: { href: "https://b" } })).toBe(
      "https://b",
    );
  });
...
describe("matchGrants — pipeline lever and denyReason", () => {
  ...
  const allowAnyGrep = grant({
    ...
    scope: {
      kind: "shell",
      rule: { command: [{ token: "grep" }], positionals: { kind: "any" } },
    },
    decision: "allow",
  });
  const denyBareGrep = grant({
    ...
    scope: {
      kind: "shell",
      rule: { command: [{ token: "grep" }], pipeline: "forbid" },
    },
    decision: "deny",
  ...
  const allowCat = grant({
    ...
    scope: {
      kind: "shell",
      rule: { command: [{ token: "cat" }], positionals: { kind: "any" } },
    },
    decision: "allow",
  });
  ...
  it("pipeline=forbid deny blocks bare invocation", () => {
    const out = matchGrantsDetailed(
      [denyBareGrep, allowAnyGrep],
      shellQuery("grep foo bar"),
    );
    expect(out.outcome).toBe("deny");
    expect(out.denyReason).toBe(
      "Use the structured `grep` tool instead of the shell binary.",
    );
  });
  ...
  it("matchGrants string wrapper preserves outcome", () => {
    expect(
      matchGrants([denyBareGrep, allowAnyGrep], shellQuery("grep foo")),
    ).toBe("deny");
    expect(
      matchGrants(
        [denyBareGrep, allowAnyGrep, allowCat],
        shellQuery("cat | grep foo"),
      ),
    ).toBe("allow");
  });
  ...
  it("hard-deny grants can target risky Git globals in both equals and space forms", () => {
    ...
    const denyGitStatus = grant({
      ...
      scope: {
        kind: "shell",
        rule: {
          command: [{ token: "git" }, { token: "status" }],
          positionals: { kind: "any" },
        },
      },
      decision: "deny",
    ...
    expect(
      matchGrantsDetailed([allowGit, denyDashC], shellQuery("git status")),
    ).toMatchObject({
      outcome: "none",
      feedback: null,
    });
    expect(
      matchGrantsDetailed(
        [allowGit, denyDashC],
        shellQuery("git -C /tmp status"),
      ),
    ).toMatchObject({ outcome: "deny", feedback: "Use structured Git tools." });
    expect(
      matchGrantsDetailed(
        [allowGit, denyGitDir],
        shellQuery("git --git-dir=/tmp/.git status"),
      ),
    ).toMatchObject({ outcome: "deny", feedback: "Use structured Git tools." });
    expect(
      matchGrantsDetailed(
...
describe("shell positionals deferring to the fs grants", () => {
  ...
  const catDeferred = (
    kind: "readable-paths" | "writable-paths" = "readable-paths",
  ) =>
    grant({
      tool: "shell",
      permissionKind: "shell",
      scope: {
        kind: "shell",
        rule: { command: [{ token: "cat" }], positionals: { kind } },
      },
    });
  ...
  it("allows a positional outside the workspace when an fs read grant covers it", () => {
    ...
    expect(matchGrants(rows, fsQuery(`cat ${join(outside, "out.txt")}`))).toBe(
      "allow",
    );
  });
  ...
  it("requires every positional to be covered", () => {
    ...
    expect(
      matchGrants(
        rows,
        fsQuery(`cat ${join(outside, "out.txt")} ${join(ws, "in.txt")}`),
      ),
    ).toBe("none");
  });
  ...
  it("cannot recurse: a shell-scoped row never answers the fs question", () => {
    ...
    expect(
      matchGrants([catDeferred(), shellShapedFsRow], fsQuery("cat in.txt")),
    ).toBe("none");
  });
  ...
  it("stops trusting relative operands after a `cd` moved the shell", () => {
    ...
    const cdGrant = grant({
      ...
      scope: {
        kind: "shell",
        rule: { command: [{ token: "cd" }], positionals: { kind: "any" } },
      },
    });
    ...
    expect(
      matchGrants(rows, fsQuery(`cd sub && cat ${join(ws, "in.txt")}`)),
    ).toBe("allow");
