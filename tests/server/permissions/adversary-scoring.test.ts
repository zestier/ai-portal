...
const _vocabularyMatches: HumanPermissionDecision =
  null as InteractivePermissionDecision | null;
...
describe("scoreShadowDecisions", () => {
  ...
  it("excludes memoized replays so a retry loop cannot vote twice", () => {
    ...
    expect(eventWeighted).toMatchObject({
      scored: 3,
      truePositives: 3,
      memoizedDuplicates: 0,
    });
  });
