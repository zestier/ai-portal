...
async function matchShell(
  ...
  const { parseShellCommand } =
    await import("../../../src/lib/server/permissions/shell-parser");
...
describe("shell grants that defer their positionals to the fs grants", () => {
  ...
  beforeEach(async () => {
    ...
    base = realpathSync(
      mkdtempSync(join(tmpdir(), "portal-shell-fs-deferred-ws-")),
    );
    ws = join(base, "ws");
  ...
  it("prompts for an out-of-workspace path until the user grants read on it", async () => {
    ...
    expect(await matchShell(userId, `dumpfile ${target}`, { ws })).toBe(
      "allow",
    );
    expect(
