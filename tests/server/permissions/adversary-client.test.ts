...
describe("parseVerdict", () => {
  it("accepts a bare JSON object", () => {
    expect(
      parseVerdict('{"verdict":"deny","rationale":"Destroys the filesystem."}'),
    ).toEqual({
      kind: "verdict",
      verdict: "deny",
  ...
  it("does not scan prose for verdict words", () => {
    ...
    expect(
      parseVerdict("I would deny this, but I need more context."),
    ).toBeNull();
  });
  ...
  it("records a missing or nonsensical deny probability as null, never a default", () => {
    ...
    expect(
      parseVerdict('{"verdict":"deny","rationale":"x"}')?.denyProbability,
    ).toBeNull();
    expect(
      parseVerdict('{"verdict":"deny","denyProbability":7,"rationale":"x"}')
        ?.denyProbability,
    ).toBeNull();
    expect(
      parseVerdict(
        '{"verdict":"deny","denyProbability":"high","rationale":"x"}',
      )?.denyProbability,
    ).toBeNull();
    ...
    expect(parseVerdict('{"verdict":"deny","rationale":"x"}')?.verdict).toBe(
      "deny",
    );
  });
  ...
  it("is not fooled by braces inside string values", () => {
    expect(
      parseVerdict('{"verdict":"deny","rationale":"uses ${x} and }"}')
        ?.rationale,
    ).toBe("uses ${x} and }");
  });
  ...
  it("truncates a runaway rationale", () => {
    const parsed = parseVerdict(
      JSON.stringify({ verdict: "deny", rationale: "x".repeat(5000) }),
    );
...
describe("reviewPermissionRequest", () => {
  ...
  it("reports unparseable output as an error, never as a verdict", async () => {
    ...
    if (outcome.kind === "error")
      expect(outcome.error).toContain("unparseable");
  });
