...
import {
  parseShellCommand,
  detectShellMisuse,
} from "../../../src/lib/server/permissions/shell-parser";

function ok(cmd: string) {
  const r = parseShellCommand(cmd);
  if (r.kind !== "parsed")
    throw new Error(`expected parsed, got unsafe: ${r.reason}`);
  return r.segments;
}
...
describe("parseShellCommand — happy path", () => {
  it("parses a single command into one segment", () => {
    ...
    expect(ok("git status -s")).toEqual([
      { argv: ["git", "status", "-s"], followingOp: null },
    ]);
  });
  ...
  it("honors quoted arguments", () => {
    ...
    expect(ok("echo 'a b'")).toEqual([
      { argv: ["echo", "a b"], followingOp: null },
    ]);
  });
  ...
  it("keeps flags with equals sign intact", () => {
    expect(ok("ls --color=auto")).toEqual([
      { argv: ["ls", "--color=auto"], followingOp: null },
    ]);
  });
});
...
describe("parseShellCommand — fd-dup redirection carve-out", () => {
  it("elides `2>&1` so the command parses normally", () => {
    expect(ok("pnpm check 2>&1")).toEqual([
      { argv: ["pnpm", "check"], followingOp: null },
    ]);
  ...
  it("elides `>&2` (default left fd) and `>&-` (close)", () => {
    expect(ok("echo hi >&2")).toEqual([
      { argv: ["echo", "hi"], followingOp: null },
    ]);
  ...
  it("preserves a digit that is space-separated from the redirect", () => {
    ...
    expect(ok("tail -n 2 >&1")).toEqual([
      { argv: ["tail", "-n", "2"], followingOp: null },
    ]);
  });
  ...
  it("does not elide fd-dup-looking text that lacks whitespace bounds", () => {
    ...
    expect(ok('echo "2>&1"')).toEqual([
      { argv: ["echo", "2>&1"], followingOp: null },
    ]);
    expect(ok("echo '2>&1'")).toEqual([
      { argv: ["echo", "2>&1"], followingOp: null },
    ]);
  ...
  it("elides /dev/null write redirects", () => {
    expect(ok("cmd >/dev/null")).toEqual([
      { argv: ["cmd"], followingOp: null },
    ]);
    expect(ok("cmd > /dev/null")).toEqual([
      { argv: ["cmd"], followingOp: null },
    ]);
    expect(ok("cmd 2>/dev/null")).toEqual([
      { argv: ["cmd"], followingOp: null },
    ]);
    expect(ok("cmd >>/dev/null")).toEqual([
      { argv: ["cmd"], followingOp: null },
  ...
  it("elides /dev/null input redirects", () => {
    ...
    expect(ok("cmd 0</dev/null")).toEqual([
      { argv: ["cmd"], followingOp: null },
    ]);
  });

  it("combines /dev/null and fd-dup in the common idiom", () => {
    expect(ok("cmd >/dev/null 2>&1")).toEqual([
      { argv: ["cmd"], followingOp: null },
    ]);
  ...
  it("refuses elision that would leave an empty pipeline segment", () => {
    ...
    expect(unsafe("cmd | 2>&1 | tail")).toMatch(
      /empty segment|trailing|operator/,
    );
  });
  ...
  it("refuses command substitutions / heredocs that happen to contain fd-dup", () => {
    ...
    // refusal.
    expect(unsafe("cmd $(echo hi 2>&1)")).toMatch(
      /substitution|subshell|variable/,
    );
...
describe("detectShellMisuse — cat/head/tail write redirects", () => {
  ...
  it("flags `cat >> file`", () => {
    expect(detectShellMisuse("cat >> out.txt")?.code).toBe(
      "cat-write-redirect",
    );
  });
  ...
  it("flags head and tail write redirects too", () => {
    expect(detectShellMisuse("head -n 5 in.txt > out.txt")?.code).toBe(
      "cat-write-redirect",
    );
    expect(detectShellMisuse("tail -f log >> mirror.log")?.code).toBe(
      "cat-write-redirect",
    );
  });

  it("flags `1>file` (explicit stdout fd)", () => {
    expect(detectShellMisuse("cat 1> out.txt")?.code).toBe(
      "cat-write-redirect",
    );
  });
  ...
  it("only flags the offending segment in a chain", () => {
    ...
    expect(detectShellMisuse("git log && cat > foo.txt")?.code).toBe(
      "cat-write-redirect",
    );
    // `cat > foo || true` — cat-write is in the first segment.
    expect(detectShellMisuse("cat > foo.txt || true")?.code).toBe(
      "cat-write-redirect",
    );
  });
