...
import { sanitizeSvg, looksLikeSvg, MAX_SVG_BYTES } from '../../src/lib/server/svg-sanitize';
...
describe('sanitizeSvg', () => {
	...
	it('drops foreignObject (embedded HTML) content', () => {
		const out = sanitizeSvg(
			wrap('<foreignObject><body><img src=x onerror=alert(1)></body></foreignObject><rect/>')
		)!;
	...
	it('removes javascript: and external href/src refs but keeps fragment refs', () => {
		const out = sanitizeSvg(
			wrap('<a href="javascript:alert(1)"/><a href="#ok"/><image href="http://evil/x.png"/>')
		)!;
	...
	it('strips DOCTYPE/ENTITY preludes', () => {
		const out = sanitizeSvg('<!DOCTYPE svg [<!ENTITY x "boom">]>' + wrap('<rect/>'))!;
