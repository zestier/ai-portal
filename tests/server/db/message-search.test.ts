...
function ftsCount(
  ...
  const row = db
    ...
    .get(
      typeof convId === "number" ? convId : conversationId.parse(convId),
    ) as { n: number };
  return row.n;
}
...
describe("messages.searchConversation (FTS-backed)", () => {
  ...
  it("finds messages by literal substring and orders chronologically", async () => {
    ...
    messages.append(c.id, {
      role: "user",
      content: "deploy the alpha service",
    });
  ...
  it("keeps exact-substring semantics (token must contain the phrase literally)", async () => {
    ...
    expect(
      messages.searchConversation(c.id, "foobar").map((m) => m.content),
    ).toEqual(["this is foobar text"]);
    ...
    expect(
      messages.searchConversation(c.id, "ooba").map((m) => m.content),
    ).toEqual(["this is foobar text"]);
  ...
  it("respects the limit", async () => {
    ...
    const c = convs.create(u.id, { title: "c", workdir: "/tmp", model: null });
    for (let i = 0; i < 5; i++)
      messages.append(c.id, { role: "user", content: `target ${i}` });
    expect(
      messages.searchConversation(c.id, "target", { limit: 3 }),
    ).toHaveLength(3);
  });
