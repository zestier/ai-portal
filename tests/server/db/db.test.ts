...
describe("db migrations + repos", () => {
  ...
  it("normalizes the workspace key for conversations upgraded from before migration 061", () => {
    ...
    getDb()
      .prepare("UPDATE conversations SET workspace_key = NULL WHERE id = ?")
      .run(c.id);
  ...
  it("round-trips the approval mode independently of the session mode", () => {
    ...
    expect(
      convs.updateSessionSettings(c.id, u.id, { approvalMode: "auto-deny" }),
    ).toBe(true);
    ...
    expect(
      convs.updateSessionSettings(c.id, u.id, {
        mode: "plan" as unknown as SessionMode,
      }),
    ).toBe(true);
    expect(convs.get(c.id, u.id)).toMatchObject({
      mode: "interactive",
  ...
  it("round-trips the per-conversation adversary model, including clearing it", () => {
    ...
    expect(
      convs.updateSessionSettings(c.id, u.id, { adversaryModel: "reviewer-y" }),
    ).toBe(true);
    ...
    expect(
      convs.updateSessionSettings(c.id, u.id, { adversaryModel: null }),
    ).toBe(true);
  ...
  it("seeds the adversary model from the user default without re-inheriting it later", () => {
    ...
    expect(convs.get(seeded.id, u.id)?.adversaryModel).toBe(
  ...
  it("indexes permission_grants(conversation_id) so the FK cascade is not a full scan", () => {
    ...
    const indexes = db
      .prepare("PRAGMA index_list('permission_grants')")
      .all() as {
      name: string;
    }[];
  ...
  it("cascades permission_grants when its conversation is deleted", () => {
    ...
    expect(
      (
        db
          .prepare(
            "SELECT COUNT(*) AS n FROM permission_grants WHERE conversation_id = ?",
          )
  ...
  it("migration 057 dedup makes pre-existing duplicate loop keys unique without new collisions", () => {
    ...
    try {
      ...
      const sql = readFileSync(
        resolve(
          "tests/fixtures/migrations/057_memory_open_loop_key_unique.sql",
        ),
        "utf8",
      );
      ...
      const rows = db
        .prepare("SELECT loop_key FROM memory_open_loops")
        .all() as {
        loop_key: string;
      }[];
  ...
  it("migration 066 splits approve-all / best-effort onto the approval-mode axis", () => {
    ...
    try {
      ...
      db.prepare(
        "INSERT INTO user_settings(user_id, default_mode) VALUES (?, ?)",
      ).run("u1", "best-effort");
      db.prepare(
        "INSERT INTO user_settings(user_id, default_mode) VALUES (?, ?)",
      ).run("u2", "plan");
      db.prepare(
        "INSERT INTO prompt_templates(id, conversation_mode) VALUES (?, ?)",
      ).run("t-be", "best-effort");
      db.prepare(
        "INSERT INTO prompt_templates(id, conversation_mode) VALUES (?, ?)",
      ).run("t-none", null);

      const sql = readFileSync(
        resolve("tests/fixtures/migrations/066_approval_mode.sql"),
        "utf8",
      );
      expect(() => db.exec(sql)).not.toThrow();
      ...
      const rows = Object.fromEntries(
        (
          db
            .prepare("SELECT id, mode, approval_mode FROM conversations")
            .all() as {
            id: string;
            mode: string;
      ...
      expect(rows["plain"]).toMatchObject({
        mode: "interactive",
        approval_mode: "ask",
      });
      expect(rows["approve-all"]).toMatchObject({
        mode: "plan",
        approval_mode: "auto-approve",
      });
      expect(rows["best-effort"]).toMatchObject({
        mode: "autopilot",
        approval_mode: "auto-deny",
      });
      expect(rows["both"]).toMatchObject({
        mode: "autopilot",
        approval_mode: "auto-approve",
      });
      ...
      const convColumns = (
        db.prepare("PRAGMA table_info(conversations)").all() as {
          name: string;
        }[]
      ).map((r) => r.name);
      ...
      expect(
        (
          db
            .prepare(
              "SELECT COUNT(*) AS n FROM conversations WHERE mode = 'best-effort'",
            )
            .get() as {
            n: number;
          }
      ...
      const settingsRows = Object.fromEntries(
        (
          db
            .prepare(
              "SELECT user_id, default_mode, default_approval_mode FROM user_settings",
            )
            .all() as {
            user_id: string;
            default_mode: string;
      ...
      const templateRows = Object.fromEntries(
        (
          db
            .prepare(
              "SELECT id, conversation_mode, approval_mode FROM prompt_templates",
            )
            .all() as {
            id: string;
  ...
  it("round-trips prompt grants and fails closed for malformed decisions", () => {
    ...
    expect(
      settings.matchGrantDetailed(u.id, c.id, "shell", "shell", "npm install"),
    ).toMatchObject({
      outcome: "prompt",
      denyReason: "npm scripts require review",
    ...
    expect(
      settings.matchGrantDetailed(u.id, c.id, "shell", "shell", "node test"),
    ).toMatchObject({
      outcome: "deny",
      denyReason: "node is blocked",
    });
    expect(settings.matchGrant(u.id, c.id, "shell", "shell", "ls src")).toBe(
      "allow",
    );
    expect(
      settings.matchGrant(u.id, c.id, "shell", "shell", "bogus value"),
    ).toBe("deny");

    const listed = settings
      .listGrantsForUser(u.id)
      .filter((g) => g.scope === null);
    expect(listed.find((g) => g.scopePattern === "npm *")?.decision).toBe(
      "prompt",
    );
    expect(listed.find((g) => g.scopePattern === "npm *")?.denyReason).toBe(
      "npm scripts require review",
    );
    expect(listed.find((g) => g.scopePattern === "bogus *")?.decision).toBe(
      "deny",
    );
  });
  ...
  it("fails closed for malformed structured grant scopes instead of falling back to wildcard", () => {
    ...
    expect(
      settings.matchGrant(u.id, c.id, "shell", "shell", "node --version"),
    ).toBe("none");
    expect(settings.matchGrant(u.id, c.id, "shell", "shell", "anything")).toBe(
      "none",
    );
  });
  ...
  it("persists background agent lifecycle outside tool_calls", () => {
    ...
    const toolColumns = (
      getDb().prepare(`PRAGMA table_info(tool_calls)`).all() as {
        name: string;
      }[]
    ).map((r) => r.name);
