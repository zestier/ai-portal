...
describe("portal extensions service", () => {
  ...
  it("validateExtensionValue accepts pinned specs and rejects bare/unpinned/bad input", async () => {
    ...
    expect(
      validateExtensionValue("package", "npm:@scope/pkg@1.2.3"),
    ).toBeNull();
    expect(
      validateExtensionValue("package", "git:github.com/user/repo@v1"),
    ).toBeNull();
    expect(validateExtensionValue("package", "npm:scope/pkg")).toMatch(
      /explicit version\/ref/,
    );
    expect(
      validateExtensionValue("package", "git:github.com/user/repo"),
    ).toMatch(/explicit version\/ref/);
    expect(validateExtensionValue("package", "npm:scope/pkg@")).toMatch(
      /explicit version\/ref/,
    );
    expect(validateExtensionValue("package", "")).toMatch(/cannot be empty/);
    expect(validateExtensionValue("package", "npm:".padEnd(501, "a"))).toMatch(
      /500/,
    );
    ...
    expect(validateExtensionValue("inline", "x".repeat(200_001))).toMatch(
      /200,000/,
    );
  });
  ...
  it("materializes inline files under DATA_DIR and removes stale/disabled ones", async () => {
    ...
    const file = join(
      svc.EXTENSION_DIR(loadConfig()),
      `portal-ext-${extensionId.parse(e.id)}.ts`,
    );
  ...
  it("enabledExtensionPaths resolves kinds (file vs PROJECT_ROOT, inline vs DATA_DIR, package passthrough)", async () => {
    ...
    expect(paths).toContain(
      join(
        svc.EXTENSION_DIR(loadConfig()),
        `portal-ext-${extensionId.parse(inline.id)}.ts`,
      ),
    );
  ...
  it("verify reports loaded for a valid inline, error entries for a broken factory and a missing file, and never throws", async () => {
    ...
    expect(
      ok.loaded.some((p) =>
        p.includes(`portal-ext-${extensionId.parse(valid.id)}.ts`),
      ),
    ).toBe(true);
    ...
    expect(miss.errors.some((e) => e.path.includes("does/not/exist.ts"))).toBe(
      true,
    );
