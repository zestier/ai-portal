...
function makeEvent(opts: {
  ...
  return {
    ...
    cookies: {
      get: (name: string) =>
        name === "portal_csrf" ? (opts.csrfCookie ?? undefined) : undefined,
      getAll: () => [],
      set: () => {},
      delete: () => {},
      serialize: () => "",
...
describe("hooks request handling (no auth gate)", () => {
  beforeEach(async () => {
  ...
  it("blocks cross-origin mutating /api/*", async () => {
    const handle = await loadHandle();
    ...
    const res = await handle({
      event,
      resolve: async () =>
        new Response("should not reach handler", { status: 200 }),
    });
