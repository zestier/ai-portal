...
const baseCfg: AppConfig = {
  ...
  WORKTREE_MAX_LEASES_PER_CONVERSATION: 8,
...
describe("redeploy build steps", () => {
  it("runs the full verify gate so a broken build never rolls over", () => {
    expect(BUILD_STEPS.map((step) => step.args.join(" "))).toEqual([
      "run verify",
    ]);
  });
...
describe("redeploy log scrubbing", () => {
  ...
  it("redacts short and broadly-named secrets", () => {
    const scrubbed = scrubRedeployLog(
      "OPENAI_COMPATIBLE_API_KEY=ab12 SHARED_SECRET=wxyz",
      {
  ...
  it("redacts a secret split across two stream chunks (buffered until newline)", async () => {
    ...
    try {
      ...
      expect(logs.every((ev) => !ev.text.includes(secret.slice(0, half)))).toBe(
        true,
      );
      expect(
        logs.some((ev) => ev.text.includes("[redacted:SESSION_SECRET]")),
      ).toBe(true);
    } finally {
...
describe("runStep", () => {
  it("resolves 0 and emits step then step-done on success", async () => {
    const events: RedeployEvent[] = [];
    const code = await runStep(nodeStep("ok", "process.exit(0)"), (ev) =>
      events.push(ev),
    );
  ...
  it("propagates a non-zero exit code", async () => {
    const events: RedeployEvent[] = [];
    const code = await runStep(nodeStep("boom", "process.exit(3)"), (ev) =>
      events.push(ev),
    );
  ...
  it("streams stdout/stderr output as scrubbed log events", async () => {
    ...
    expect(
      logs.some(
        (ev) => ev.stream === "stdout" && ev.text.includes("hello-out"),
      ),
    ).toBe(true);
    expect(
      logs.some(
        (ev) => ev.stream === "stderr" && ev.text.includes("hello-err"),
      ),
    ).toBe(true);
  });
  ...
  it("merges step.env over the inherited environment for the child", async () => {
    ...
    const code = await runStep(
      {
        ...
        args: [
          "-e",
          'process.stdout.write(process.env.ZAP_STEP_ENV_PROBE ?? "unset")',
        ],
        display: "envcheck",
    ...
    expect(
      logs.some(
        (ev) => ev.stream === "stdout" && ev.text.includes("injected-value"),
      ),
    ).toBe(true);
  });
});
...
describe("runRedeploy", () => {
  ...
  it("does not hang and reports failure when a step promise rejects", async () => {
    ...
    const errLog = events.find(
      (ev): ev is Extract<RedeployEvent, { type: "log" }> =>
        ev.type === "log" &&
        ev.stream === "stderr" &&
        ev.text.includes("boom-rejection"),
    );
    expect(errLog).toBeDefined();
    expect(events.at(-1)).toEqual({
      type: "done",
  ...
  it("preserves event ordering: step before its step-done, across steps", async () => {
    ...
    const firstStep = events.findIndex(
      (ev) => ev.type === "step" && ev.label === "first",
    );
    const firstDone = events.findIndex(
      (ev) => ev.type === "step-done" && ev.label === "first",
    );
    const secondStep = events.findIndex(
      (ev) => ev.type === "step" && ev.label === "second",
    );
    const secondDone = events.findIndex(
      (ev) => ev.type === "step-done" && ev.label === "second",
    );
  ...
  it("drains queued log events emitted during a step", async () => {
    const events = await drain([
      nodeStep(
        "noisy",
        'process.stdout.write("queued-output");process.exit(1)',
      ),
    ]);
