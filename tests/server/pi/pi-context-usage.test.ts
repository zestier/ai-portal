...
describe('piContextUsageToEvent', () => {
	it('maps a normal snapshot to a context.usage event', () => {
		const usage: ContextUsage = { tokens: 2000, contextWindow: 200_000, percent: 1 };
	...
	it('returns null when tokens are unknown (right after compaction)', () => {
		expect(
			piContextUsageToEvent({ tokens: null, contextWindow: 200_000, percent: null })
		).toBeNull();
	});

	it('returns null when contextWindow is not positive', () => {
		expect(piContextUsageToEvent({ tokens: 100, contextWindow: 0, percent: null })).toBeNull();
	});

	it('omits percentage when percent is null so the client computes it', () => {
		const event = piContextUsageToEvent({ tokens: 2000, contextWindow: 200_000, percent: null });
