...
describe("pi model override", () => {
  ...
  it("runs the per-conversation override model instead of the PI_MODEL default", async () => {
    const session = await open(
      makeTmpDir("pi-override-wd-"),
      "test-provider/override-model",
    );
  ...
  it("falls back to the PI_MODEL default when the bridge model is the DEFAULT_MODEL default", async () => {
    ...
    const session = await open(
      makeTmpDir("pi-default-wd-"),
      cfg.loadConfig().DEFAULT_MODEL,
    );
  ...
  it("rejects a model id the runtime does not know", async () => {
    await expect(
      open(makeTmpDir("pi-unknown-wd-"), "test-provider/nope"),
    ).rejects.toThrow("pi model not found: test-provider/nope");
  });
  ...
  it("validates the model id on PATCH /session, failing fast instead of storing a silent fallback", async () => {
    ...
    const callPatch = async (model: string) => {
      const { PATCH } =
        await import("../../../src/routes/api/conversations/[id]/session/+server");
      const req = new Request(
        `http://127.0.0.1/api/conversations/${conv.id}/session`,
        {
          method: "PATCH",
    ...
    expect(convs.get(conv.id, user.id)!.model).toBe(
      "test-provider/override-model",
    );
