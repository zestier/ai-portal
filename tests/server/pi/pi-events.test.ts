...
describe("PiEventMapper", () => {
  ...
  it("maps message_start / text_delta / message_end to portal events", () => {
    ...
    expect(
      ...
      { type: "message.delta", messageId: MESSAGE_ID, text: "Hello" },
    ]);
    expect(
      mapper.map(
        event({ type: "message_end", message: { role: "assistant" } }),
      ),
    ).toEqual([{ type: "message.end", messageId: MESSAGE_ID }]);
  ...
  it("skips the user prompt echo pi emits before the assistant reply", () => {
    const mapper = new PiEventMapper(MESSAGE_ID);
    expect(
      mapper.map(event({ type: "message_start", message: { role: "user" } })),
    ).toEqual([]);
    expect(
      mapper.map(event({ type: "message_end", message: { role: "user" } })),
    ).toEqual([]);
  ...
  it("keeps one reasoning segment per thinking block across deltas", () => {
    ...
    const segmentId = (
      first[0] as { type: "message.reasoning"; segmentId: string }
    ).segmentId;
    ...
    expect(
      (second[0] as { type: "message.reasoning"; segmentId: string }).segmentId,
    ).toBe(segmentId);
  ...
  it("surfaces stream errors once", () => {
    ...
    expect(out).toEqual([
      { type: "error", code: "pi_stream_error", message: "boom" },
    ]);
  ...
  it("maps tool executions to tool.call / tool.result", () => {
    ...
    const toolCallId = (call[0] as { type: "tool.call"; toolCallId: string })
      .toolCallId;
    ...
    expect(result).toEqual([
      { type: "tool.result", toolCallId, ok: true, summary: "file body" },
    ]);
  });
});
