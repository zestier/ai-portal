...
import { modelToJson, serializeCatalog } from '../../../src/lib/server/models/models-json';
...
describe('serializeCatalog', () => {
	...
	it('keeps a built-in provider with only a baseUrl override', () => {
		const doc = serializeCatalog([provider({ baseUrl: 'https://proxy.example.com/v1' })], []);
		expect(doc.providers.anthropic).toMatchObject({ baseUrl: 'https://proxy.example.com/v1' });
	...
	it('omits disabled models and only lists enabled ones per provider', () => {
		...
		const doc = serializeCatalog(
			[p],
			[
				model({ providerId: 'openai', id: 'gpt-4o', name: 'GPT-4o', enabled: true }),
				model({ providerId: 'openai', id: 'gpt-4o-mini', name: 'GPT-4o mini', enabled: false })
			]
		);
...
describe('modelToJson', () => {
	it('emits minimal fields for a plain text model', () => {
		expect(modelToJson(model())).toEqual({ id: 'claude-sonnet-4-5', name: 'Claude Sonnet 4.5' });
	});
	...
	it('emits cost only when non-zero, with tiers', () => {
		const m = model({
			cost: {
				...
				cacheWrite: 3.75,
				tiers: [
					{ inputTokensAbove: 200_000, input: 6, output: 30, cacheRead: 0.6, cacheWrite: 7.5 }
				]
			}
		});
	...
	it('emits sampling params when present', () => {
		...
		expect(modelToJson(m).samplingParams).toEqual({ temperature: 1.0, top_p: 0.95 });
	});
});
