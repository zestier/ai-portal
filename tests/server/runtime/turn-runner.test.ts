...
describe("turn-runner", () => {
  ...
  it("reports a running turn in runningConversationIds and publishes activity transitions", async () => {
    const { users, convs, turnRunner } = await freshImports();
    const appEvents =
      await import("../../../src/lib/server/runtime/app-events");
    const user = users.ensureLocalUser();
    const wd = makeTmpDir("portal-wd-");
    const conv = convs.create(user.id, {
      title: "c",
    ...
    expect(turnRunner.runningConversationIds()).toEqual(
      new Set([convCodec.parse(conv.id)]),
    );
  ...
  it("reserveTurn throws a typed TurnAlreadyInProgressError on a second reservation", async () => {
    ...
    try {
      expect(() => turnRunner.reserveTurn(id)).toThrow(
        turnRunner.TurnAlreadyInProgressError,
      );
      const err = (() => {
        try {
          turnRunner.reserveTurn(id);
  ...
  it("replays initial conversation.update before the terminal done", async () => {
    ...
    const doneIndices = received
      .map((e, i) => (e.type === "done" ? i : -1))
      .filter((i) => i >= 0);
    expect(doneIndices).toEqual([received.length - 1]);

    // The initial conversation.update must arrive before `done`.
    const updateIdx = received.findIndex(
      (e) => e.type === "conversation.update",
    );
  ...
  it("does not emit conversation.update when the title is already set", async () => {
    ...
    expect(
      received.find((e) => e.type === "conversation.update"),
    ).toBeUndefined();
  ...
  it("captures the full provider input when a userMessageId is provided", async () => {
    const { users, convs, turnRunner } = await freshImports();
    const turnInputs =
      await import("../../../src/lib/server/db/repos/turn-inputs");
    const messages = await import("../../../src/lib/server/db/repos/messages");
    const { PORTAL_PRELUDE } =
      await import("../../../src/lib/server/runtime/portal-prelude");
    const user = users.ensureLocalUser();
    const wd = makeTmpDir("portal-wd-");
    const conv = convs.create(user.id, {
      title: "Custom title",
  ...
  it("persists a failed stream as error and tags the terminal done with status error", async () => {
    ...
    expect(
      seen.some((e) => e.type === "error" && e.code === "stream_failed"),
    ).toBe(true);
  ...
  it("emits persisted assistant message ids for streamed assistant events", async () => {
    ...
    const assistantEvents = received.filter((event) =>
  ...
  it("getTurnById returns null when the turn id does not match", async () => {
    ...
    expect(
      turnRunner.getTurnById(convCodec.parse(conv.id), turn.id),
    ).toBeTruthy();
    expect(
      turnRunner.getTurnById(convCodec.parse(conv.id), "nonexistent"),
    ).toBeNull();
  ...
  it("dedupes repeated file edit events during incremental persistence", async () => {
    ...
    expect(assistant?.fileEdits?.[0]).toMatchObject({
      path: "src/a.ts",
      diff: edit.diff,
    });
  });
  ...
  it("surfaces tool-calling memory extraction as a persisted subagent card", async () => {
          ...
          choices: [
            {
              delta: {
                reasoning:
                  "The user chose append-only migrations; store that decision.",
              },
            },
          ],
        },
                    ...
                    function: {
                      ...
                      arguments: JSON.stringify({
                        ...
                        attributes: [
                          {
                            predicate: "decision",
                            value: "Use append-only migrations.",
                          },
                        ],
                      }),
                    },
      ...
      [
        {
          choices: [
            {
              delta: { content: "Stored the migration decision." },
              finish_reason: "stop",
            },
          ],
        },
    ...
    try {
      ...
      const child = assistant?.toolCalls?.find(
        (t) =>
          t.parentToolCallId === parent?.id &&
          t.tool === "memory_set_attributes",
      );
      ...
      expect(parent?.resultJson ?? "").toContain(
        "Stored the migration decision.",
      );
    } finally {
  ...
  it("nudges memory-mode turns that only queried recall tools to actually answer", async () => {
    ...
    vi.doMock("../../../src/lib/server/memory/extractor", async () => {
      const actual = await vi.importActual<
        typeof import("../../../src/lib/server/memory/extractor")
  ...
  it("frees the turn and marks memory skipped when Stop hits during extraction", async () => {
    ...
    try {
      ...
      expect(
        events.find((e) => e.type === "memory.status" && e.phase === "skipped"),
      ).toBeTruthy();
      ...
      expect(turnRunner.getTurn(convCodec.parse(conv.id))?.status).not.toBe(
        "running",
      );
    } finally {
      vi.doUnmock("../../../src/lib/server/memory/extractor");
  ...
  it("finalizes via the watchdog ceiling when extraction ignores abort and overruns", async () => {
    ...
    try {
      ...
      expect(
        events.find(
          (e) => e.type === "memory.status" && e.phase === "needs_review",
        ),
      ).toBeTruthy();
  ...
  it("re-runs extraction for an existing assistant message via startExtractionRetryTurn", async () => {
    ...
    const sseChunks = [
      [
        {
          choices: [
            { delta: { reasoning: "Re-extracting the migration decision." } },
          ],
        },
        {
          choices: [
    ...
    try {
      ...
      expect(
        events.find(
          (e) => e.type === "memory.status" && e.phase === "extracting",
        ),
      ).toBeTruthy();
      expect(
        events.find(
          (e) => e.type === "memory.status" && e.phase === "committed",
        ),
      ).toBeTruthy();
      expect(events.find((e) => e.type === "done")).toMatchObject({
        type: "done",
  ...
  it("undoes the prior committed patch only after a successful re-extraction", async () => {
    ...
    try {
      ...
      const memory = await import("../../../src/lib/server/db/repos/memory");
      const { commitPatch } =
        await import("../../../src/lib/server/memory/engine");
      const user = users.ensureLocalUser();
      const wd = makeTmpDir("portal-wd-");
      const conv = convs.create(user.id, {
  ...
  it("feeds the re-extractor memory as of turn start, not its own prior committed output", async () => {
    ...
    try {
      ...
      expect(
        requestBodies.some((body) => body.includes("Stale decision.")),
      ).toBe(false);
  ...
  it("on retry timeout surfaces needs_review (not cancelled) and preserves the prior patch", async () => {
    ...
    try {
      ...
      expect(
        memory.listPatches(conv.id).find((p) => p.id === prior.patch.id)
          ?.status,
      ).toBe("committed");
      expect(memory.listFacts(conv.id).map((f) => f.value)).toContain(
        "Stale decision.",
  ...
  it("persists manual rerun tool calls as separate attempts without overwriting the original", async () => {
    ...
    const originalMsg = messages.append(conv.id, {
      role: "assistant",
      content: "",
    });
  ...
  it("never persists a silent empty complete from a synthetic message.end — surfaces empty_response instead", async () => {
    ...
    acquireMock.mockResolvedValue(
      makeFakeSession([{ type: "message.end", messageId: "M0" }]),
    );
    ...
    expect(
      seen.some((e) => e.type === "error" && e.code === "empty_response"),
    ).toBe(true);
    expect(done).toMatchObject({ type: "done", status: "error" });
    expect(turnRunner.getTurn(convCodec.parse(conv.id))?.status).toBe("error");
