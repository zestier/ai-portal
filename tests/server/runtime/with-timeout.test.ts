...
import {
  withTimeout,
  TimeoutError,
} from "../../../src/lib/server/runtime/with-timeout";
...
describe("withTimeout", () => {
  ...
  it("rejects with TimeoutError when the promise hangs past the deadline", async () => {
    ...
    await expect(withTimeout(hang, 10, "hung call")).rejects.toBeInstanceOf(
      TimeoutError,
    );
  ...
  it("disables the guard when timeoutMs is 0", async () => {
    await expect(withTimeout(Promise.resolve("ok"), 0, "x")).resolves.toBe(
      "ok",
    );
  });
});
