...
describe("readFileSafe", () => {
  ...
  it("does not flag a non-image binary (or mis-typed .png) as an image", async () => {
    ...
    if (bin.ok && "binary" in bin && bin.binary)
      expect(bin.imageMimeType).toBeUndefined();
    const fake = await readFileSafe(root, "fake.png");
    if (fake.ok && "binary" in fake && fake.binary)
      expect(fake.imageMimeType).toBeUndefined();
  });
  ...
  it.skipIf(!symlinksWork)(
    "refuses to read through a tail symlink that escapes root",
    async () => {
...
describe("resolveContainedToolPath", () => {
  ...
  it("rejects an absolute path outside the root", () => {
    ...
    expect(
      resolveContainedToolPath(root, join(outside, "secret.txt")),
    ).toBeNull();
  ...
  it.skipIf(!symlinksWork)(
    "rejects an absolute path reached via an escaping symlink",
    () => {
      expect(
        resolveContainedToolPath(root, join(root, "escape", "secret.txt")),
      ).toBeNull();
    },
  );
  ...
  it.skipIf(!symlinksWork)(
    ...
    () => {
      ...
      try {
        ...
        const abs = resolveContainedToolPath(
          linkedRoot,
          join(linkedRoot, "pic.png"),
        );
        expect(abs).toBe(realpathSync(join(root, "pic.png")));
        // Out-of-root absolute paths are still rejected through the symlinked root.
        expect(
          resolveContainedToolPath(linkedRoot, join(outside, "secret.txt")),
        ).toBeNull();
      } finally {
