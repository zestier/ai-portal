...
import { formatReviewMessage, lineKey, type ReviewComment } from '../src/lib/client/review-format';
...
describe('formatReviewMessage', () => {
	...
	test('labels old-side comments and includes commit sha in heading', () => {
		const msg = formatReviewMessage([
			comment({ side: 'old', lineNo: 7, sha: 'abcdef1234', body: 'removed line note' })
		]);
	...
	test('handles general (no line) comments', () => {
		const msg = formatReviewMessage([comment({ lineNo: null, body: 'overall structure' })]);
