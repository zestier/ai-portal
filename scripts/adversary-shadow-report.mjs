...
function parseArgs(argv) {
	const out = { dataDir: process.env.DATA_DIR ?? join(repoRoot, 'data'), json: false, since: null };
...
if (args.help) {
	console.log('usage: adversary-shadow-report.mjs [--data-dir <dir>] [--since <date>] [--json]');
	process.exit(0);
}
...
if (!tableExists) {
	console.error('permission_shadow_decisions does not exist — run the app once to migrate.');
	process.exit(1);
}
...
function denyProbabilityCoverage(subset) {
	...
	return verdicts.filter((r) => typeof r.deny_probability === 'number').length / verdicts.length;
}
...
const isSkip = (r) => typeof r.error === 'string' && r.error.startsWith('skipped: ');
				...
				strata: Object.fromEntries(
					[...strata].map(([key, subset]) => [
						key,
						{
							...
							eventWeighted: scoreShadowDecisions(subset, { includeMemoized: true }),
							denyProbabilityCoverage: denyProbabilityCoverage(subset),
							byTool: Object.fromEntries(
								[...groupBy(subset, (r) => r.tool)].map(([k, v]) => [k, scoreShadowDecisions(v)])
							),
							byResolutionSource: Object.fromEntries(
								[...groupBy(subset, (r) => r.resolution_source ?? 'unknown')].map(([k, v]) => [
									k,
...
console.log('Adversary shadow-mode report (Phase 0 — the adversary had no authority)');
if (strata.size > 1) {
	console.log('');
	console.log(`  NOTE: ${strata.size} experiment strata present (model x prompt x truncation).`);
	console.log('        They are separate experiments and are NOT pooled below.');
}
...
for (const [key, subset] of strata) {
	...
	const autoApproved = subset.filter((r) => r.resolution_source === 'auto-approve').length;
	console.log('');
	console.log(`== ${key}`);
	...
	console.log('');
	console.log('  Adversary DENY vs human deny (agreement with the human, per unique question):');
	console.log(
	...
	for (const [source, sourceRows] of bySource) {
		...
		console.log(
			`    ${source.padEnd(28)} ${String(s.scored).padStart(5)}   ${pct(s.denyRecall)}${
				source === 'auto-approve' ? '   (unlabelled by construction; for later adjudication)' : ''
			}`
		);
	}
	...
	for (const [tool, toolRows] of [...groupBy(subset, (r) => r.tool)].sort(
		...
		console.log(`    ${tool.padEnd(28)} ${String(s.scored).padStart(5)}   ${pct(s.denyRecall)}`);
	}
}
...
if (humanLatencies.length > 0) {
	...
	console.log('  A low human p50 suggests rubber-stamping, which makes the human label — and');
	console.log('  therefore every precision number above — unreliable in the human\u2019s favour.');
}
console.log('');
console.log('  Reminder: these are agreement-with-the-human metrics, not correctness — the');
console.log('  human defines the labels, so human recall is 1 by construction and this cannot');
console.log('  show the adversary beating a human. `auto-approve` rows carry no label at all;');
console.log('  they are collected for later adjudication, because the requests cannot be');
