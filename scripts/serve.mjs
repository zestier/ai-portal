...
function rollbackToPrev() {
  if (!existsSync(prevEntry)) {
    log(
      "rollback requested but build.prev/ has no index.js — staying on current tree.",
    );
    return false;
  }
  try {
    ...
    log(
      "rolled back build.live/ from build.prev/ (broken tree kept at build.broken/)",
    );
    return true;
  } catch (err) {
...
function start() {
  ...
  child.on("exit", (code, signal) => {
    ...
    if (
      ...
      log(
        `child crash-looped (non-zero exit within ${CRASH_THRESHOLD_MS}ms) — rolling back.`,
      );
      rollbackToPrev();
    }
...
function shutdown(sig) {
  ...
  setTimeout(() => {
    ...
    log(
      `child still running ${SHUTDOWN_GRACE_MS}ms after ${sig} — sending SIGKILL`,
    );
    try {
      child.kill("SIGKILL");
    } catch (err) {
