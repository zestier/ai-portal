...
function seedConversation(db, conversationId, opts) {
  ...
  const toolsPerAssistant = Math.ceil(
    opts.toolCalls / Math.max(1, assistantCount),
  );
  const reasoningPerAssistant = Math.ceil(
    opts.reasoningBlocks / Math.max(1, assistantCount),
  );
  ...
  const tx = db.transaction(() => {
    for (let i = 0; i < opts.messages; i++) {
      ...
      for (
        let t = 0;
        t < toolsPerAssistant && toolsLeft > 0;
        t++, toolsLeft--
      ) {
        ...
        const result = jsonOfSize(
          rand,
          { ok: true },
          "result",
          drawResultBytes(rand),
        );
      ...
      for (
        let r = 0;
        r < reasoningPerAssistant && reasoningLeft > 0;
        r++, reasoningLeft--
      ) {
...
async function measureClient(browser, baseUrl, conversationId) {
  ...
  await page.waitForFunction(() => window.__tti !== null, undefined, {
    timeout: 60_000,
  });
  const ttiMs = await page.evaluate(() => window.__tti);
  await page
    .waitForFunction(() => window.__cards !== null, undefined, {
      timeout: 60_000,
    })
    .catch(() => {});
  ...
  await page.waitForFunction(() => window.__topCards !== null, undefined, {
    timeout: 60_000,
  });
...
async function main() {
  ...
  try {
    ...
    const transcriptPayloadBytes = await measureTranscriptPayloadBytes(
      baseUrl,
      conversationId,
    );
    ...
    if (result.transcriptPayloadBytes > 40960) {
      failures.push(
        `transcript payload ${result.transcriptPayloadBytes} B > 40960 B (~40 KB)`,
      );
    }
    ...
    if (opts.json) {
      ...
      console.log(
        `  full page HTML     : ${fmtBytes(ssrBytes)} (${ssrBytes} bytes)`,
      );
      console.log(
        `  TTI (median)       : ${Math.round(result.ttiMsMedian)} ms`,
      );
      console.log(
      ...
      console.log(
        `  scroll jank (4x)   : ${
          jank?.fps === null || jank?.fps === undefined
            ? "n/a (no Chromium)"
            : `${jank.fps} fps`
        }`,
      );
      console.log(
        `  runs               : ${runs
          .map(
            (r) =>
              `${Math.round(r.ttiMs)}ms/${r.cardsAfterLoad}/${r.cardsAtTop}cards`,
          )
          .join(", ")}`,
      );
    ...
    if (opts.serve) {
      ...
      keepServing = true;
      console.log(
        `[bench] serving at ${baseUrl}/conversations/${conversationId}`,
      );
