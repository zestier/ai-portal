...
test("settings page loads with no client-side errors", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Saved permission grants" }),
  ).toBeVisible();
  await expect(
    page.getByRole("tabpanel", { name: /Permissions/ }),
  ).toBeVisible();
...
test("creating a shell+workspace-paths grant adds a row to the list", async ({
  page,
}) => {
  await page.goto("/settings");
  await page.getByRole("tab", { name: /Permissions/ }).click();
...
test("a shell grant can defer its positionals to the fs read grants", async ({
  page,
}) => {
  ...
  const row = page
    ...
    .filter({ has: page.locator(`code.pattern:has-text("command=${argv0}")`) });
  await expect(row).toBeVisible();
  await expect(row.locator("code.pattern")).toContainText(
    "positionals=readable-paths",
  );
...
test("settings tabs isolate activity from general settings", async ({
  page,
}) => {
  await page.goto("/settings");
  ...
  await expect(
    page.getByRole("heading", { name: "Recent permission decisions" }),
  ).toBeHidden();
...
test("settings tab selection survives reload and deep links", async ({
  page,
}) => {
  await page.goto("/settings?tab=permissions");
...
test("the Extensions tab renders, deep-links, and lists the admin-managed sources", async ({
  ...
  await expect(
    page.getByRole("tab", { name: "Extensions", exact: true }),
  ).toHaveAttribute("aria-selected", "true");
  await expect(
...
test("the default approval mode is saved and seeds newly created conversations", async ({
  ...
  try {
    ...
    await page.reload();
    await expect(
      page.locator('select[name="defaultApprovalMode"]'),
    ).toHaveValue("auto-deny");

    // A conversation created without an explicit approvalMode inherits it.
    const inherited = await request
      .post("/api/conversations", {
        data: { title: uniqueTitle("Inherits approvals") },
      })
      .then((r) => r.json());
    ...
    const explicit = await request
      .post("/api/conversations", {
        data: {
          title: uniqueTitle("Explicit approvals"),
    ...
    await page.goto("/settings");
    await page
      .locator('select[name="defaultApprovalMode"]')
      .selectOption("ask");
    await page.getByRole("button", { name: "Save", exact: true }).click();
    await expect(page.getByText("Saved.")).toBeVisible();
  }
