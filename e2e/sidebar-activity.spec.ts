...
test("a background conversation flags an unseen response, and opening it clears the flag", async ({
  ...
  const viewed = await createConversation(
    request,
    uniqueTitle("E2E activity viewed"),
  );
  const background = await createConversation(
    request,
    uniqueTitle("E2E activity background"),
  );
  ...
  const backgroundRow = page.locator(
    `nav a[href="/conversations/${background}"]`,
  );
...
test("a reply that arrives while the user is watching is never flagged unseen", async ({
  ...
  const id = await createConversation(
    request,
    uniqueTitle("E2E activity watched"),
  );
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: reply while I watch/,
  );
