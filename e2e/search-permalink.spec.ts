...
test('in-conversation search finds a message and jumps to it', async ({ page, request }) => {
	page.on('pageerror', (e) => console.log('PAGE ERROR:', e.message));
	page.on('console', (m) => {
		if (m.type() === 'error' || m.type() === 'warn') console.log('CONSOLE:', m.text());
	});
	...
	const searchResponse = page.waitForResponse(
		(r) => r.url().includes('/api/conversations/') && r.url().includes('/search?q=')
	);
	...
	await expect(page.getByText(`search for ${needle} inside`).first()).toBeVisible();
});

test('?message=<id> permalink scrolls to and highlights the target', async ({ page, request }) => {
	...
	const userMessage = (conv.transcript.tail as Array<{ id: number; content: string }>).find(
		(m) => m.content === 'permalink target message'
	);
	expect(userMessage).toBeTruthy();
	await page.goto(`/conversations/${id}?message=${userMessage!.id}`);

	await expect(page.getByText('permalink target message').first()).toBeVisible();
