...
test("an empty first reply surfaces a visible error, not a dead empty bubble", async ({
  ...
  await expect(
    page.getByText(/Error: The model returned an empty response/),
  ).toBeVisible();
  ...
  await expect(
    bubble.getByRole("button", { name: "Regenerate this response" }),
  ).toBeVisible();
  ...
  await expect(
    reloaded.getByRole("button", { name: "Regenerate this response" }),
  ).toBeVisible();
  ...
  await waitForAssistantMessage(
    request,
    id,
    new RegExp(`Stubbed reply to: ${prompt}`),
  );
  await expect(page.getByText(`Stubbed reply to: ${prompt}`)).toBeVisible();
  await expect(
    page.locator('article.msg[data-role="assistant"] .status'),
  ).toHaveCount(0);
  await expect(
    page.getByText(/Error: The model returned an empty response/),
  ).toHaveCount(0);
...
test("retry regenerates the reply in place without ghost duplicates", async ({
  page,
  request,
}) => {
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: retry me @trigger-slow-start/,
  );
  await expect(page.locator('article.msg[data-role="assistant"]')).toHaveCount(
    1,
  );
...
test("inline edit re-runs in place from the edited prompt", async ({
  page,
  request,
}) => {
...
test("a reload mid-stream reattaches and the reply still lands", async ({
  page,
  request,
}) => {
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: reload me @trigger-slow-start/,
  );
  await expect(
    page.getByText("reload me @trigger-slow-start", { exact: true }).first(),
  ).toBeVisible();
  await expect(
    page.getByText("Stubbed reply to: reload me @trigger-slow-start"),
  ).toBeVisible();
  await expect(page.locator('article.msg[data-role="user"]')).toHaveCount(1);
  await expect(page.locator('article.msg[data-role="assistant"]')).toHaveCount(
    1,
...
test("a second consecutive turn renders after the first completes", async ({
  page,
  request,
}) => {
