...
test("streamed assistant reply (stubbed) appears and persists across reloads", async ({
  ...
  await expect(
    page.getByText("Stubbed reply to: hello world").first(),
  ).toBeVisible();
...
test("context meter appears in the chat header after a turn", async ({
  page,
  request,
}) => {
  const id = await createConversation(
    request,
    uniqueTitle("E2E context meter"),
  );
  await page.goto(`/conversations/${id}`);
  ...
  await expect(
    page.getByRole("img", {
      name: /Context window usage: .* of 200000 tokens/,
    }),
  ).toBeVisible();
});

test("a worktree fork failure is visible on the assistant message", async ({
  page,
  request,
}) => {
  const id = await createConversation(
    request,
    uniqueTitle("E2E worktree fork failure"),
  );
  await page.goto(`/conversations/${id}`);
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: fork this reply/,
  );
  await page.reload();

  await page.route(
    `**/api/conversations/${id}/messages/*/fork`,
    async (route) => {
      await route.fulfill({
        status: 409,
  ...
  await expect(assistantBubble.getByRole("alert")).toContainText(
    "snapshot unavailable",
  );
});

test("setup indicator renders inside the assistant turn bubble", async ({
  page,
  request,
}) => {
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: @trigger-slow-start/,
  );
...
test("a failed turn start does not corrupt the previous assistant reply", async ({
  ...
  await expect(page.getByText("second message", { exact: true })).toHaveCount(
    0,
  );
  await expect(assistantBubble).toBeVisible();
...
test("an armed follow-up auto-sends after the active turn finishes", async ({
  page,
  request,
}) => {
  ...
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: first @trigger-slow-start/,
  );
  // ...and the armed follow-up is then auto-sent as a brand new turn.
  await waitForAssistantMessage(
    request,
    id,
    /Stubbed reply to: second follow-up message/,
  );
  await expect(
    page.getByText("second follow-up message", { exact: true }).first(),
  ).toBeVisible();
...
test("the approval-mode dropdown gates auto-approve behind a conversation-scoped confirmation", async ({
  ...
  await expect(dialog).toBeVisible();
  await dialog
    .getByRole("button", { name: "Enable for this conversation" })
    .click();
  await expect(dialog).toBeHidden();
