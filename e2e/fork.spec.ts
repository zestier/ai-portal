...
test("fork by editing a user message produces a new conversation with the edited prompt", async ({
  ...
  await waitForAssistantMessage(
    request,
    sourceId,
    "Stubbed reply to: context seed",
  );
  await expect(
    page.getByText("Stubbed reply to: context seed").first(),
  ).toBeVisible();
  await composer.click();
  ...
  await waitForAssistantMessage(
    request,
    sourceId,
    "Stubbed reply to: original prompt",
  );
  await expect(
    page.getByText("Stubbed reply to: original prompt").first(),
  ).toBeVisible();
  ...
  ).find((m) => m.role === "user" && m.content === "original prompt");
  ...
  const contents = (
    newMsgs.messages as Array<{ role: string; content: string }>
  ...
  const srcContents = (
    messagesOf(srcMsgs) as Array<{ role: string; content: string }>
  ).map((m) => m.content);
...
test("edit-fork while the source turn is running defers and prefills the new composer", async ({
  ...
  const sourceId = await createConversation(
    request,
    uniqueTitle("Busy Source"),
  );
  ...
  const firstUser = (
    messagesOf(msgs) as Array<{ id: string; role: string; content: string }>
  ).find((m) => m.role === "user" && m.content === "first");
...
test("deferred edit-fork of a non-first message seeds the composer, then clears after Send", async ({
  ...
  await expect(
    page.getByText("Stubbed reply to: second").first(),
  ).toBeVisible();
  ...
  await waitForAssistantMessage(
    request,
    newId,
    "Stubbed reply to: second edited while busy",
  );
  await page.reload();
...
test("inline edit replaces a user message, truncates later messages, and reruns in place", async ({
  ...
  await waitForAssistantMessage(
    request,
    sourceId,
    "Stubbed reply to: first prompt",
  );
  await expect(
    page.getByText("Stubbed reply to: first prompt").first(),
  ).toBeVisible();
  ...
  await waitForAssistantMessage(
    request,
    sourceId,
    "Stubbed reply to: second prompt",
  );
  await expect(
    page.getByText("Stubbed reply to: second prompt").first(),
  ).toBeVisible();
  ...
  await waitForAssistantMessage(
    request,
    sourceId,
    "Stubbed reply to: first prompt edited inline",
  );

  const after = await getConversation(request, sourceId);
  const list = messagesOf(after) as Array<{
    id: string;
    role: string;
    content: string;
  }>;
...
test("regenerate an assistant message re-runs in place from the unchanged user prompt", async ({
  ...
  const before = await getConversation(request, sourceId);
  const beforeList = messagesOf(before) as Array<{
    id: string;
    role: string;
    content: string;
  }>;
  const firstAssistant = beforeList.find(
    (m) =>
      m.role === "assistant" && m.content === "Stubbed reply to: first prompt",
  );
  const firstUser = beforeList.find(
