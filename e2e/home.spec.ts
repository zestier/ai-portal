...
test("home page renders and creates a new conversation", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "Zestier's AI Portal" }),
  ).toBeVisible();
...
test("worktree creation surfaces the server error message", async ({
  page,
}) => {
...
test("bulk deletion confirms and retries worktrees holding work", async ({
  page,
}) => {
  await page.goto("/");
...
test("bulk deletion retries worktrees blocked for unmerged commits", async ({
  page,
}) => {
  await page.goto("/");
  await page.getByRole("button", { name: "New shared chat" }).first().click();
...
test("a default chat template drives the New chat buttons", async ({
  page,
  request,
}) => {
  ...
  const convBody = await (
    await request.get(`/api/conversations/${worktreeConvId}`)
  ).json();
