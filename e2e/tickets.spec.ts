...
test("sidebar Open link navigates to the ticket detail page", async ({
  page,
  request,
}) => {
  ...
  await page
    .getByRole("link", { name: /Open ticket page: Repro ticket/ })
    .click();
  await page.waitForURL(new RegExp(`/tickets/${id}$`));
  await expect(
    page.getByRole("heading", { name: "Repro ticket" }),
  ).toBeVisible();
});
...
test("ticket detail page renders directly", async ({ page, request }) => {
  const created = await request
    .post("/api/tickets", {
      data: { title: "Direct nav ticket", body: "hello" },
    })
    .then((r) => r.json());
  const id: string = created.ticket.id;
  ...
  await expect(
    page.getByRole("heading", { name: "Direct nav ticket" }),
  ).toBeVisible();
...
test("detail toolbar transitions status in place and confirms archive", async ({
  ...
  await expect(
    toolbar.getByRole("button", { name: "Mark done" }),
  ).toBeVisible();
  ...
  await expect(toolbar.getByRole("button", { name: "Mark done" })).toHaveCount(
    0,
  );
...
test("detail toolbar permanently deletes a ticket after confirmation", async ({
  ...
  await expect(dialog).toBeHidden();
  await expect(
    page.getByRole("heading", { name: "Deletable ticket" }),
  ).toBeVisible();
...
test("tickets index browses, filters by status, and paginates", async ({
  page,
  request,
}) => {
  ...
  const done = await request
    ...
    .then((r) => r.json());
  await request.patch(`/api/tickets/${done.ticket.id}`, {
    data: { status: "done" },
  });
  ...
  await expect(page.getByRole("link", { name: `${run} finished` })).toHaveCount(
    0,
  );
  ...
  await expect(
    page.getByRole("link", { name: `${run} finished` }),
  ).toBeVisible();
  await expect(
    page.getByRole("link", { name: `${run} open 0`, exact: true }),
  ).toHaveCount(0);
...
test("detail page shows a priority pill and edits priority in place", async ({
  page,
  request,
}) => {
  const created = await request
    .post("/api/tickets", {
      data: { title: "Priority detail ticket", priority: "P1" },
    })
    .then((r) => r.json());
  const id: string = created.ticket.id;
  ...
  await expect(
    page.locator(".header-pills").getByText("P1", { exact: true }),
  ).toBeVisible();

  // Editing the select PATCHes and refreshes loader data in place.
  const patched = page.waitForResponse(
    (r) =>
      r.url().includes(`/api/tickets/${id}`) &&
  ...
  await expect(
    page.locator(".header-pills").getByText("P0", { exact: true }),
  ).toBeVisible();
...
test("tickets index filters and sorts by priority", async ({
  page,
  request,
}) => {
  ...
  for (let i = 0; i < 20; i++) {
    await request.post("/api/tickets", {
      data: { title: `${run} fill ${i}`, priority: "P2" },
    });
  }
  await request.post("/api/tickets", {
    data: { title: `${run} high`, priority: "P0" },
  });
  await request.post("/api/tickets", {
    data: { title: `${run} low`, priority: "P3" },
  });
  ...
  await expect(
    page.getByRole("link", { name: `${run} fill 0`, exact: true }),
  ).toHaveCount(0);
  ...
  await expect(page.locator("li.ticket-row .ticket-title").first()).toHaveText(
    `${run} high`,
  );
  await expect(low).toHaveCount(0);
...
test("sidebar links to the full tickets page", async ({ page, request }) => {
  await request.post("/api/tickets", {
    data: { title: "Sidebar link ticket" },
  });
...
test("sidebar drops a ticket marked done live, without navigation", async ({
  page,
  request,
}) => {
  ...
  const patched = await request.patch(`/api/tickets/${id}`, {
    data: { status: "done" },
  });
