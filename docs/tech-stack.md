...
| `ADVERSARY_SHADOW_MODEL`  | —                        | **Experimental, off by default.** Server-wide default reviewer model for the Phase 0 adversarial approval shadow: a second model records what it *would* have decided about each permission request, next to what the human clicked, with no authority over the outcome. Setting a model is what enables it — there is no separate on/off flag. A user default (Settings → General) seeds new conversations and a per-conversation value wins; clearing that value falls back here, not to the user default. Skipped when it matches the conversation's own model on the same backend. Read the results with `pnpm run report:adversary-shadow`. |
| `ADVERSARY_SHADOW_BACKEND` | —                       | Backend that serves the reviewer, resolved independently of the model: conversation → user default → this → **the conversation's own backend**. That last fallback is the default and is usually right — reviewer independence is a property of the model's weights, not of which endpoint serves them, and the chat backend already receives every tool call and its arguments, so reviewing them there adds no new egress destination. Set it only to route the reviewer elsewhere (e.g. `openai-compatible` for a local model). Backends must support out-of-band completions (`capabilities.sideCompletion`). |
