...
model sees one tool per job. `grep`, `glob`, and `write` are the exceptions:
their SDK counterparts (`Grep`, `Glob`, `Write`) are rerouted to the portal
implementations via `toolAliases`, so the portal tools are exposed rather than
omitted.
