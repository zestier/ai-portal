...
export const GET: RequestHandler = ({ locals }) => {
  ...
  const customTemplates = promptTemplates
    .list(userId, { type: "chat" })
    .map((template): PromptTemplateListItem => ({
      ...template,
      source: "custom",
    }));
  return json({
...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  const template = promptTemplates.create(userId, {
    ...
    ...(body.launchBehavior !== undefined
      ? { launchBehavior: body.launchBehavior }
      : {}),
  ...
  return json(
    { ok: true, template: { ...template, source: "custom" } },
    { status: 201 },
  );
};
