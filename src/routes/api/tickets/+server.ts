...
export const GET: RequestHandler = ({ locals, url }) => {
  ...
  const status = Status.catch("open").parse(
  ...
  const sort = Sort.catch("recency").parse(
    url.searchParams.get("sort") ?? "recency",
  );
...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  if (body.sourceConversationId) {
    ...
    if (sourceConvId === null)
      throw error(404, "source conversation not found");
    ...
    throw error(
      400,
      "sourceConversationId is required when sourceMessageId is set",
    );
  }
  ...
  const ticket = tickets.create(userId, {
    ...
    sourceMessageId: body.sourceMessageId
      ? msgCodec.parse(body.sourceMessageId)
      : null,
