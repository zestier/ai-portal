...
const ProviderSchema = z
  .object({
    ...
    id: z
      ...
      .regex(
        /^[a-z0-9][a-z0-9_-]*$/,
        "provider id: lowercase letters, digits, - and _ only",
      ),
...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  if (body.action === "model") {
    ...
    const model = await saveModel(v.providerId, {
      ...
      ...(v.thinkingLevelMap !== undefined
        ? { thinkingLevelMap: v.thinkingLevelMap }
        : {}),
      ...(v.compat !== undefined ? { compat: v.compat } : {}),
      ...(v.samplingParams !== undefined
        ? { samplingParams: v.samplingParams }
        : {}),
  ...
  if (body.action === "delete-model") {
    const parsed = z
      ...
      .safeParse(body);
    if (!parsed.success)
      throw error(400, "Expected { action, providerId, id }.");
    if (!providersRepo.get(parsed.data.providerId))
