...
export const POST: RequestHandler = async ({
  ...
  const { conversation, workdir } = authorizeConversationWorkdir(
    params.id,
    locals.userId,
  );
  ...
  try {
    ...
    async function* withGuardRelease() {
      ...
      try {
        ...
        const endDetail: Record<string, unknown> = {
          actionId,
          permission,
          phase: "end",
        };
        if (!terminal) {
          ...
          if (terminal.failedStep !== undefined)
            endDetail.failedStep = terminal.failedStep;
