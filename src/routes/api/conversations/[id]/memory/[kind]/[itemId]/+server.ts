...
function parseMemoryItemId(
  ...
  const codec =
    kind === "entities" || kind === "entity" ? memoryEntityId : memoryFactId;
...
export const PATCH: RequestHandler = async ({ params, locals, request }) => {
  ...
  return json({
    item: updated,
    memory: memory.listSnapshot(convCodec.parse(conv.id), {
      userId: conv.userId,
    }),
  });
};
...
function updateMemoryItem(
  ...
  if (kind === "globalMemories") {
    ...
    if (result.status === "conflict")
      throw error(409, "Global memory key already exists.");
...
function parsePatchBody<T extends z.ZodTypeAny>(
  schema: T,
  body: unknown,
): z.infer<T> {
  ...
  if (!parsed.success) {
    ...
    const message = path
      ? `${path}: ${first?.message}`
      : (first?.message ?? "Invalid memory item");
