...
export const POST: RequestHandler = async ({
  ...
  const tplId = body.promptTemplateId
    ? promptTemplateId.tryParse(body.promptTemplateId)
    : null;
  ...
  if (requested) {
    ...
    audit({
      ...
      detail: {
        ...
        source: body.workdir ? "explicit" : "user_default",
      },
    });
  } else {
    workdir = projectRoot();
  ...
  if (workspace?.kind !== "worktree") {
    return json(
      { ok: true, conversation: createRow(workdir) },
      { status: 201 },
    );
  }
