<script lang="ts">
  ...
  let deployStatus = $state<
    "idle" | "running" | "ok" | "failed" | "restarting"
  >("idle");
    ...
    try {
      for await (const ev of streamSse<RedeployEvent>("/api/admin/redeploy", {
        ...
        switch (ev.type) {
          ...
          case "step-done":
            if (ev.code !== 0)
              appendLog(`[${ev.label}] exited with code ${ev.code}\n`);
            break;
          case "done":
            if (ev.ok) {
...
>
  ...
  <p class="muted small">
    Runs <code>pnpm run verify</code> (lint, type-check, unit tests, build, and
    e2e), then exits so the <code>pnpm serve</code> supervisor relaunches on the
