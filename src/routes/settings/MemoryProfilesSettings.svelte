<script lang="ts">
  ...
  const openProfiles = $derived(
    profiles.filter((profile) => profile.status === "open"),
  );
  const archivedProfiles = $derived(
...
>
  <PanelHeader title="Memory profiles">
    {#snippet meta()}Author custom profile schemas and instructions. These are
      saved for inspection and future activation; built-in modes remain the
      runtime-safe choices today.{/snippet}
  </PanelHeader>
  ...
  {#if form?.formId?.includes("MemoryProfile")}
    <Alert kind={form.ok ? "success" : "error"}>
      {form.ok
        ...
        : (form.error ?? "Memory profile update failed.")}
    </Alert>
  {/if}
    ...
    <form method="POST" action="?/createMemoryProfile" class="settings-form">
      <label>
        ...
        <input
          ...
          maxlength="120"
          required
          placeholder="Campaign lore keeper"
        />
      </label>
      <label>
        Description
        <input
          name="description"
          maxlength="500"
          placeholder="Tracks factions, lore, and canon"
        />
      </label>
      <label>
        Instructions
            ...
            >
              ...
              <div class="actions">
                ...
                <button
                  class="btn danger"
                  type="submit"
                  form="archive-memory-profile-{profile.id}"
                >
                  Archive
                </button>
              </div>
