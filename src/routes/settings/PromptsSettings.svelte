<script lang="ts">
  ...
  const chatTemplates = $derived(
    promptTemplates.filter((t) => t.type === "chat"),
  );
  const ticketActions = $derived(
    promptTemplates.filter((t) => t.type === "ticket-action"),
  );

  const openChatTemplates = $derived(
    chatTemplates.filter((t) => t.status === "open"),
  );
  const openTicketActions = $derived(
    ticketActions.filter((t) => t.status === "open"),
  );
  const archivedTemplates = $derived(
    promptTemplates.filter((t) => t.status === "archived"),
  );

  const ticketPlaceholders = placeholdersForType("ticket-action");
  const ticketPlaceholderHint = ticketPlaceholders
    .map((name) => `{{${name}}}`)
    .join(", ");
  ...
  async function refineTemplate(template: PromptTemplate) {
    ...
    try {
      ...
      await invalidateAll();
      await goto(resolve(result.href as `/${string}`));
    } catch (err) {
      if (
...
{#snippet launchFields(
    ...
    <label>
      ...
      <select name="workspaceMode">
        {#each workspaceModeOptions as opt (opt.value)}
          <option
            value={opt.value}
            selected={(current.workspaceMode ?? "") === opt.value}
          >
            {opt.label}
          </option>
        {/each}
    ...
    <label>
      ...
      <select name="conversationMode">
        {#each conversationModeOptions as opt (opt.value)}
          <option
            value={opt.value}
            selected={(current.conversationMode ?? "") === opt.value}
          >
            {opt.label}
          </option>
        {/each}
    ...
    <label>
      ...
      <select name="approvalMode">
        {#each approvalModeOptions as opt (opt.value)}
          <option
            value={opt.value}
            selected={(current.approvalMode ?? "") === opt.value}
          >
            {opt.label}
          </option>
        {/each}
  ...
  <p class="muted small">
    "Review before sending" opens a dialog to edit the prompt and these settings
    — including the Git workspace — before the chat is created.
  </p>
{/snippet}
...
{#snippet toolGroupFieldset(disabled: string[])}
  <fieldset class="tool-groups-fieldset">
    ...
    <p class="muted small">
      Unchecked groups are disabled up front in chats started from this template
      (a seed — the chat can re-enable them). Checked groups stay available.
      Native CLI tools (bash, view, edit…) are always available and unaffected.
      Applies to chat templates and ticket actions alike.
    </p>
...
>
  <PanelHeader title="Prompts">
    {#snippet meta()}Save reusable prompt templates for recurring workflows.
      Built-in templates are always available when starting a chat.{/snippet}
  </PanelHeader>
  ...
  {#if form?.formId?.includes("PromptTemplate") || form?.formId === "restorePromptTicketActions"}
    <Alert kind={form.ok ? "success" : "error"}>
      {form.ok
        ...
        : (form.error ?? "Prompt template update failed.")}
    </Alert>
  {/if}
    ...
    <form method="POST" action="?/createPromptTemplate" class="settings-form">
      ...
      <label>
        ...
        <input
          ...
          maxlength="120"
          required
          placeholder="Debug production error"
        />
      </label>
      <label>
        Description
  ...
  <section aria-labelledby="default-template-heading" class="card">
    ...
    <p class="muted small">
      Choose a template the New chat buttons launch by default (a blank chat
      when None). The template's launch behavior applies — a draft pre-fills the
      composer, a send posts the prompt immediately, and a review opens the
      launch dialog first.
    </p>
    <form
      method="POST"
      action="?/saveDefaultPromptTemplate"
      class="default-template-form"
    >
      <label>
        Template
        <select name="defaultPromptTemplateId">
          ...
          {#each builtInTemplates as template (template.id)}
            <option
              value={template.id}
              selected={settings.defaultPromptTemplateId === template.id}
            >
        ...
        {#each openChatTemplates as template (template.id)}
          <details class="custom-template">
            <summary>
              ...
              {#if template.appendSystemPrompt}<Pill tone="accent"
                  >Sys append</Pill
                >{/if}
            </summary>
            <form
            ...
            >
              ...
              <div class="actions">
                ...
                <button
                  class="btn danger"
                  type="submit"
                  form="archive-template-{template.id}"
                >
                  Archive
                </button>
              </div>
  ...
  <section aria-labelledby="ticket-actions-heading" class="card">
    ...
    <p class="muted small">
      Each ticket action renders as a button on every workspace ticket. The
      title is the button label. Use placeholders <code
        >{ticketPlaceholderHint}</code
      > in the prompt to inject ticket details at launch.
    </p>

    <form
      method="POST"
      action="?/restorePromptTicketActions"
      class="restore-form"
    >
      <button class="btn secondary" type="submit"
        >Restore default actions</button
      >
    ...
    {#if openTicketActions.length === 0}
      <p class="muted empty">
        No ticket actions. Tickets show only the Remove button until you add or
        restore actions.
      </p>
    {:else}
      <div class="custom-list">
        {#each openTicketActions as action (action.id)}
          <details class="custom-template">
            <summary>
              ...
              {#if action.appendSystemPrompt}<Pill tone="accent"
                  >Sys append</Pill
                >{/if}
            </summary>
            <form
            ...
            >
              ...
              <p class="muted small">
                Placeholders: <code>{ticketPlaceholderHint}</code>
              </p>
              ...
              <div class="actions">
                ...
                <button
                  class="btn danger"
                  type="submit"
                  form="archive-template-{action.id}"
                >
                  Remove
                </button>
              </div>
