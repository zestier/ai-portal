<script lang="ts">
  ...
  const customToolError = $derived(
    isCustomTool ? customToolNameError(newGrantToolName) : null,
  );
  const submittedScopeJson = $derived(
    isCustomTool ? '{"kind":"any"}' : scopeJson,
  );
  const canSubmitGrant = $derived(
    isCustomTool ? customToolError === null : scopeJson !== "",
  );
  const buildError = $derived(
    userTouched ? (isCustomTool ? customToolError : scopeError) : null,
  );
  const portalToolsByName = $derived(
    new Map(portalTools.map((t) => [t.name, t])),
  );
  const selectedPortalTool = $derived(
    portalToolsByName.get(newGrantToolName.trim()) ?? null,
  );
  ...
  function canEditGrant(g: PermissionGrant): boolean {
    ...
    if (g.scope.kind === "fs" && g.scope.perms && g.scope.perms.length > 1)
      return false;
    return true;
  }
  ...
  function isExpiringSoon(g: PermissionGrant): boolean {
    return (
      ...
      g.expiresAt <= Date.now() + 7 * 86_400_000
    );
  }
  ...
  function matchesFilters(g: PermissionGrant): boolean {
    ...
    if (kindFilter !== "all" && (g.permissionKind ?? "any kind") !== kindFilter)
      return false;
    if (scopeFilter === "global" && g.conversationId !== null) return false;
    if (scopeFilter === "conversation" && g.conversationId === null)
      return false;
    if (provenanceFilter !== "all" && g.source !== provenanceFilter)
      return false;
  ...
  function getGrantStats(items: PermissionGrant[]) {
    return {
      total: items.length,
      allow: items.filter(
        (g) => g.decision === "allow" || g.decision === "force-allow",
      ).length,
  ...
  function buildGrantSections(items: PermissionGrant[]) {
    ...
    return [
      ...
      },
      {
        ...
        description:
          "Approve rules not marked as default seeds that apply across conversations.",
        grants: userGlobal,
      },
      {
        id: "conversation",
        ...
        description:
          "Approve rules created from a specific conversation prompt.",
        grants: conversation,
      },
      {
        id: "defaults",
        ...
        description:
          "Built-in safe defaults installed for each user; revocable and restorable.",
        grants: defaults,
      },
...
>
  <PanelHeader title="Saved permission grants" fullBleed>
    {#snippet meta()}Review persistent approve, deny, and prompt rules; audit
      defaults; and find conversation-scoped rules quickly.{/snippet}
  </PanelHeader>

  {#if workspaceFile && workspaceFile.present}
    <section
      class="workspace-file-panel"
      aria-label="Workspace permissions file"
    >
      <div class="wf-head">
        <span class="wf-title">
      ...
      <p class="muted small">
        Checked-in permission grants for <code
          >{workspaceFile.workspaceRoot}</code
        >.
        ...
        The file only becomes active grants after a human approves it; an agent cannot
        widen its own permissions by editing it.
      </p>
      ...
      {:else if workspaceFile.diff}
        ...
        <details class="wf-diff">
          ...
          <DiffView
            path={workspaceFile.fileName}
            diff={workspaceFile.diff}
            collapsible={false}
          />
        </details>
        <div class="wf-actions">
          <form method="POST" action="?/workspaceFileApprove">
            <button class="btn btn-primary" type="submit"
              >Approve &amp; apply current file</button
            >
          </form>
  ...
  <div class="grant-toolbar">
    <details
      ...
      bind:this={editorDetails}
    >
      <summary
        >{editingGrantId !== null
          ? "Edit permission grant"
          : "Add a permission grant"}</summary
      >
      <form
        method="POST"
      ...
      >
        {#if editingGrantId !== null}
          <p class="muted small">
            Editing grant #{editingGrantId}{editingGrantMeta?.conversationId
              ...
              : " (user-global)"}. The conversation scope and original grant
            time are preserved.
          </p>
          <input type="hidden" name="id" value={editingGrantId} />
        {:else}
          <p class="muted small">
            Author a user-global grant directly, without waiting for a tool
            prompt. Conversation-scoped grants are still created from the "Allow
            always" / "Deny always" buttons on the in-chat permission dialog.
          </p>
        {/if}
          ...
          <label class="deny-reason">
            ...
            {#if newGrantDecision === "prompt"}
              <span class="muted small"
                >Prefills the permission dialog's deny feedback and is sent to
                the agent if the prompt grant must auto-deny because no dialog
                can be shown. Max 500 chars.</span
              >
            {:else}
              <span class="muted small"
                >Surfaced to the agent as the SDK reject `feedback` string —
                explain *why* and what to do instead. Max 500 chars.</span
              >
            {/if}
          </label>
        {:else}
        ...
        {#if newGrantTool === "custom-tool"}
          <fieldset class="scope-fields custom-tool-fields">
            ...
            <label>
              ...
              />
              <span class="muted small">
                A portal tool is authorized as a whole — there is no finer
                scope, so the name is the scope. Names not in the list are
                accepted too (MCP / provider tools).
              </span>
            </label>
        ...
        {#if form?.formId === "createGrant" && form.ok}
          <div class="ok small">
            {form.duplicate
              ? "An identical grant already exists — no change."
              : "Grant created."}
          </div>
        {/if}
        ...
        <div class="form-actions">
          ...
          {#if editingGrantId !== null}
            <button class="btn" type="button" onclick={resetGrantForm}
              >Cancel</button
            >
          {/if}
        </div>
  ...
  {:else}
    ...
    >
      <div class="filter-header">
        <div>
          <h3>Find grants</h3>
          <p class="muted small">
            Showing {filteredStats.total} of {stats.total} grants. Hidden grants are
            only filtered from this view, not revoked.
          </p>
        </div>
        {#if hasActiveFilters}
          <button class="btn small" type="button" onclick={resetFilters}
    ...
    {:else}
      ...
      {#each grantSections as section (section.id)}
        <section
          class="grant-section"
          aria-labelledby={`grant-section-${section.id}`}
        >
            ...
            {#each section.grants as g (g.id)}
              <li class="grant-row">
                <div class="grant-row-main">
                  ...
                  {#if g.denyReason}
                    <p class="deny-reason-row muted small">
                      Feedback: {g.denyReason}
                    </p>
                  {/if}
                </div>
                  ...
                  <details class="grant-details">
                    ...
                    <dl>
                      ...
                      <div>
                        <dt>Conversation</dt>
                        <dd>
                          {g.conversationId ? grantScopeLabel(g) : "Global"}
                        </dd>
                      </div>
                      {#if g.conversationId}
                        <div>
                  ...
                  >
                    ...
                    {#if g.source !== "workspace-file" && canEditGrant(g)}
                      <button
                        ...
                        title="Prefill the grant editor with this grant"
                        >Edit</button
                      >
                    {/if}
