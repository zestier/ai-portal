<script lang="ts">
  ...
  import {
    applyLiveThemePreference,
    type ThemeMode,
  } from "$lib/client/theme-preview";
  ...
  <form method="POST" action="?/save" class="settings-form">
    <label>
      ...
      <span class="muted small">
        Seeds the model id for newly created conversations. Leave blank to use
        the server default.
      </span>
    </label>
    <label>
    ...
    <label>
      ...
      <span class="muted small">
        Applies to newly created conversations. Existing conversations keep
        their current mode.
        <br />
        {MODE_OPTIONS.find(
          (opt) => opt.value === settings.defaultConversationMode,
        )?.hint}
      </span>
    </label>
    <label>
      ...
      <span class="muted small">
        Applies to newly created conversations; each conversation can change it
        in its header.
        <br />
        {APPROVAL_MODE_OPTIONS.find(
          (opt) => opt.value === settings.defaultApprovalMode,
        )?.hint}
      </span>
    </label>
    <label>
    ...
    <label>
      ...
      <span class="muted small">
        Tints buttons, links, highlights and the favicon. Handy for telling
        apart multiple portal copies at a glance. Applies on top of the
        light/dark theme.
      </span>
    </label>
    ...
    <div class="form-actions">
      ...
      {#if form?.formId === "save" && form.ok}<span class="ok">Saved.</span
        >{/if}
      {#if form?.formId === "save" && form.error}<span class="err"
          >{form.error}</span
        >{/if}
    </div>
  </form>
