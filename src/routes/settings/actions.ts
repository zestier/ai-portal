...
} from "$lib/server/permissions/seed-grants";
import { applyWorkspaceFile } from "$lib/server/permissions/workspace-file-gate";
...
const UpdatePromptTemplateSchema = z
  .object({
    id: z
      .string()
      .min(1)
      .refine(promptTemplateId.is, { message: "Invalid prompt template id" }),
    type: z.enum(["chat", "ticket-action"]).optional().default("chat"),
    title: z.string().trim().min(1).max(120),
    description: z.string().trim().max(500).optional(),
    prompt: z.string().trim().min(1).max(20_000),
...
export const actions: Actions = {
  save: async ({ request, locals, getClientAddress }) => {
    if (!locals.userId)
      return { ok: false, error: "Not authenticated", formId: "save" };
    ...
    const next: UserSettings = {
      ...
      defaultConversationMode: parsed.data
        .defaultConversationMode as SessionMode,
  ...
  createPromptTemplate: async ({ request, locals }) => {
    ...
    const data = await request.formData();
    const type =
      data.get("type") === "ticket-action" ? "ticket-action" : "chat";
    ...
    const parsed = PromptTemplateSchema.safeParse({
      ...
      appendSystemPrompt:
        (data.get("appendSystemPrompt") as string) || undefined,
      ...
      disabledToolGroups: PORTAL_TOOL_GROUP_IDS.filter(
        (id) => !enabled.has(id),
      ),
    ...
    promptTemplates.create(locals.userId, {
      ...
      disabledToolGroups: sanitizeDisabledToolGroups(
        parsed.data.disabledToolGroups,
      ),
  ...
  updatePromptTemplate: async ({ request, locals }) => {
    ...
    const updated = promptTemplates.update(id, locals.userId, {
      ...
      ...(patch.launchBehavior !== undefined
        ? { launchBehavior: patch.launchBehavior }
        : {}),
  ...
  saveDefaultPromptTemplate: async ({ request, locals }) => {
    ...
    if (raw !== "") {
      ...
      const custom =
        intId === null ? null : promptTemplates.get(intId, locals.userId);
      const isOpenChatCustom =
        custom?.type === "chat" && custom?.status === "open";
  ...
  updateMemoryProfile: async ({ request, locals }) => {
    ...
    const updated = memoryProfiles.updateCustomProfile(
      parsed.data.id,
      locals.userId,
      {
        name: parsed.data.name,
  ...
  restoreSeedGrants: async ({ locals }) => {
    if (!locals.userId) {
      return fail(401, {
        ok: false,
        error: "Not authenticated",
        formId: "restoreSeedGrants",
      });
    }
    const result = restoreSeedGrantsForUser(locals.userId);
    log.info("settings.seed_grants_restored", {
      userId: locals.userId,
      ...result,
    });
  ...
  createGrant: async ({ request, locals }) => {
    if (!locals.userId)
      return fail(401, { ok: false, error: "Not authenticated" });
    const data = await request.formData();
...
function parseGrantFormData(data: FormData, formId: string): ParseGrantResult {
  ...
  if (typeof scopeJson !== "string" || scopeJson.length === 0) {
    return {
      ok: false,
      failure: fail(400, { ok: false, error: "Missing scope payload", formId }),
    };
  }
  try {
    ...
    return {
      ...
      failure: fail(400, {
        ok: false,
        error: "Scope payload was not valid JSON",
        formId,
      }),
    };
  }
  ...
  if (expiresAt !== null && Number.isNaN(expiresAt)) {
    return {
      ok: false,
      failure: fail(400, { ok: false, error: "Invalid expiry date", formId }),
    };
  }
...
export function markSeedGrants(grants: settings.GrantSummary[]) {
