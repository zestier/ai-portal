<script lang="ts">
  ...
  async function saveProvider() {
    ...
    if (!/^[a-z0-9][a-z0-9_-]*$/.test(pId)) {
      flash(
        "err",
        'Provider id must be lowercase letters, digits, "-" or "_".',
      );
      return;
    }
    if (!pBuiltin && !pBaseUrl.trim()) {
      flash(
        "err",
        "Custom providers need a base URL (e.g. http://localhost:11434/v1).",
      );
      return;
    }
    savingProvider = true;
...
>
  <PanelHeader title="Models" fullBleed>
    {#snippet meta()}
      Configure providers and models the portal serves. Keys are encrypted at
      rest; the catalog is written to <code>DATA_DIR/models.json</code> and reloaded
      live.
    {/snippet}
  </PanelHeader>
  ...
  {:else}
    <div class="models-body">
      {#if providers.length === 0}
        <p class="muted">
          No providers configured yet. Add one to start serving models — pick a
          bundled provider (Anthropic, OpenAI, …) to use pi's built-in catalog,
          or a custom OpenAI-compatible endpoint (Ollama, vLLM, LM Studio,
          gateways).
        </p>
      {/if}
          ...
          <header class="provider-head">
            ...
            <div>
              ...
              <span class="muted small"
                >{p.id} · {p.api}{p.builtin ? " · built-in" : ""}</span
              >
            </div>
            <div class="head-actions">
              ...
              <button
                type="button"
                class="danger"
                onclick={() => removeProvider(p.id)}>Delete</button
              >
            </div>
          </header>
            ...
            <div id="provider-body-{p.id}">
              ...
              <div class="key-row">
                ...
                {#if p.hasKey}
                  <button
                    type="button"
                    class="danger"
                    onclick={() => saveKey(p.id)}>Remove</button
                  >
                {/if}
              </div>
              ...
              <div class="provider-actions">
                <button
                  ...
                  title={p.hasKey
                    ? undefined
                    : "Save an API key first to fetch the catalog."}
                  ...
                  <div class="model-row">
                    <label class="toggle">
                      ...
                      <span>
                        ...
                        <span class="muted small cost"
                          >{formatCost(m.cost)}</span
                        >
                      </span>
                    </label>
                    <button
              ...
              {#if modelFormFor === p.id}
                <div class="model-form">
                  ...
                  <div class="checks">
                    <label
                      ><input type="checkbox" bind:checked={mReasoning} /> reasoning</label
                    >
                    <label
                      ><input type="checkbox" bind:checked={mEnabled} /> enabled</label
                    >
                  </div>
              ...
              {:else}
                <button
                  type="button"
                  class="add"
                  onclick={() => (modelFormFor = p.id)}>+ Add model</button
                >
              {/if}
            </div>
          {/if}
        ...
        <section class="provider-card new">
          ...
          <div class="grid">
            <label>
              ...
              <input
                bind:value={pId}
                placeholder="anthropic, ollama, my-gateway…"
              />
            </label>
            <label
              >Name <input bind:value={pName} placeholder="Anthropic" /></label
            >
            <label>
              API
              <select
                value={pApi}
                onchange={(e) => (pApi = e.currentTarget.value as ProviderApi)}
              >
            ...
            <label class="span-2">
              ...
              <textarea
                bind:value={pHeaders}
                rows="2"
                placeholder="x-portkey-api-key: …"></textarea>
            </label>
          </div>
          <div class="checks">
            <label
              ><input type="checkbox" bind:checked={pBuiltin} /> built-in provider
              (pi's bundled catalog)</label
            >
            <label
              ><input type="checkbox" bind:checked={pEnabled} /> enabled</label
            >
          </div>
          <div class="form-actions">
            <button
