<script lang="ts">
  ...
  function surfaceVerify(res: VerifyResult, label: string) {
    ...
    if (failed === 0) {
      ...
      flash(
        "err",
        `${label}: ${loaded} loaded, ${failed} error(s)${detail ? ` — ${detail}` : ""}`,
      );
    }
  }
</script>
...
<section class="extensions-card">
  ...
  <div class="warning">
    <strong
      >Extensions run with full system permissions and execute arbitrary code.</strong
    >
    Only add code you trust. Package sources install remote code via npm/git on the
    first open after a change (into the pi agent dir's temp folder). In multi-user
    mode this panel is admin-only.
  </div>

  <p class="muted small note">
    Changes apply to every conversation on its next turn (open sessions are
    recreated automatically). No auto-discovery — only the sources listed here
    load.
  </p>
  ...
  {:else}
    <div class="ext-list">
      ...
      {:else}
        <p class="muted small">
          No extensions configured — everything behaves exactly as today.
        </p>
      {/each}
    </div>
        ...
        <div class="grid">
          ...
          <label>
            ...
            <select
              value={formKind}
              onchange={(e) =>
                (formKind = e.currentTarget.value as PortalExtensionKind)}
            >
          ...
          <label class="span-2">
            {#if formKind === "file"}
              Path to a <code>.ts</code> file or directory (<code>index.ts</code
              >), relative to the project root (absolute paths pass through)
            ...
            {#if formKind === "package"}
              ...
              <span class="muted small"
                >Pin an explicit @version/@ref — unpinned git refs re-pull on
                every session.</span
              >
            {:else if formKind === "file"}
              <input
                bind:value={formValue}
                placeholder="extensions/my-extension.ts"
              />
            {:else}
              <textarea
                bind:value={formValue}
