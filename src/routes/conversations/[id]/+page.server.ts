...
export const load: PageServerLoad = async ({ params, locals, url }) => {
  ...
  const memorySnapshot = memory.listSnapshot(conversationId, {
    userId: conv.userId,
  });
  let initialComposer = "";
  ...
  if (draftTicketId && !hasMessages) {
    ...
    const ticket =
      draftTicketInt === null
        ? null
        : tickets.get(draftTicketInt, locals.userId);
    if (
      !ticket ||
      ticket.workspaceKey !== ticketWorkspaceFromConversation(conv)
    ) {
      throw error(404);
    }
    ...
    const action =
      actionInt === null ? null : promptTemplates.get(actionInt, locals.userId);
    if (
      !action ||
  ...
  if (conv.forkedFromConversationId) {
    const p = convs.get(
      convCodec.parse(conv.forkedFromConversationId),
      locals.userId,
    );
    if (p) {
      ...
      if (conv.forkedFromMessageId) {
        ...
        const i = parentMsgs.findIndex(
          (m) => m.id === conv.forkedFromMessageId,
        );
        idx = i >= 0 ? i : null;
      }
      parent = {
