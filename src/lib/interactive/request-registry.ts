...
export function defaultInteractiveResponse(
...
export type InformationalInteractiveKind =
...
interface InformationalRequestDescriptor<
  K extends InformationalInteractiveKind,
> {
  heading(
    request: Extract<InformationalInteractiveRequest, { kind: K }>,
  ): string;
  actionLabel: string;
...
export function isInformationalInteractiveRequest(
  ...
  return (
    ...
    request.kind === "mcp_oauth" ||
    request.kind === "external_tool"
  );
}

export function informationalHeading(
...
export function informationalActionLabel(
  request: InformationalInteractiveRequest,
): string {
