...
export const PORTAL_TOOL_GROUPS: readonly PortalToolGroup[] = [
  {
    id: "shell",
    label: "Shell",
    hint: "Run supervised, workspace-scoped Bash commands.",
  },
  {
    id: "git",
    label: "Git",
    hint: "Structured git status/diff/log/show/commit tools.",
  },
  {
    id: "filesystem",
    label: "Filesystem",
  ...
  },
  {
    id: "tickets",
    label: "Tickets",
    hint: "Durable workspace ticket create/list/update tools.",
  },
  {
    id: "permissions",
    label: "Permissions",
...
export const PORTAL_TOOL_GROUP_IDS: readonly PortalToolGroupId[] =
  PORTAL_TOOL_GROUPS.map((g) => g.id);

const PORTAL_TOOL_GROUP_ID_SET: ReadonlySet<string> = new Set(
  PORTAL_TOOL_GROUP_IDS,
);

export function isPortalToolGroupId(
  value: unknown,
): value is PortalToolGroupId {
 ...
 */
export function sanitizeDisabledToolGroups(
  value: unknown,
): PortalToolGroupId[] {
