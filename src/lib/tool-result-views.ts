// Shared, client-safe home of the envelope → views projection. This module is
// pure (zero imports) so both sides of the `$lib/server` boundary can use it:
// the server re-exports it from `src/lib/server/tools/types.ts` (every existing
// server caller and test is untouched), and the client decoder
// (`src/lib/client/tool-result.ts`) imports it directly to recompute the exact
// model-facing text of a persisted envelope. Both views — model text and UI
// payload — derive from this ONE source, so they can never silently diverge.
//
// The envelope (`ToolResult`) is the single provider-agnostic internal API: at
// each provider boundary it is projected (via `deriveToolResultViews`) into two
// views that derive from this one source — a concise, RAW text for the model
// and the full serialized payload for the UI.
...
export function ok(
  ...
  if (opts?.followUpHint !== undefined)
    envelope.followUpHint = opts.followUpHint;
  if (opts?.binary !== undefined && opts.binary.length > 0)
    envelope.binary = opts.binary;
  if (opts?.views !== undefined && opts.views.length > 0)
    envelope.views = opts.views;
  return envelope;
}
...
export function err(
  ...
  if (opts?.detailsUiOnly !== undefined)
    error.detailsUiOnly = opts.detailsUiOnly;
...
function deriveModelText(envelope: ToolResult): string {
  if (!envelope.ok) {
    ...
    let text = detailsUiOnly
      ? message
      : code
        ? `${message} (code: ${code})`
        : message;
    if (details !== undefined && !detailsUiOnly)
      text += `\n${renderReadable(details)}`;
    return text;
  }
  const rendered = renderViewText(envelope.views);
  const payload =
    rendered !== undefined ? rendered : derivePayloadText(envelope);
...
function renderViewText(
  views: ToolResultView[] | undefined,
): string | undefined {
  const texts =
    views
      ?.filter(
        (v): v is { type: "text"; text: string } =>
          v.type === "text" && v.text.trim().length > 0,
      )
      .map((v) => v.text) ?? [];
...
function derivePayloadText(envelope: {
  result?: unknown;
  summary?: string;
}): string {
  if (envelope.result !== undefined) {
    const text =
      typeof envelope.result === "string"
        ? envelope.result
        : renderReadable(envelope.result);
