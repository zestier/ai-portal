...
export function templatePermissionPreview(
  ...
  const action: "create" | "update" =
    tool === "template_update" ? "update" : "create";

  const id = action === "update" ? optionalString(args.id) : null;
  const title = optionalString(args.title);
  const type =
    optionalString(args.type) ?? (action === "create" ? "chat" : null);
  ...
  const merged =
    action === "update" && before ? buildMergedView(args, before) : null;
...
function buildMergedView(
  ...
  pushChange(
    "Model",
    before.model,
    afterTicketActionField("model", before.model),
  );
  ...
  pushChange(
    "Status",
    before.status,
    afterStringKeepCurrent(args, "status", before.status),
  );
...
export function summarizeTemplatePermission(
  tool: string,
  args: unknown,
): string | null {
  ...
  if (preview.action === "update" && preview.id)
    lines.push(`Template: ${preview.id}`);
  ...
  if (preview.launchBehavior)
    lines.push(`Launch behavior: ${preview.launchBehavior}`);
  if (preview.conversationMode)
    lines.push(`Conversation mode: ${preview.conversationMode}`);
  if (preview.approvalMode) lines.push(`Approvals: ${preview.approvalMode}`);
  if (preview.model) lines.push(`Model: ${preview.model}`);
  if (preview.pinned !== null)
    lines.push(`Pinned: ${preview.pinned ? "yes" : "no"}`);
  ...
  lines.push(
    "Approval: one-time only; stored grants are disabled for this tool.",
  );
  return lines.join("\n");
}
