...
export function shellOptionSpecsToCsv(specs: ShellOptionSpec[]): string {
  return specs
    .map((spec) =>
      spec.kind === "flag" ? spec.name : `${spec.name}=${spec.value.kind}`,
    )
    .join(", ");
}

export function commandTailToText(
  command: ShellCommandStep[] | undefined,
): string {
  return (command ?? [])
    .slice(1)
    .map((step) => step.token)
...
export function buildGrantScopeJson(
  tool: GrantTool,
...
export function grantScopeToFormFields(scope: GrantScope | null): {
  ...
  switch (scope.kind) {
    case "shell":
      ...
      fields.shellPositionalMin = countBoundToText(
        scope.rule.positionalCount?.min,
      );
      fields.shellPositionalMax = countBoundToText(
        scope.rule.positionalCount?.max,
      );
...
function parseCountBounds(fields: GrantScopeFormFields): {
  ...
  if (
    ...
    return {
      value: null,
      error: "positional count minimum must not exceed the maximum",
    };
  }
...
function parseCountBound(
  ...
  if (!Number.isInteger(n) || n < 0) {
    return {
      value: undefined,
      error: `positional count ${label} must be a non-negative integer`,
    };
  }
...
function buildShellScopeJson(fields: GrantScopeFormFields): BuildResult {
  if (!fields.shellArgv0.trim())
    return { json: null, error: "argv0 is required" };
  const command: ShellCommandStep[] = shellCommandTokens(fields).map(
    (token) => ({ token }),
  );
  const rule: Record<string, unknown> = { command };
  if (fields.shellPositionals !== "unset")
    rule.positionals = { kind: fields.shellPositionals };
  ...
  for (let i = 0; i < command.length; i++) {
    const allow = parseShellOptionSpecs(
      fields.shellStepOptions[i]?.allow ?? "",
    );
...
function buildUrlScopeJson(fields: GrantScopeFormFields): BuildResult {
  const builders = {
    exact: () => {
      if (!fields.urlExact.trim())
        return { json: null, error: "URL is required" } as const;
      return {
        json: JSON.stringify({
          kind: "url",
          rule: { kind: "exact", url: fields.urlExact.trim() },
        }),
        error: null,
      } as const;
    },
    host: () => {
      if (!fields.urlHost.trim())
        return { json: null, error: "host is required" } as const;
      return {
        json: JSON.stringify({
          kind: "url",
          rule: { kind: "host", host: fields.urlHost.trim() },
        }),
        error: null,
      } as const;
    },
    "host-suffix": () => {
      if (!fields.urlSuffix.trim())
        return { json: null, error: "suffix is required" } as const;
      return {
        json: JSON.stringify({
          kind: "url",
...
function buildFsScopeJson(
  tool: Exclude<GrantTool, "shell" | "url">,
  fields: GrantScopeFormFields,
) {
  const perms = [tool];
  ...
  if (fields.fsBehavior === "any") {
    if (fields.fsRoot === "absolute") {
      return {
        json: null,
        error: "absolute root requires exact, prefix, or glob behavior",
      };
    }
...
export function isFsBehaviorWithValue(
  ...
  return FS_RULE_BEHAVIORS_WITH_VALUE.includes(
    behavior as FsRuleBehaviorWithValue,
  );
}
