...
export const BUILT_IN_PROMPT_TEMPLATES: ChatPromptTemplate[] = [
  {
    ...
    title: "Code review",
    description:
      "Review changed code for bugs, regressions, and security issues.",
    prompt:
  ...
  },
  {
    ...
    description:
      "Investigate a failing command, stack trace, or unexpected behavior.",
    prompt:
  ...
  },
  {
    ...
    title: "Explain code",
    description:
      "Explain how a feature, file, or flow works in this repository.",
    prompt:
...
export function listBuiltInPromptTemplates(): PromptTemplateListItem[] {
  return BUILT_IN_PROMPT_TEMPLATES.map((template) => ({
    ...template,
    source: "builtin",
  }));
}
...
export function getBuiltInPromptTemplate(
  ...
  return (
    BUILT_IN_PROMPT_TEMPLATES.find((template) => template.id === id) ?? null
  );
}
...
export function templateNeedsReview(
  template: Pick<ChatPromptTemplate, "launchBehavior">,
): boolean {
...
export function workspaceModeLabel(
  ...
  return templateWorkspace(template) === "worktree"
    ? "Isolated worktree"
    : null;
}
...
export function placeholdersForType(
 ...
 */
export function unknownPlaceholderMessage(
  type: PromptTemplateType,
  unknown: string[],
): string {
...
export const TICKET_ACTION_DEFAULTS: readonly TicketActionDefault[] = [
  ...
  },
  {
    ...
    title: "Draft",
    description:
      "Open an editable draft chat pre-filled with the ticket prompt.",
    prompt: DO_PROMPT,
    launchBehavior: "draft",
    conversationMode: null,
    approvalMode: null,
