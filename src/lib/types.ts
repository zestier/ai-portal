...
export const TICKET_PRIORITIES: readonly WorkspaceTicketPriority[] = [
  "P0",
  "P1",
  "P2",
  "P3",
...
export function normalizePromptTemplateType(
...
export const PROMPT_LAUNCH_BEHAVIORS: readonly PromptLaunchBehavior[] = [
  "send",
  "draft",
  "review",
];
...
export function defaultLaunchBehavior(
...
export function normalizeMemoryMode(
  raw: string | null | undefined,
): MemoryMode {
  return raw === "lightweight" ||
    raw === "project" ||
    raw === "story" ||
    raw === "strict"
    ? raw
...
export const THEME_ACCENT_IDS = THEME_ACCENTS.map(
  (a) => a.value,
) as readonly ThemeAccent[];

export function normalizeThemeAccent(
  raw: string | null | undefined,
): ThemeAccent {
  return THEME_ACCENT_IDS.includes(raw as ThemeAccent)
    ? (raw as ThemeAccent)
    : "default";
}

export function themeAccentHex(accent: ThemeAccent): string {
  return (THEME_ACCENTS.find((a) => a.value === accent) ?? THEME_ACCENTS[0])
    .hex;
}
...
export type InteractiveRequestView = {
  requestId: string;
