 ...
 */
export type TranscriptEntry = Omit<TranscriptIndexEntry, 'id'> & { id: DisplayId };
...
function clientPreview(content: string | null, maxChars: number): string | null {
...
export function entryFromMessage(m: DisplayMessage): TranscriptEntry {
	...
	for (const r of m.reasoningBlocks ?? []) {
		...
		records.push({
			...
			summary: r.summary ?? (r.kind === 'content' ? (preview ?? '(content)') : 'Thinking…')
		});
	}
	return {
		id: m.id,
...
export function prependIndexPage(
	...
	state.entries = [...entries.filter((e) => !existing.has(e.id)), ...state.entries];
 ...
 */
export function mergeRefresh(state: TranscriptState, p: TranscriptProjection): void {
	const lastLocal = state.entries.length > 0 ? state.entries[state.entries.length - 1] : null;
	const lastServer = p.tail.length > 0 ? p.tail[p.tail.length - 1] : null;
	const truncated =
		(lastServer === null && lastLocal !== null) ||
		(lastServer !== null && lastLocal !== null && Number(lastServer.id) < Number(lastLocal.id));
