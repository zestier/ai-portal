...
export function selectSubagentChildren(
  ...
  return {
    tools: (pools.tools ?? []).filter(
      (t) =>
        t.parentToolCallId === parentToolCallId && t.id !== parentToolCallId,
    ),
    reasoning: (pools.reasoning ?? []).filter(
      (r) => r.parentToolCallId === parentToolCallId,
    ),
    edits: (pools.edits ?? []).filter(
      (e) => e.parentToolCallId === parentToolCallId,
    ),
  };
}
...
export function procWorkerPrompt(args: SubagentArgs): string | null {
  if (!args.procedure || !args.result_requirements) return null;
  return JSON.stringify(
    {
      ...(args.summary ? { summary: args.summary } : {}),
      procedure: args.procedure,
      result_requirements: args.result_requirements,
...
export function getSubagentPresentation(
  agentType: string | undefined,
): SubagentPresentation {
  if (
    agentType &&
    Object.prototype.hasOwnProperty.call(AGENT_PRESENTATION, agentType)
  ) {
...
function toolElapsedMs(toolCall: ToolCallRecord): number | null {
  return toolCall.endedAt != null
    ? Math.max(0, toolCall.endedAt - toolCall.startedAt)
    : null;
}
...
export function getSubagentDisplayState(
  ...
  const isBackgroundLaunch =
    args.mode === "background" && toolCall.status === "ok";
  const pending = toolCall.status === "pending";
  const backgroundAgentId =
    toolCall.backgroundAgentId ?? getBackgroundAgentId(toolCall.resultJson);
  ...
  if (isBackgroundLaunch) {
    ...
    const elapsedStartMs = inProgress
      ? (toolCall.backgroundAgentStartedAt ?? null)
      : null;
    return {
      pending,
      isBackgroundLaunch,
