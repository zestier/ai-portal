...
class InProcessAppEventBus implements AppEventBus {
  ...
  private reapIdle(now: number): void {
    for (const [userId, ch] of this.channels) {
      if (
        ch.subscribers.size === 0 &&
        now - ch.lastActivityAt > IDLE_CHANNEL_TTL_MS
      ) {
  ...
  publish(userId: number, event: AppEvent): void {
    ...
    if (ch.buffer.length > REPLAY_LIMIT)
      ch.buffer.splice(0, ch.buffer.length - REPLAY_LIMIT);
  ...
  async *subscribe(
    ...
    const replay =
      sinceId === undefined
        ? ch.buffer.slice()
        : ch.buffer.filter((e) => e.id > sinceId);
