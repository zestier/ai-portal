    ...
    track(
      setTimeout(
        () =>
          trip(
            new Error(
              "Memory extraction exceeded its time budget and was abandoned.",
            ),
          ),
 ...
 * turn that owns it.
 */
export async function runMemoryExtractionCard(
  o: MemoryExtractionCardOptions,
): Promise<void> {
  ...
  if (o.turnAc.signal.aborted) linkExtractionAbort();
    ...
    o.turnAc.signal.addEventListener("abort", linkExtractionAbort, {
      once: true,
    });
  ...
  try {
    ...
    const committed = await runExtractionWithWatchdog(
      extractAndCommitMemory({
        ...
        assistantMessage,
        onActivity,
        ...(o.extractorModel !== undefined
          ? { extractorModel: o.extractorModel }
          : {}),
        ...(o.beforeCommit !== undefined
          ? { beforeCommit: o.beforeCommit }
          : {}),
        ...(o.priorPatchId !== undefined
    ...
    o.emit({
      ...
      conversationId: convCodec.encode(o.conversationId),
      phase:
        committed.patch.status === "needs_review"
          ? "needs_review"
          : "committed",
    ...
    if (extractorParentId) {
      o.dispatch({
        ...
        summary: aborted
          ...
          : memoryFailureSummary(memoryErr),
