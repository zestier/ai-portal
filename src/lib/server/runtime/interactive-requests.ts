...
export class InteractivePromptCancelledError extends Error {
  ...
  constructor(
    reason: string,
    auditDecision: "auto-cancelled" | "auto-expired",
  ) {
    super(reason);
...
function conversationHasBlocking(conversationId: number): boolean {
  for (const p of pending.values()) {
    if (p.conversationId === conversationId && isBlockingKind(p.kind))
      return true;
  }
  return false;
}
...
export function register(opts: RegisterOptions) {
  ...
  log.info("interactive.registered", {
    requestId: opts.requestId,
    kind: opts.kind,
  });
}
    ...
    try {
      ...
      const isAlways =
        ...
        (response.decision === "allow-always" ||
          response.decision === "deny-always");
      if (isAlways) {
        const grantDecision =
          response.decision === "allow-always" ? "allow" : "deny";
        const targetConversationId = response.applyToAllConversations
          ? null
          : p.conversationId;
        const expiresAt =
          typeof response.expiresInMs === "number"
            ? Date.now() + response.expiresInMs
            : null;
        ...
        if (response.additionalScopes)
          scopes.push(...response.additionalScopes);
      ...
      log.warn("interactive.permission_persist_failed", {
        requestId,
        err: String(e),
      });
    }
...
export function cancel(requestId: string, reason: string = "cancelled") {
  ...
  if (p.kind === "permission" && p.view.kind === "permission") {
    try {
      ...
      log.warn("interactive.cancel_audit_failed", {
        requestId,
        err: String(e),
      });
    }
 ...
 */
export function expireConversation(
  conversationId: number,
...
function expire(requestId: string, reason: string) {
  ...
  if (p.kind === "permission" && p.view.kind === "permission") {
    try {
      ...
      log.warn("interactive.expire_audit_failed", {
        requestId,
        err: String(e),
      });
    }
...
function expiredResponse(kind: InteractiveKind): InteractiveResponse {
  if (kind === "permission") {
    return {
      kind: "permission",
      decision: "deny",
      feedback: SESSION_EXPIRED_FEEDBACK,
    };
  }
...
function normalizeResponse(response: InteractiveResponse): InteractiveResponse {
  ...
  if (
    !feedback ||
    (response.decision !== "deny" && response.decision !== "deny-always")
  ) {
  ...
  switch (policy) {
    ...
    default:
      if (isFilesystemPermissionKind(pk)) {
        ...
        if (target && isPathInAnyWorkspace(target, ctx?.workspaceRoots))
          return "approved";
        return "ask";
      }
      return "ask";
