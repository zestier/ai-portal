...
export function previewCut(text: string | null, maxChars: number): string | null {
	if (!text) return null;
...
function taskMeta(tc: ToolCallRecord): Record<string, unknown> | undefined {
	...
	if (tc.backgroundAgentStatus) meta.backgroundAgentStatus = tc.backgroundAgentStatus;
	try {
		const args = tc.argsJson ? (JSON.parse(tc.argsJson) as Record<string, unknown>) : null;
		if (args && typeof args === 'object' && !Array.isArray(args)) {
			for (const key of ['agent_type', 'model', 'mode', 'description', 'name'] as const) {
				const v = args[key];
...
function reasoningSummary(r: ReasoningBlockRecord): string {
	...
	return (
		[duration ? `Thought for ${duration}` : null, preview].filter(Boolean).join(' · ') ||
		'Thinking…'
	);
}
...
function projectTool(t: ToolCallRecord, rules: TrimRules): ToolCallRecord {
	...
	return {
		...t,
		argsJson: args.value,
		...(args.truncated ? { argsTruncated: true as const, argsBytes: args.bytes } : {}),
		resultJson: result.value,
		...(result.truncated ? { resultTruncated: true as const, resultBytes: result.bytes } : {}),
...
function projectEdit(e: FileEditRecord, rules: TrimRules): FileEditRecord {
	...
	return {
		...e,
		diff: diff.value,
		...(diff.truncated ? { diffTruncated: true as const, diffBytes: diff.bytes } : {}),
		summary: editSummary(e)
	};
}

function projectReasoning(r: ReasoningBlockRecord, rules: TrimRules): ReasoningBlockRecord {
	...
	return {
		...r,
		text: text.value,
		...(text.truncated ? { textTruncated: true as const, textBytes: text.bytes } : {}),
...
export function projectMessageBody(msg: Message, rules: TrimRules): Message {
	return {
		...
		reasoningBlocks: (msg.reasoningBlocks ?? []).map((r) => projectReasoning(r, rules))
	};
}
...
function reasoningDescriptor(r: ReasoningBlockRecord): TranscriptRecordDescriptor {
	return {
		kind: 'reasoning',
		id: r.id,
...
function recordDescriptors(msg: Message): TranscriptRecordDescriptor[] {
	...
	for (const r of msg.reasoningBlocks ?? []) {
		...
		else anchors.push({ offset: Math.min(r.textOffset, msg.content.length), ts: r.startedAt, d });
	}
	for (const t of msg.toolCalls ?? []) {
		const d = toolDescriptor(t);
		if (t.textOffset == null) trailing.push(d);
		else anchors.push({ offset: Math.min(t.textOffset, msg.content.length), ts: t.startedAt, d });
	}
	for (const e of msg.fileEdits ?? []) {
		const d = editDescriptor(e);
		if (e.textOffset == null) trailing.push(d);
		else anchors.push({ offset: Math.min(e.textOffset, msg.content.length), ts: e.createdAt, d });
	}
