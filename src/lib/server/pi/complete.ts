...
export async function resolveModelSelection(
	...
	if (!model) {
		throw new ModelCompletionError(selection, `pi model not found: ${selection}`);
	}
...
export async function piChat(
	...
	const stream = opts.runtime.stream(opts.model, buildContext(opts.model, messages, tools), {
		temperature: 0,
...
export async function piCompleteSimple(opts: PiCompleteOptions): Promise<string> {
...
function toPiTool(spec: ExtractorToolSpec): NonNullable<PiContext['tools']>[number] {
	return {
		...
		switch (event.type) {
			...
			case 'error':
				...
				throw new ModelCompletionError(
					...
					event.error.errorMessage ?? `model completion failed (${event.reason})`
				);
		}
	}
	throw new ModelCompletionError(
...
function assistantTurnOf(message: PiAssistantMessage): ExtractorAssistantTurn {
	...
	for (const block of message.content) {
		if (block.type === 'text') content.push(block.text);
			...
			reasoning = reasoning === undefined ? block.thinking : `${reasoning}\n${block.thinking}`;
