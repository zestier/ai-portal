...
export async function forkAtMessage(input: ForkInput): Promise<ForkResult> {
  ...
  if (target.role === "user") {
    ...
    throw new ForkRejected(
      "unsupported_role",
      `Cannot fork from a ${target.role} message.`,
    );
  }
  ...
  const isolate =
    input.workspaceKind === "managed-worktree" ||
  ...
  if (isolate) {
    ...
    if (!snapshot) {
      throw new ForkRejected(
        "no_snapshot",
        "No snapshot exists for this message.",
      );
    }
    const sourceWorkdir =
      source.workspaceKind === "managed-worktree"
        ? convs.getManagedWorktree(convCodec.parse(source.id), input.userId)
            ?.sourceWorkdir
        : source.workdir;
    if (!sourceWorkdir) {
      throw new ForkRejected(
        "source_not_found",
        "Managed source workspace is unavailable.",
      );
    }
  ...
  const tx = getDb().transaction((): ForkResult => {
    if (managedWorktree) {
      ...
      getDb()
        .prepare(
          `UPDATE conversations SET workdir = ?, workspace_kind = ? WHERE id = ?`,
        )
        .run(
    ...
    const messageIdMap = cloneMessagePrefix(
      convCodec.parse(newConv.id),
      prefix,
    );
    ...
    const memoryCounts = memoryRepo.replaySessionMemoryLogForFork(
      ...
      {
        messageIdMap,
        createdBefore: firstDiscarded
          ? firstDiscarded.createdAt
...
function cloneMessagePrefix(
  ...
  const setToolParent = db.prepare(
    `UPDATE tool_calls SET parent_tool_call_id = ? WHERE id = ?`,
  );
    ...
    prefix.forEach((m, i) => {
      ...
      for (const t of m.toolCalls ?? []) {
        ...
        if (
          ...
          t.backgroundAgentId &&
          t.backgroundAgentStartedAt != null
        ) {
          insertLifecycle.run(
            remappedToolId,
            t.backgroundAgentId,
      ...
      for (const e of m.fileEdits ?? []) {
        insertEdit.run(
          ...
          e.textOffset,
          e.parentToolCallId
            ? (toolIdRemap.get(toolCodec.parse(e.parentToolCallId)) ?? null)
            : null,
        );
      }
      for (const r of m.reasoningBlocks ?? []) {
        insertReasoning.run(
          ...
          r.durationMs,
          r.parentToolCallId
            ? (toolIdRemap.get(toolCodec.parse(r.parentToolCallId)) ?? null)
            : null,
        );
      }
    });
