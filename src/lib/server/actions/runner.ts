...
export function scrubLog(text: string, env: NodeJS.ProcessEnv = process.env): string {
	let scrubbed = text;
...
export function runStep(
	...
	return new Promise<number>((resolve) => {
		...
		const childEnv = step.inheritEnv === false ? (step.env ?? {}) : { ...process.env, ...step.env };
...
export interface RunSequenceOptions {
	...
	runner?: (step: Step, emit: (ev: ActionEvent) => void, signal?: AbortSignal) => Promise<number>;
...
export async function* runSequence(
	...
	const { rollover = false, runner = runStep, signal, logLabel = 'action' } = opts;
