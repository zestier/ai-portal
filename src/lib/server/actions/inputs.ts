...
export function resolveInputValues(
	...
	for (const input of inputs) {
		...
		const hasProvided = provided !== undefined && provided !== null && provided !== '';
		...
		switch (input.type) {
			...
			case 'number': {
				const num = typeof source === 'number' ? source : Number(String(source).trim());
 ...
 */
export function substituteArg(arg: string, values: Record<string, string>): string {
