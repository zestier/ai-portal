...
type MergeableStatus = WorktreeIntegrationStatus & {
  branch: string;
...
export async function mergeToSource(
  ...
  const squashedCommits = opts.squash
    ? await squashBranch(status, opts.squash)
    : null;
...
async function finish(
  ...
  return {
    ...partial,
    headSha,
    status: await worktreeIntegrationStatus(status.path, opts),
  };
}
