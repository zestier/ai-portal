...
export async function listWorktrees(
  ...
  const records = parseWorktreeList(
    await gitOk(cwd, ["worktree", "list", "--porcelain"]),
  );
  const main = records[0];
  if (!main) {
    throw new WorktreeIntegrationError(
      "git_failed",
      "could not determine the main worktree",
    );
  }
  const worktrees = await Promise.all(
    records.map(async (record): Promise<WorktreeListEntry> => {
      ...
      const dirty =
        opts.includeDirty && inspectable
          ? await dirtyCount(path).catch(() => null)
          : null;
      return {
        path,
        isMain: record === main,
...
export async function worktreeIntegrationStatus(
  ...
  const requestedUpstream = opts.upstreamPath
    ? resolve(opts.upstreamPath)
    : null;
  ...
  const [dirty, upstreamDirty] = await Promise.all([
    dirtyCount(path),
    upstreamPath !== path
      ? dirtyCount(upstreamPath).catch(() => 0)
      : Promise.resolve(0),
  ]);
  const counts =
...
const statusCache = new Map<
  string,
  { at: number; value: Promise<WorktreeIntegrationStatus> }
>();
...
export async function cachedWorktreeIntegrationStatus(
  ...
  const key = opts.upstreamPath
    ? `${resolve(cwd)}\0${resolve(opts.upstreamPath)}`
    : resolve(cwd);
