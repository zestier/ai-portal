...
export function buildToolExtractorSystemPrompt(): string {
	return [
		...
		[
			...
			'  • remember_attributes — things to KNOW: durable current state (a value, status, preference, relationship, ownership, role, location, constraint, deadline, identifier). This is the default; most things are attributes. One call sets MANY attributes on one entity, and is ALSO the entity constructor: pass entityType + displayName (+ optional summary) in the same call to type and name a brand-new referent.',
