...
function screenInjectionFacts(facts: MemoryPatchProposal["facts"]): {
  ...
  if (!facts)
    return { facts: undefined, directivesRemoved: 0, valuesRedacted: 0 };
  ...
  return {
    facts: kept.length > 0 ? kept : undefined,
    directivesRemoved,
    valuesRedacted,
  };
}
...
function canonicalizeEntityKeys(
  ...
  let remapped = 0;
  let merged = 0;
  const canonicalKeyForEntity = (
    entity: NonNullable<MemoryPatchProposal["entities"]>[number],
  ) => {
    const existing =
      aliases.get(normalizedName(entity.entityKey)) ??
      existingByTypedName.get(
        typedNameKey(entity.entityType, entity.displayName),
      ) ??
  ...
  const next: MemoryPatchProposal = {
    events: patch.events?.map((event) => ({
      ...event,
      entityKey: rewriteKey(event.entityKey),
    })),
    facts: patch.facts?.map((fact) => ({
      ...fact,
      entityKey: rewriteKey(fact.entityKey),
    })),
    openLoops: patch.openLoops?.map((loop) => ({
      ...loop,
      relatedEntityKeys: loop.relatedEntityKeys?.map(
        (key) => rewriteKey(key) ?? key,
      ),
    })),
...
function setUniqueAlias(
  ...
  aliases.set(
    alias,
    prior === undefined ? entityKey : prior === entityKey ? prior : null,
  );
}

function typedNameKey(
