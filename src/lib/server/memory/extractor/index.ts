      ...
      parameters: {
        ...
        properties: {
          summary: {
            ...
            maxLength: 1000,
            description:
              "Optional one-line summary of what you recorded this turn.",
          },
  ...
  async extractPatch(input: ExtractPatchInput): Promise<ExtractPatchResult> {
    ...
    for (const tool of readTools)
      handlers.set(tool.name, async (args) =>
        JSON.stringify(await tool.handler(args)),
      );
    ...
    const maxFailedCallNudges =
      this.opts.maxFailedCallNudges ?? DEFAULT_MAX_FAILED_CALL_NUDGES;
    const deadline =
      Date.now() + (this.opts.maxWallClockMs ?? Number.POSITIVE_INFINITY);
    for (
      let iteration = 0;
      iteration < this.opts.maxToolIterations;
      iteration += 1
    ) {
      ...
      const stepToolChoice: "auto" | "required" =
        this.opts.toolChoice === "required" && iteration === 0
          ? "required"
          : "auto";
      ...
      if (!thinkSegmentId && !contentSegmentId) {
        const thoughts = [turn.reasoning?.trim(), think.trim()]
          .filter(Boolean)
          .join("\n\n");
      ...
      const finishCall = turn.toolCalls.find(
        (call) => call.name === FINISH_EXTRACTION_TOOL,
      );
      if ((!hasMoreToolCalls || finishCall) && visible.trim())
        finalContent = visible.trim();
      if (finishCall) {
      ...
      messages.push({
        ...
        content:
          truncate(visible, ASSISTANT_TRANSCRIPT_MAX_CHARS) ||
          (hasMoreToolCalls ? null : ""),
      ...
      for (const call of turn.toolCalls) {
        ...
        if (!activityResultOk(result)) {
          ...
          try {
            ...
            if (
              typeof parsed.clearedFailureId === "string" &&
              parsed.clearedFailureId
            )
    ...
    if (proposeCalls === 0) {
      diagnostics.push({
        ...
        message:
          "The tool-calling extractor stored nothing durable for this turn.",
      });
    }
  ...
  try {
    ...
    if (typeof parsed.ok === "boolean") {
      if (parsed.ok === false) {
        ...
        const detail =
          typeof err.message === "string" ? err.message : `${count} issue(s)`;
        return `not staged — ${detail}`;
      }
      const action =
        typeof parsed.action === "string" ? parsed.action : "staged";
    ...
    if (Array.isArray(parsed.results))
      return `${parsed.results.length} result(s)`;
  } catch {
...
function mergePatchProposals(
  ...
  const keepOpenLoops = [
    ...new Set(patches.flatMap((patch) => patch.keepOpenLoops ?? [])),
  ];
  ...
  const forgetByKey = new Map<
    string,
    NonNullable<MemoryPatchProposal["forgetFacts"]>[number]
  >();
  for (const patch of patches) {
  ...
  if (resolutionById.size > 0)
    merged.resolveOpenLoops = [...resolutionById.values()];
...
async function acquireExtractionLock(
  ...
  for (;;) {
    if (
      memoryRepo.tryAcquireExtractionLock(conversationId, holder, {
        ttlMs: opts.ttlMs,
      })
    ) {
      return true;
...
export async function extractAndCommitMemory(input: ExtractPatchInput): Promise<
  ReturnType<typeof commitPatch> & {
    extraction: ExtractPatchResult;
...
> {
  ...
  const cfg = loadConfig();
  const lockTtlMs =
    cfg.MEMORY_EXTRACTOR_MAX_WALLCLOCK_MS +
    cfg.MEMORY_EXTRACTOR_WATCHDOG_GRACE_MS +
    30_000;
...
> {
  ...
  if (!input.initialPacket) {
    const globalMemoryEnabled =
      conversationsRepo.get(intConv, input.userId)?.globalMemoryEnabled ??
      false;
    const build = () =>
    ...
    input.initialPacket = input.priorPatchId
      ? memoryRepo.readMemoryAtTurnStart(
          intConv,
  ...
  if (
    ...
    const keptLoopIds = [
      ...
      ...(extraction.patch.resolveOpenLoops ?? []).map(
