...
export function validatePatch(
  ...
  if (opts.conversationId) {
    ...
    for (const resolution of patch.resolveOpenLoops ?? []) {
      ...
      const loopId = memoryRepo.resolveOpenLoopId(
        opts.conversationId,
        resolution.id,
      );
      const existing = loopId
        ? memoryRepo.getOpenLoop(opts.conversationId, loopId)
        : null;
  ...
  if (
    (opts.mode === "story" || opts.mode === "strict") &&
    opts.conversationId
  ) {
    for (const fact of patch.facts ?? []) {
      if (
      ...
      const existing = memoryRepo
        ...
        .find(
          (row) => JSON.stringify(row.value) !== JSON.stringify(fact.value),
        );
      if (existing) {
        issues.push({
  ...
  if (opts.mode === "strict") {
    for (const fact of patch.facts ?? []) {
      ...
      if (
        fact.predicate === "clue" &&
        !hasObjectStringFields(fact.value, ["id", "status"])
      ) {
        issues.push({
          severity: "error",
          code: "strict_clue_shape_invalid",
          message:
            "Strict mode clue facts must store an object with string id and status fields.",
        });
      }
      if (
        isSecretPredicate(fact.predicate) &&
        !isHiddenVisibility(fact.visibility)
      ) {
        issues.push({
          severity: "error",
    ...
    for (const event of patch.events ?? []) {
      if (
        ["timeline", "alibi", "clue_revealed"].includes(event.eventType) &&
        !event.entityKey
      ) {
        issues.push({
          severity: "warning",
...
export function commitPatch(
  ...
  return getDb().transaction(() => {
    ...
    for (const entity of input.patch.entities ?? []) {
      ...
      if (
        !existing &&
        (entity.entityType === undefined || entity.displayName === undefined)
      ) {
    ...
    for (const event of input.patch.events ?? []) {
      ...
      const row = memoryRepo.addEvent(intConv, {
        ...
        sourceMessageId: intSourceMessageId ?? null,
        targetEntityId: event.entityKey
          ? (entityIdsByKey.get(event.entityKey) ?? null)
          : null,
      });
      memoryRepo.recordPatchItem(intConv, {
        patchId: patchRecord.id,
    ...
    const ensureSessionEntity = (): string => {
      ...
      const row = memoryRepo.upsertEntity(intConv, {
        ...
        displayName: "Session",
        summary:
          "Catch-all for session-scoped facts not tied to a specific entity.",
        sourceMessageId: intSourceMessageId ?? null,
        turnId: input.turnId ?? null,
      });
    ...
    for (const fact of input.patch.facts ?? []) {
      const entityId = fact.entityKey
        ? ensureEntityForKey(fact.entityKey)
        : ensureSessionEntity();
    ...
    for (const resolution of input.patch.resolveOpenLoops ?? []) {
      ...
      if (existing.status !== "open" && existing.status === resolution.status)
        continue;
...
function summarizePatch(patch: MemoryPatchProposal): string {
  return [
    ...
    patch.resolveOpenLoops?.length
      ? `${patch.resolveOpenLoops.length} resolved loops`
      : "",
    patch.forgetFacts?.length
      ? `${patch.forgetFacts.length} forgotten facts`
      : "",
  ]
    .filter(Boolean)
    .join(", ");
...
function collectEntityKeys(patch: MemoryPatchProposal): Set<string> {
  ...
  for (const event of patch.events ?? [])
    if (event.entityKey) keys.add(event.entityKey);
  for (const fact of patch.facts ?? [])
    if (fact.entityKey) keys.add(fact.entityKey);
  for (const loop of patch.openLoops ?? []) {
