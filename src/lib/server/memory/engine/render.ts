...
export function entityLine(
  entity: memoryRepo.MemoryEntity,
  includeId = false,
): string {
  const status =
    entity.status && entity.status !== "active" ? ` [${entity.status}]` : "";
...
export function entityIndexLine(
  entry: MemoryEntityIndexEntry,
  includeId = false,
): string {
  const status =
    entry.status && entry.status !== "active" ? ` [${entry.status}]` : "";
...
export function factDetail(
  ...
  if (fact.visibility && fact.visibility !== "session")
    meta.push(fact.visibility);
...
export function loopLine(
  ...
  opts: {
    ...
    expiry?:
      { baseThreshold: number; warnWithin?: number | undefined } | undefined;
  } = {},
): string {
  ...
  const status =
    loop.status && loop.status !== "open" ? ` [${loop.status}]` : "";
  ...
  if (opts.expiry && opts.expiry.baseThreshold > 0 && loop.status === "open") {
    ...
    const effectiveThreshold =
      opts.expiry.baseThreshold + Math.max(0, loop.priority);
...
export function eventLine(
  ...
  const meta: string[] = [];
  if (event.visibility && event.visibility !== "session")
...
export function renderMemoryPacket(
  ...
  const entityById = new Map(
    packet.entities.map((entity) => [entity.id, entity]),
  );
  const indexById = new Map(
    packet.entityIndex.map((entry) => [entry.entityId, entry]),
  );
  ...
  const shown = (
    type: "entity" | "fact" | "event" | "open_loop",
    id: string | number,
  ): string => {
    const key = `${type}:${id}`;
    shownIds.add(key);
  ...
  if (remainingIndex.length) {
    lines.push(
      "",
      `also on record (${remainingIndex.length}) — queryable by name:`,
    );
