 ...
 */
export function deriveEntityFromKey(key: string): {
  entityType: string;
  displayName: string;
} {
...
export function buildInitialPacket(
  ...
  for (const directive of loadedDirectives) {
    ...
    if (
      budgetedDirectives.length &&
      directiveTokens + cost > DIRECTIVE_TOKEN_BUDGET
    )
      break;
  ...
  const factPool = allFacts.filter(
    (fact) => !isDirectivePredicate(fact.predicate),
  );
  const eventPool = memoryRepo.listEvents(intConv, {
    limit: strict ? 200 : 100,
  });
  const openLoops = memoryRepo.listOpenLoops(intConv, {
    limit: strict ? 80 : 40,
  });
  ...
  const searchHits = query
    ? memoryRepo.search(intConv, {
        query,
        limit: Math.max(AUTO_SEARCH_LIMIT, 300),
      })
    : [];
  ...
  for (const hit of searchHits) {
    scores.set(
      hit.itemId,
      Math.max(scores.get(hit.itemId) ?? 0, hit.score ?? 0),
    );
  }
  ...
  return {
    ...
    toolGuidance: {
      ...
      availableTools: [
        ...
        "memory_check_claims",
        ...(opts.globalMemoryEnabled
          ? ["memory_global_record", "memory_global_search"]
          : []),
      ],
      recallTriggers: [
...
export function buildPromptWithMemory(params: {
  ...
  const query = [params.userMsg.content, recent]
    .filter(Boolean)
    .join("\n")
    .trim();
  ...
  const hasGlobalMemory =
    ...
    (params.userId
      ? memoryRepo.listGlobalMemories(params.userId, { limit: 1 }).length > 0
      : true);
