...
export function createCustomProfile(
  ...
  return rowToCustomProfile(
    getDb()
      .prepare("SELECT * FROM memory_custom_profiles WHERE id = ?")
      .get(id) as CustomProfileRow,
  );
}
...
export function updateCustomProfile(
  ...
  return rowToCustomProfile(
    getDb()
      .prepare(
        "SELECT * FROM memory_custom_profiles WHERE id = ? AND user_id = ?",
      )
