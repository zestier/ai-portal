...
function extractorContextSections(input: ExtractPatchInput): string[] {
	return [
		...
		'Capture per-session directives (standing rules) as facts with predicate "directive" and the instruction text as a string value. A directive is a durable, forward-looking behavioral instruction the user gives for how you should act for the rest of the session ("always do X", "from now on Y", "never Z", e.g. "when creating new characters, give them names"). Only record genuinely standing, forward-looking instructions this way — not one-off requests for the current turn. Directives are additive and stay in effect until the user explicitly retires them: emit one only for a genuinely new standing rule, and re-emit the exact same text only when the user reaffirms it (identical values are de-duplicated automatically). Two directives with different wording both remain active, so when the user replaces one standing rule with a contradictory one, phrase the new directive to fully restate the current rule (the prior wording is not auto-removed and is retired out-of-band via the memory inspector).',
