 ...
 */
export function conversationWorkspaceRoot(
...
function stillContained(rootReal: string, abs: string): boolean {
  ...
  return (
    resolved === rootReal || (!inside.startsWith("..") && !isAbsolute(inside))
  );
}
 ...
 */
export function resolveContainedToolPath(
  root: string,
  p: string,
): string | null {
...
export function listDir(
  ...
  try {
    ...
    if (!st.isDirectory())
      return { ok: false, reason: "not a directory", status: 400 };
