...
export function list(userId: number, opts: extensionsRepo.ListOptions = {}): PortalExtension[] {
...
export function enabledEntries(userId: number): PortalExtension[] {
	return extensionsRepo.list(userId, { status: 'open' }).filter((e) => e.enabled);
}
 ...
 */
export function validateExtensionValue(kind: PortalExtensionKind, value: string): string | null {
	const v = value.trim();
	if (!v) return 'Extension value cannot be empty.';
	if (kind === 'file') {
		if (v.length > 4096) return 'File extension paths must be 4096 characters or fewer.';
		return null;
	}
	if (kind === 'inline') {
		if (v.length > 200_000) return 'Inline extension source must be 200,000 characters or fewer.';
		return null;
	}
	if (kind === 'package') {
		if (v.length > 500) return 'Package specs must be 500 characters or fewer.';
		const pinned = /^npm:[^\s]+@[^\s]+$/.test(v) || /^git:[^\s]+@[^\s]+$/.test(v);
		if (!pinned) {
...
export async function enabledExtensionPaths(userId: number): Promise<string[]> {
	...
	for (const e of enabledEntries(userId)) {
		if (e.kind === 'file') {
			out.push(isAbsolute(e.value) ? e.value : resolve(cfg.PROJECT_ROOT, e.value));
...
export async function fingerprint(userId: number): Promise<string> {
	...
	const body =
		...
		entries.map((e) => `${e.id}\u0000${e.kind}\u0000${e.value}\u0000${e.updatedAt}`).join('\n');
 ...
 */
export async function verify(userId: number, id?: string): Promise<VerifyResult> {
	try {
		let paths: string[];
		if (id !== undefined) {
			const e = extensionsRepo.get(userId, id);
			if (!e) return { loaded: [], errors: [{ path: id, error: `Extension not found: ${id}` }] };
			await materializeInlineFiles(userId);
