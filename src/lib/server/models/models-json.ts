...
export function modelsJsonPath(cfg: ReturnType<typeof loadConfig> = loadConfig()): string {
...
export function modelToJson(m: ManagedModel): PiModelJson {
	...
	if (m.input && (m.input.length !== 1 || m.input[0] !== 'text')) out.input = m.input;
...
export function serializeCatalog(
	...
	for (const p of providers) {
		...
		if (p.builtin && !p.baseUrl && !hasHeaders && !p.authHeader && pmodels.length === 0) continue;
...
export function writeModelsJsonFile(cfg: ReturnType<typeof loadConfig> = loadConfig()): string {
