...
async function getJson(url: string, headers: Record<string, string>): Promise<unknown> {
	...
	if (!res.ok) {
		...
		throw new Error(`Catalog request to ${url} failed (${res.status}): ${body}`);
	}
	return res.json();
}
...
async function fetchAnthropic(baseUrl: string, key: string): Promise<FetchCatalogResult> {
	...
	return { models, source: `${new URL(endpoint(baseUrl, 'v1/models')).host}/v1/models` };
}
...
async function fetchOpenRouter(baseUrl: string, key: string): Promise<FetchCatalogResult> {
	...
	return { models, source: `${new URL(endpoint(baseUrl, 'models')).host}/models` };
}

/** Generic OpenAI-compatible /models: ids only (Ollama, vLLM, gateways, OpenAI). */
async function fetchOpenAiCompatible(baseUrl: string, key: string): Promise<FetchCatalogResult> {
	const json = (await getJson(endpoint(baseUrl, 'models'), {
		authorization: `Bearer ${key}`
...
async function fetchGoogle(baseUrl: string, key: string): Promise<FetchCatalogResult> {
 ...
 */
export async function fetchProviderCatalog(provider: ManagedProvider): Promise<FetchCatalogResult> {
	const key = providersRepo.getApiKey(provider.id);
	if (!key) {
		throw new Error(`No API key stored for provider "${provider.id}" — save one first.`);
	}
	const baseUrl = provider.baseUrl ?? DEFAULT_BASE_URLS[provider.id];
	if (!baseUrl) {
		throw new Error(`No base URL for provider "${provider.id}" — set one to fetch its catalog.`);
	}
	switch (provider.api) {
