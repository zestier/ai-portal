...
function renderBashText(opts: {
  ...
  if (opts.persisted) {
    ...
    if (genuineExit && opts.exitCode !== 0)
...
// a complete line).
function persistedOutputBlock(p: {
  path: string;
  size: number;
  preview: string;
}): string {
...
function runShell(
  ...
  return new Promise((resolveResult) => {
    ...
    child.on(
      "error",
      (error) =>
        void finish(err(error.message, { code: "shell_spawn_failed" })),
    );
    child.on("close", (code, signal) => {
      const exitCode = exitCodeFor(code, signal);
      const interrupted =
        aborted || (code === null && signal !== null && !timedOut);
      const payload = {
        command,
        cwd,
        stdout,
        ...
        ...(spill && spill.bytes > 0
          ? {
              persistedOutputPath: spill.path,
              persistedOutputSize: spill.bytes,
            }
          : {}),
        ...(exitCode === 127
        ...
        ...(exitCode === 126
          ? {
              returnCodeInterpretation:
                "Permission denied or command is not executable",
            }
          : {}),
      };
      ...
      const genuineFailure =
        !timedOut && !aborted && !interrupted && exitCode !== 0;
  ...
  return [
    {
      name: "bash",
      description:
        "Run a non-interactive Bash command in the workspace. Foreground only.",
      promptGuidelines: [
      ...
      parameters: {
        ...
        properties: {
          ...
          cwd: {
            type: "string",
            description: "Optional cwd (workspace-relative).",
          },
          maxOutputBytes: {
            type: "number",
