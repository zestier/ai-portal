  ...
  return [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          body: {
            type: "string",
            description: "Optional details, notes, or acceptance criteria.",
          },
          plan: {
            type: "string",
        ...
        try {
          const ticket = tickets.create(opts.userId, {
            ...
            ...(parsed.blocks !== undefined
              ? { blocks: parsed.blocks.map(parseEdgeId) }
              : {}),
    ...
    },
    {
      name: "ticket_list",
      description:
        "List durable workspace tickets for the current workspace (defaults to open).",
      promptGuidelines: [
      ...
      async handler(args) {
        ...
        if (fields !== undefined) {
          ...
          return ok(
            withOmitted({ tickets: projected.value }, projected.omitted),
            summary,
          );
        }
        const rendered = rows
    ...
    },
    {
      name: "ticket_update",
      description:
        "Update a ticket's title, body, plan, priority, status, or blocking edges.",
      promptGuidelines: [
        ...
        try {
          const updated = tickets.update(ticketId.parse(id), opts.userId, {
            ...
            ...(patch.blocks !== undefined
              ? { blocks: patch.blocks.map(ticketId.parse) }
              : {}),
          } as UpdateInput);
