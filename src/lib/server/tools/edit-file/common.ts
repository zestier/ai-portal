...
export async function readExisting(abs: string): Promise<string | null> {
  ...
  if (content.includes("\0"))
    throw new Error("file contains null bytes and is likely binary");
  return content;
}
...
export function planNumberingTabEating(
  oldString: string,
): TabEatingPlan | null {
  ...
  return {
    ...
    oldTabs,
    normalized: lines
      .map((line, i) => (ate[i] ? line!.slice(1) : line))
      .join("\n"),
  };
}
...
function buildSnippet(window: readonly string[]): string {
  const lines =
    window.length > MAX_SUGGEST_LINES
      ? window.slice(0, MAX_SUGGEST_LINES)
      : window;
...
export function findClosestMatch(
  ...
  for (let i = 0; i + oldLineCount <= fileLines.length; i++) {
    ...
    const cap = Math.min(
      Math.floor(maxLen * (1 - MIN_SUGGEST_SIMILARITY)),
