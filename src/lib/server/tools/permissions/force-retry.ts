  ...
  .superRefine((val, ctx) => {
    ...
    if (!/^[0-9a-f]{24}$/.test(val.token)) {
      ctx.addIssue({
        ...
        path: ["token"],
        message:
          "token must be the 24-hex-character token from the denial feedback",
      });
    }
  });
...
export function buildForceRetryTool(opts: ForceRetryToolOpts): PortalTool {
  return {
    ...
    description:
      "Escalate ONE previously denied tool call to a fresh human approval prompt.",
    promptGuidelines: [
    ...
    async handler(args, ctx) {
      ...
      const view: InteractiveRequestView = {
        ...
        ...(entry.deniedFeedback !== null
          ? { defaultDenyFeedback: entry.deniedFeedback }
          : {}),
      };
      ...
      if (
        ...
        const portalTool = opts.resolvePortalTool
          ? opts.resolvePortalTool(entry.tool)
          : null;
        if (portalTool) {
          if (entry.args === null) {
