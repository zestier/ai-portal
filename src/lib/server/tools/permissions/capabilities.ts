...
export type CapabilityStatus =
  "allowed" | "denied" | "prompt_required" | "partially_allowed";
...
export function buildCapabilitiesTool(opts: CapabilitiesToolOpts): PortalTool {
  return {
    ...
    promptGuidelines: [
      "Use after a rejection to find allowed alternatives before escalating.",
    ],
  ...
  const grants = settings
    ...
    .filter(
      (g) =>
        ...
        convCodec.parse(g.conversationId) === opts.conversationId,
    )
    ...
    .filter(
      (g) =>
        !opts.permissionKind ||
        grantCoversPermissionKind(g, opts.permissionKind),
    );
...
export function capabilityForKind(
  ...
  const relevant = grants.filter((g) =>
    grantCoversPermissionKind(g, permissionKind),
  );
  ...
  const promptRequired = relevant
    .filter((g) => g.decision === "prompt")
    .map(grantToRule);
...
function capabilityStatus(
  ...
  if (allowed.length > 0 && promptRequired.length > 0)
    return "partially_allowed";
...
export function grantCoversPermissionKind(
  ...
  if (permissionKind === "custom-tool" && g.permissionKind === "custom-tool")
    return true;
...
export function grantToRule(g: settings.GrantSummary): CapabilityRule {
  return {
    ...
    decision: g.decision,
    scope:
      g.conversationId === null ? "all-conversations" : "current-conversation",
...
export function grantSummary(g: settings.GrantSummary): string {
  if (g.argsHash)
    return `${decisionVerb(g)} a previously approved exact ${g.tool} invocation.`;
  if (!g.scope)
    return `${decisionVerb(g)} ${g.tool} requests covered by a legacy grant.`;
