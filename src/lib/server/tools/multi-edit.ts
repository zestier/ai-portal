	...
	return [
		{
			...
			async handler(args) {
				...
				for (let i = 0; i < parsed.edits.length; i++) {
					...
					const hunks = structuredPatch('a', 'b', current, applied.content, '', '', {
						context: 3
					}).hunks;
					...
					if (applied.lenient !== undefined) output.lenientTabEating = applied.lenient;
					if (gitRepo) {
						output.gitDiff = gitDiffFor('update', target.rel, current, applied.content, hunks);
					}
					outputs.push(output);
				}
				...
				return ok({ edits: outputs, summary }, summary, {
					views: [{ type: 'text', text: multiEditConfirmation(parsed.edits.length, fileCount) }]
				});
			}
		}
	];
