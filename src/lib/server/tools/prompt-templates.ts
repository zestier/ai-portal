...
const ToolGroupId = z.enum(
  PORTAL_TOOL_GROUP_IDS as unknown as [string, ...string[]],
);
const WorkspaceMode = z.enum(["shared", "worktree"]);

const WORKSPACE_MODE_DESCRIPTION =
  '"shared" | "worktree" for launched chats. Omit/null = shared.';
...
export function buildPromptTemplateTools(opts: {
  userId: number;
}): PortalTool[] {
  return [
    {
      name: "template_list",
      description:
        "List the user\u2019s stored prompt templates (chat + ticket-action).",
      argsSchema: ListArgs,
    ...
    },
    {
      ...
      promptGuidelines: [
        "Validates `{{placeholders}}` by type and rejects unknown ones. " +
          placeholderHint(),
      ],
      ...
      parameters: {
        ...
        properties: {
          ...
          systemPrompt: {
            type: "string",
            description:
              "Optional Markdown; replaces the default system prompt (persona).",
          },
          appendSystemPrompt: {
            type: "string",
            description:
              "Optional Markdown; appended under the active system prompt.",
          },
          launchBehavior: {
            type: "string",
          ...
          pinned: {
            type: "boolean",
            description: "Pin to the top of its list.",
          },
        },
        ...
        try {
          const tpl = templates.create(opts.userId, {
            ...
            ...(p.appendSystemPrompt !== undefined
              ...
              : {}),
            ...(p.launchBehavior !== undefined
              ? { launchBehavior: p.launchBehavior }
              : {}),
            ...(p.conversationMode !== undefined
          ...
          return ok(
            summarize(tpl),
            `Created ${tpl.type} template ${tpl.id}: ${tpl.title}`,
          );
        } catch (e) {
    ...
    },
    {
      ...
      async handler(args) {
        ...
        if (!templates.get(id, opts.userId))
          return err(`Template not found: ${id}`);
        try {
          const updated = templates.update(
            id,
            opts.userId,
            patch as templates.UpdateInput,
          );
          if (!updated) return err(`Template not found: ${id}`);
          return ok(
            summarize(updated),
            `Updated template ${updated.id}: ${updated.title}`,
          );
        } catch (e) {
          return err(e instanceof Error ? e.message : String(e));
        }
