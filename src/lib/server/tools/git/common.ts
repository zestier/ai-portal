...
export const DiffOutput = z.enum([
  "patch",
  "stat",
  "numstat",
  "name-only",
...
export function mergeStatusFollowUpHint(
  ...
  if (merge.sequencer)
    return { followUpHint: sequencerFollowUpHint(merge.sequencer, leaseId) };
  if (merge.inProgress)
    return { followUpHint: mergeInProgressFollowUpHint(leaseId) };
