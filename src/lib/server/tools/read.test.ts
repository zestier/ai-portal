...
describe("read tool", () => {
  ...
  it("defaults a range-less read of a large file to an outline", async () => {
    ...
    const file = result.result as {
      file: { outlined?: boolean; content: string };
    };
  ...
  it("paginates a huge page with the continuation banner", async () => {
    ...
    expect(
      (result.result as { file: { truncatedByTokenCap?: boolean } }).file
        .truncatedByTokenCap,
    ).toBe(true);
  });
  ...
  it("returns an outline for a medium file too (not size-gated to huge files)", async () => {
    ...
    if (!result.ok) throw new Error(result.error.message);
    const file = result.result as {
      file: { outlined?: boolean; totalLines: number };
    };
    expect(file.file.outlined).toBe(true);
  ...
  it("mode outline forces an outline even when auto would return raw", async () => {
    ...
    if (!result.ok) throw new Error(result.error.message);
    expect(
      (result.result as { file: { outlined?: boolean } }).file.outlined,
    ).toBe(true);
  });
  ...
  it("a broad re-read of an unchanged file returns a short unchanged marker", async () => {
    ...
    for (let i = 0; i < 25; i++) lines.push(`def g${i}():`, `    return ${i}`);
    writeFileSync(join(root, "med.py"), lines.join("\n"));
    const first = await readTool(root).handler({
      file_path: "med.py",
      offset: 1,
      limit: 100,
    });
    ...
    const second = await readTool(root).handler({
      file_path: "med.py",
      offset: 1,
      limit: 100,
    });
  ...
  it("a broad re-read after a change returns only the delta hunks", async () => {
    ...
    writeFileSync(
      path,
      lines.map((l, i) => (i === 1 ? "    return 99" : l)).join("\n"),
    );
    const second = await readTool(root).handler({
      file_path: "med.py",
      offset: 1,
  ...
  it("targeted range re-reads stay raw (not swallowed by the delta)", async () => {
    ...
    const drill = await readTool(root).handler({
      file_path: "med.py",
      offset: 1,
      limit: 3,
    }); // targeted
