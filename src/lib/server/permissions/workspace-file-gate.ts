...
export function checkWorkspaceFileGate(opts: WorkspaceFileGateOptions): void {
  ...
  const state = settingsRepo.getWorkspacePermissionState(
    opts.userId,
    workspaceRoot,
  );
  ...
  const view: InteractiveRequestViewBody = {
    ...
    activeGrantCount: settingsRepo.countWorkspaceFileGrants(
      opts.userId,
      workspaceRoot,
    ),
...
export function getWorkspaceFileStatus(
  ...
  const diff = accepted
    ? null
    : capDiff(buildDiff(state?.snapshotText ?? null, file?.text ?? ""));
  return {
    fileName: `.zap/${WORKSPACE_PERMISSIONS_FILE}`,
    workspaceRoot: root,
    present: file !== null,
