...
export function shellRuleMatches(
  ...
  for (let i = 0; i < segments.length; i++) {
    const inPipeline =
      segments[i].followingOp === "|" ||
      (i > 0 && segments[i - 1].followingOp === "|");
    const isPipeTarget = i > 0 && segments[i - 1].followingOp === "|";
    if (
      !shellRuleMatchesSegment(rule, segments[i], {
        ...ctx,
        inPipeline,
        isPipeTarget,
      })
...
export function shellRuleMatchesSegment(
  ...
  if (rule.pipeline) {
    ...
    if (rule.pipeline === "pipe-target" && ctx.isPipeTarget !== true)
      return false;
  }
...
function commandPathMatches(
  ...
  for (let stepIndex = 0; stepIndex < path.length - 1; stepIndex++) {
    ...
    for (const optionIndex of matched.ignoredOptionIndexes)
      ignored.add(optionIndex);
  ...
  for (let i = bodyStartIndex; i < argv.length; i++) {
    ...
    if (looksLikeShellOptionToken(tok)) {
      ...
      if (finalOptions?.allow) {
        const matched = matchShellOptionToken(
          tok,
          argv[i + 1],
          finalOptions.allow,
        );
        if (!matched) return false;
        if (!optionSpecMatchesValue(matched.spec, matched.value, ctx))
          return false;
 ...
 */
function positionalCountMatches(
  rule: PositionalCountRule | undefined,
  count: number,
): boolean {
  if (!rule) return true;
