...
export const RISKY_GIT_GLOBAL_PATTERNS = RISKY_GIT_GLOBAL_OPTIONS.flatMap(
  (option) => {
...
export const GIT_STRUCTURED_SUBCOMMAND_DENIES: {
  subcommand: string;
  tools: string;
}[] = [
...
export function gitStructuredSubcommandFeedback(
  subcommand: string,
  tools: string,
): string {
