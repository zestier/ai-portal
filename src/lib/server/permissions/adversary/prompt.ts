...
export function buildAdversaryFacts(
  input: BuildAdversaryFactsInput,
): AdversaryFacts {
  ...
  return {
    ...
    fsTargets: (input.fsTargets ?? []).filter(
...
export function buildAdversaryPrompt(
  ...
  maxArgChars = DEFAULT_MAX_ARG_CHARS,
) {
  ...
  const lines: string[] = [];
  lines.push(
    "PERMISSION REQUEST (portal-derived facts; every value is JSON-quoted):",
  );
  lines.push(`tool: ${q(facts.tool)}`);
  lines.push(`permissionKind: ${q(facts.permissionKind)}`);
  lines.push(
    `scopeKey: ${facts.scopeKey === null ? "(none)" : q(facts.scopeKey)}`,
  );
  lines.push(
  ...
  if (facts.shell?.kind === "parsed" && facts.shell.segments) {
    ...
    for (const segment of facts.shell.segments) {
      const op = segment.followingOp
        ? ` [followed by ${q(segment.followingOp)}]`
        : "";
      lines.push(
        `  - ${escapeLineSeparators(JSON.stringify(segment.argv))}${op}`,
      );
    }
