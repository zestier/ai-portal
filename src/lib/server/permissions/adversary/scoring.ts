 ...
 */
export type HumanPermissionDecision = 'allow-once' | 'allow-always' | 'deny' | 'deny-always' | null;
...
export function scoreShadowDecisions(
	...
	const scored = truePositives + falsePositives + falseNegatives + trueNegatives;
	...
	const denyF1 =
		denyPrecision === null || denyRecall === null || denyPrecision + denyRecall === 0
			? null
	...
	return {
		...
		coverage: ratio(scored, rows.length - memoizedDuplicates - unlabellableRows)
	};
}
