...
export type ParseWorkspaceGrantFileResult =
	| { ok: true; grants: WorkspaceFileGrant[] }
	| { ok: false; error: string };

const KNOWN_SECTIONS = new Set(['shell', 'path', 'url', 'tool']);
const DECISIONS = new Set(['allow', 'deny', 'prompt']);

export function parseWorkspaceGrantFile(text: string): ParseWorkspaceGrantFileResult {
	let root: unknown;
	try {
		root = parse(text);
	} catch (e) {
	...
	if (root === null || typeof root !== 'object' || Array.isArray(root)) {
		return {
			ok: false,
			error: 'invalid TOML: expected a document with [[shell]]/[[path]]/[[url]]/[[tool]] sections'
		};
	}
	...
	for (const kind of ['shell', 'path', 'url', 'tool'] as const) {
		...
		if (!Array.isArray(raw)) {
			return { ok: false, error: `section [${kind}] must be an array of tables (use [[${kind}]])` };
		}
...
type SectionResult = { ok: true; grant: WorkspaceFileGrant } | { ok: false; error: string };
...
function parseSection(
	...
	if (decisionRaw === null) {
		if (e['decision'] !== undefined) return fail(index, 'decision must be a string');
	} else if (!DECISIONS.has(decisionRaw)) {
		return fail(index, `decision must be one of allow | deny | prompt, got "${decisionRaw}"`);
	}
...
function parseShell(
	...
	if (hasMin || hasMax) {
		...
		if (hasMin) {
			if (typeof e['positional_min'] !== 'number' || !Number.isInteger(e['positional_min'])) {
				return fail(index, 'shell.positional_min must be a non-negative integer');
			}
			count.min = e['positional_min'] as number;
		}
		if (hasMax) {
			if (typeof e['positional_max'] !== 'number' || !Number.isInteger(e['positional_max'])) {
				return fail(index, 'shell.positional_max must be a non-negative integer');
			}
	...
	if (optionsRaw !== undefined) {
		...
		for (let i = 0; i < optionsRaw.length; i++) {
			...
			if (allowRaw !== undefined) {
				if (!Array.isArray(allowRaw) || allowRaw.some((s) => typeof s !== 'string')) {
					return fail(index, `shell.options[${i}].allow must be an array of strings`);
				}
				...
				for (const s of allowRaw as string[]) {
					try {
						...
						return fail(index, `shell.options[${i}].allow entry "${s}": ${(err as Error).message}`);
					}
				}
			...
			if (denyRaw !== undefined) {
				if (!Array.isArray(denyRaw) || denyRaw.some((s) => typeof s !== 'string')) {
					return fail(index, `shell.options[${i}].deny must be an array of strings`);
				}
				if ((denyRaw as string[]).length > 0) rule.deny = denyRaw as string[];
			}

			if (rule.allow === undefined && rule.deny === undefined) {
				return fail(index, `shell.options[${i}] must set at least one of allow/deny`);
			}
...
function parsePath(
	...
	if (root === null) {
		return fail(index, 'path.root is required (workspace, session-workspace, or absolute)');
	}
	const behavior = strField(e, 'behavior');
	if (behavior === null) {
		return fail(index, 'path.behavior is required (any, exact, prefix, or glob)');
	}
...
function parseUrl(
	...
	if (ruleKind !== 'exact' && ruleKind !== 'host' && ruleKind !== 'host-suffix') {
		return fail(index, `url.rule must be one of exact | host | host-suffix, got "${ruleKind}"`);
	}
	const value = strField(e, 'value');
	if (value === null) {
...
function parseTool(
	...
	if (name === null) {
		return fail(index, 'tool.name is required (the portal tool name, e.g. "git_commit")');
	}
	return validateInput(index, {
		tool: 'custom-tool',
		toolName: name,
