// Image detection + capture for the `view`-tool attachment feature.
//
// The native SDK `view` tool's completion event carries no image bytes, so the
// portal captures them itself at permission (read) time. This module decides
// whether a path is a capturable image and reads its bytes under a size cap.
//
...
// not a raster image we want to inline. The helpers that take a buffer are pure
// so they can be unit-tested without touching the filesystem.

import { statSync, readFileSync } from 'node:fs';
import { extname } from 'node:path';

// Hard ceiling on a single captured attachment. Larger images are skipped
// gracefully (no capture, no row) rather than bloating the DB.
...
// and rendered in the card via the authed endpoint, but get no inline preview.
...
/**
 * Decide the mime for a path given its extension and head bytes. Both must
 * agree on it being an allowlisted image: the extension gates the lookup, and
 * the magic bytes must sniff to a supported format. Returns null otherwise.
 */
 ...
 */
export function captureImageAttachment(absPath: string): CapturedImage | null {
	try {
