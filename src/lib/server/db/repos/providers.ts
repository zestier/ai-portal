...
function rowToProvider(r: ProviderRow): ManagedProvider {
	return {
		id: r.id,
		name: r.name,
		api: VALID_APIS.includes(r.api as ProviderApi) ? (r.api as ProviderApi) : 'openai-completions',
		baseUrl: r.base_url,
...
export function clearApiKey(id: string): void {
	getDb()
		.prepare('UPDATE providers SET api_key_ct = NULL, updated_at = ? WHERE id = ?')
		.run(Date.now(), id);
}

/** Decrypted key for a provider, or null when none is stored. */
export function getApiKey(id: string): string | null {
	const r = getDb().prepare('SELECT api_key_ct FROM providers WHERE id = ?').get(id) as
		| { api_key_ct: Buffer | null }
		| undefined;
...
export function listWithKeys(): { id: string }[] {
	return getDb()
		.prepare('SELECT id FROM providers WHERE api_key_ct IS NOT NULL ORDER BY id')
		.all() as { id: string }[];
}
