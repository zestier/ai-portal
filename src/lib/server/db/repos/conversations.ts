...
function rowToConv(r: ConvRow): Conversation {
  ...
  return {
    ...
    forkedFromMessageId:
      r.forked_from_message_id === null
        ? null
        : messageId.encode(r.forked_from_message_id),
...
export function get(id: string | number, userId: number): Conversation | null {
  ...
  const r = getDb()
    .prepare(
      `${CONVERSATION_SELECT} WHERE conversations.id = ? AND conversations.user_id = ?`,
    )
  ...
  db.transaction(() => {
    ...
    if (workspaceKind === "managed-worktree") {
      if (!input.managedWorktree) {
        throw new Error(
          "managed-worktree conversation requires managedWorktree metadata",
        );
      }
 ...
 */
export function setManagedWorktree(
  id: string | number,
...
export function renameIfDefault(
  id: string | number,
  userId: number,
  title: string,
): boolean {
...
export function updateSessionSettings(
  ...
  if (patch.disabledToolGroups !== undefined) {
    ...
    args.push(
      JSON.stringify(sanitizeDisabledToolGroups(patch.disabledToolGroups)),
    );
  }
...
export function setSessionFile(
  ...
  getDb()
    .prepare(
      "UPDATE conversations SET session_file = ? WHERE id = ? AND user_id = ?",
    )
...
export function clearDraftPrompt(id: string | number) {
  getDb()
    .prepare("UPDATE conversations SET draft_prompt = NULL WHERE id = ?")
    .run(convInt(id));
}
