...
function rowToTemplate(row: PromptTemplateRow): ChatPromptTemplate {
  ...
  return {
    ...
    approvalMode: row.approval_mode
      ? normalizeApprovalMode(row.approval_mode)
      : null,
...
export function create(userId: number, input: CreateInput): ChatPromptTemplate {
  ...
  const workspaceMode = normalizePromptTemplateWorkspaceMode(
    input.workspaceMode,
  );
  const now = Date.now();
  const orderIndex = Number.isFinite(input.orderIndex)
    ? Math.trunc(input.orderIndex ?? 0)
    : 0;
  const id = Number(
    getDb()
      .prepare(
...
export function update(
  ...
  if (title !== undefined && !title)
    throw new Error("prompt template title cannot be empty");
  if (prompt !== undefined && !prompt)
    throw new Error("prompt template body cannot be empty");
  if (prompt !== undefined) assertPlaceholders(prompt, current.type);
  const nextStatus = patch.status ?? current.status;
  const now = Date.now();
  const archivedAt =
    nextStatus === "archived" ? (current.archivedAt ?? now) : null;
  const orderIndex =
