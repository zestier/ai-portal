...
function rowToModel(r: ModelRow): ManagedModel {
  return {
    ...
    thinkingLevelMap: parseJsonRecord(r.thinking_map_json) as Record<
      string,
      string | null
    > | null,
...
export function list(providerId?: string): ManagedModel[] {
  const rows = (
    providerId
      ? getDb()
          .prepare(
            "SELECT * FROM models WHERE provider_id = ? ORDER BY sort_order ASC, id ASC",
          )
          .all(providerId)
      : getDb()
...
export function upsert(providerId: string, input: ModelInput): ManagedModel {
  ...
  const sampling =
    input.samplingParams !== undefined
      ? input.samplingParams
      : (existing?.samplingParams ?? null);
...
export function remove(providerId: string, id: string): void {
  getDb()
    .prepare("DELETE FROM models WHERE provider_id = ? AND id = ?")
    .run(providerId, id);
}
