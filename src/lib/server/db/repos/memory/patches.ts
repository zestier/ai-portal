...
export function createPatch(
  ...
  const row = getDb()
    .prepare("SELECT * FROM memory_patches WHERE id = ?")
    .get(id) as PatchRow;
...
export function updatePatchStatus(
  ...
  const row = getDb()
    .prepare(
      "SELECT * FROM memory_patches WHERE id = ? AND conversation_id = ?",
    )
...
export function recordPatchItem(
  ...
  const now = Date.now();
  const itemId =
    resolveId(input.itemId, codecFor(input.itemType) ?? memoryPatchItemId) ?? 0;
  const info = getDb()
    .prepare(
...
export function reviewPatchItem(
  ...
  const current = getDb()
    .prepare(
      "SELECT * FROM memory_patch_items WHERE id = ? AND conversation_id = ?",
    )
