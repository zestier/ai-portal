...
export function isMultiValuedPredicate(predicate: string): boolean {
  ...
  return MULTI_VALUED_PREDICATE_PREFIXES.some((prefix) =>
...
export function tryAcquireExtractionLock(
  ...
  return db.transaction(() => {
    ...
    const row = db
      .prepare(
        "SELECT holder FROM memory_extraction_locks WHERE conversation_id = ?",
      )
 ...
 */
export function releaseExtractionLock(
  conversationId: string | number,
  holder: string,
): void {
  getDb()
    .prepare(
      "DELETE FROM memory_extraction_locks WHERE conversation_id = ? AND holder = ?",
    )
...
export function applyOpenLoopLivenessProjection(
  ...
  const presented = Array.isArray(payload.presented)
    ? (payload.presented as number[])
    : [];
  const baseThreshold =
    typeof payload.baseThreshold === "number" ? payload.baseThreshold : 0;
  if (presented.length === 0 || baseThreshold <= 0) return [];
  const kept = new Set(
    Array.isArray(payload.kept) ? (payload.kept as number[]) : [],
  );
...
export function getCurrentMemoryHead(
  db: Database.Database,
  conversationId: number,
): string | null {
  const row = db
...
export function getProjectionMemoryHead(
  ...
  const row = db
    .prepare(
      "SELECT projection_event_id FROM memory_heads WHERE conversation_id = ?",
    )
...
export function getAppendParentMemoryHead(
  ...
  if (sourceMessageId) {
    const row = db
      ...
      .get(conversationId, sourceMessageId) as
      { head_event_id: string | null } | undefined;
    if (row) return row.head_event_id;
  }
...
function setMemoryRef(
  ...
  const prev = db
    .prepare(
      "SELECT target_event_id FROM memory_refs WHERE ref_kind = ? AND source_key = ?",
    )
  ...
  return prev && prev.target_event_id !== targetEventId
    ? prev.target_event_id
    : null;
}
...
function dropMemoryRef(
  ...
  const prev = db
    ...
    .get(kind, key) as { target_event_id: string } | undefined;
  db.prepare(
    "DELETE FROM memory_refs WHERE ref_kind = ? AND source_key = ?",
  ).run(kind, key);
  return prev?.target_event_id ?? null;
}

function memoryEventIsReferenced(
  db: Database.Database,
  eventId: string,
): boolean {
  const row = db
    .prepare(
      "SELECT 1 AS ok FROM memory_refs WHERE target_event_id = ? LIMIT 1",
    )
...
export function gcMemoryEventChain(
  ...
  while (id && guard++ < 1_000_000) {
    ...
    const row = db
      .prepare(
        "SELECT parent_id FROM memory_event_log WHERE conversation_id = ? AND id = ?",
      )
...
export function dropMemoryMessageHead(
  ...
  db.prepare(
    "DELETE FROM memory_message_heads WHERE conversation_id = ? AND message_id = ?",
  ).run(conversationId, messageId);
  const orphaned = dropMemoryRef(
    db,
    "message_head",
    msgCodec.encode(messageId),
  );
...
export function headForMessagePrefix(
  ...
  for (const messageId of ids) {
    const row = db
      ...
      .get(conversationId, messageId) as
      { head_event_id: string | null } | undefined;
...
export function chainRowsForHead(
  ...
  while (id) {
    const row = db
      .prepare(
        "SELECT * FROM memory_event_log WHERE conversation_id = ? AND id = ?",
      )
...
export function appendSessionMemoryLog(
  ...
  const id = ulid();
  const parentId = getAppendParentMemoryHead(
    db,
    conversationId,
  ...
  const headMessageId =
    input.sourceMessageId &&
    messageBelongsToConversation(db, conversationId, input.sourceMessageId)
      ? input.sourceMessageId
      : getLatestMessageId(db, conversationId);
  if (headMessageId) {
    recordMemoryMessageHead(db, conversationId, headMessageId, id);
  }
  const currentHead = headMessageId
    ? getCurrentMemoryHead(db, conversationId)
    : id;
  if (currentHead === id) {
...
function messageBelongsToConversation(
  ...
  const row = db
    .prepare(
      "SELECT 1 AS ok FROM messages WHERE conversation_id = ? AND id = ?",
    )
    .get(conversationId, messageId) as { ok: number } | undefined;
  return row !== undefined;
}

export function clearSessionMemoryProjection(
  db: Database.Database,
  conversationId: number,
): void {
  db.prepare("DELETE FROM memory_search_index WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_patch_items WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_tool_calls WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare(
    "DELETE FROM memory_validation_issues WHERE conversation_id = ?",
  ).run(conversationId);
  db.prepare("DELETE FROM memory_patches WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_open_loops WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_facts WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_events WHERE conversation_id = ?").run(
    conversationId,
  );
  db.prepare("DELETE FROM memory_entities WHERE conversation_id = ?").run(
    conversationId,
  );
}
...
function applySessionMemoryLogProjection(
  ...
  if (row.item_type === "entity") {
    ...
    applyOpenLoopLivenessProjection(
      db,
      row.conversation_id,
      record as OpenLoopLivenessPayload,
    );
...
function rebuildSessionMemoryIndexes(
  db: Database.Database,
  conversationId: number,
): void {
  db.prepare("DELETE FROM memory_search_index WHERE conversation_id = ?").run(
...
function upsertEntityProjection(
  db: Database.Database,
  item: MemoryEntity,
): void {
  db.prepare(
...
function upsertOpenLoopProjection(
  db: Database.Database,
  item: MemoryOpenLoop,
): void {
  db.prepare(
...
function upsertPatchProjection(
  db: Database.Database,
  patch: MemoryPatch,
): void {
  db.prepare(
...
function upsertPatchItemProjection(
  db: Database.Database,
  item: MemoryPatchItem,
): void {
  db.prepare(
...
function upsertIssueProjection(
  db: Database.Database,
...
function upsertToolCallProjection(
  db: Database.Database,
