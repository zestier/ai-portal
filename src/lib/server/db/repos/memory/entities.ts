...
export function upsertEntity(
  ...
  const tx = db.transaction((): MemoryEntity => {
    const existing = db
      .prepare(
        "SELECT * FROM memory_entities WHERE conversation_id = ? AND entity_key = ?",
      )
...
export function getEntity(
  ...
  const intConv = convInt(conversationId);
  const id =
    typeof entityKeyOrId === "string"
      ? memoryEntityId.tryParse(entityKeyOrId)
      : entityKeyOrId;
  const key =
    typeof entityKeyOrId === "string" && id === null ? entityKeyOrId : null;
  const row = getDb()
    .prepare(
...
export function entityFactCounts(conversationId: number): Map<string, number> {
  ...
  for (const row of rows)
    counts.set(memoryEntityId.encode(row.entityId), row.count);
  return counts;
}
...
export function updateEntity(
  ...
  const row = getDb()
    .prepare(
      "SELECT * FROM memory_entities WHERE id = ? AND conversation_id = ?",
    )
...
export function mergeEntities(
  ...
  const tx = db.transaction((): MergeEntitiesResult => {
    ...
    for (const eventRow of eventRows) {
      const nextActor =
        eventRow.actor_entity_id === fromId ? intoId : eventRow.actor_entity_id;
      const nextTarget =
        eventRow.target_entity_id === fromId
          ? intoId
          : eventRow.target_entity_id;
      db.prepare(
    ...
    const loopRows = db
      .prepare(
        `SELECT * FROM memory_open_loops WHERE conversation_id = ? AND status != 'deleted'`,
      )
      .all(intConv) as OpenLoopRow[];
    for (const loopRow of loopRows) {
      const related = parseJson(
        loopRow.related_entity_ids_json,
        [],
      ) as number[];
