...
export function deleteItem(
  ...
  if (normalized === "fact")
    return updateFact(intConv, id, { status: "deleted" }) !== null;
 ...
 */
export function revertCommittedPatch(
  conversationId: string | number,
  patchId: number,
): void {
  const intConv = convInt(conversationId);
...
export function wipe(conversationId: string | number): void {
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare("DELETE FROM memory_event_log WHERE conversation_id = ?").run(
      intConv,
    );
    db.prepare("DELETE FROM memory_refs WHERE conversation_id = ?").run(
      intConv,
    );
    db.prepare("DELETE FROM memory_heads WHERE conversation_id = ?").run(
      intConv,
    );
    db.prepare(
      "DELETE FROM memory_message_heads WHERE conversation_id = ?",
    ).run(intConv);
  });
  tx();
}
...
export function compactSessionMemoryLog(
  ...
  const tx = db.transaction(() => {
    ...
    const deleteRefBySource = db.prepare(
      "DELETE FROM memory_refs WHERE source_key = ?",
    );
...
export function rebuildSessionMemoryProjection(
  conversationId: string | number,
): void {
  const db = getDb();
  const intConv = convInt(conversationId);
...
function countSessionProjectionRows(
  ...
  const row = (
    status
      ? db
          .prepare(
            `SELECT COUNT(*) AS n FROM ${table} WHERE conversation_id = ? AND status = ?`,
          )
...
function getMaxRemapperId(
  db: Database.Database,
  type: string,
): number | undefined {
  const table = REMAPPER_TABLES[type];
  if (!table) return undefined;
  const row = db.prepare(`SELECT MAX(id) AS m FROM ${table}`).get() as {
    m: number | null;
  };
...
function createForkMemoryRemapper(
  ...
  const remapItem = (
    ...
    if (itemType === "entity") {
      ...
      return {
        ...item,
        id: enc("entity", mintId("entity", item.id)!),
  ...
  return (itemType, payload) => {
    ...
    if ("toolCall" in result)
      result.toolCall = remapItem("tool_call", result.toolCall);
    ...
    if (itemType === "open_loop_liveness") {
      const remapLoopIds = (value: unknown): number[] =>
        Array.isArray(value)
          ? value
              .map((id) =>
                typeof id === "number" ? refIntId("open_loop", id) : null,
              )
...
export function replaySessionMemoryLogForFork(
  ...
  const isIncluded = (type: string, id: number) =>
    included.has(`${type}:${id}`);
  const markIncluded = (type: string, id: number) =>
  ...
  const tx = db.transaction(() => {
    ...
    const sourceHead = headForMessagePrefix(
      db,
      sourceConversationId,
      opts.messageIdMap.keys(),
      {
    ...
    const remap = createForkMemoryRemapper(
      targetConversationId,
      opts.messageIdMap,
      isIncluded,
    );
    for (const row of copied) {
      ...
      const targetItemId = remapIdForPayload(
        row.item_type,
        row.item_id,
        transformed,
      );
    ...
    counts.events = countSessionProjectionRows(
      db,
      "memory_events",
      targetConversationId,
      null,
    );
    counts.facts = countSessionProjectionRows(
      db,
      "memory_facts",
      targetConversationId,
      "active",
...
export function rewindSessionMemoryLogToMessagePrefix(
  ...
  const tx = db.transaction(() => {
    ...
    removed = Math.max(
      0,
      chainRowsForHead(db, conversationId, currentHead).length - kept,
    );
    ...
    const headRows = db
      .prepare(
        "SELECT message_id FROM memory_message_heads WHERE conversation_id = ?",
      )
