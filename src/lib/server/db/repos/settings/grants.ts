  ...
  return rows.map((r) => ({
    ...
    conversationId:
      r.conversation_id === null
        ? null
        : conversationId.encode(r.conversation_id),
...
export function clearWorkspacePermissionState(
  userId: number,
  workspaceRoot: string,
): void {
  getDb()
    .prepare(
      `DELETE FROM workspace_permission_state WHERE user_id = ? AND workspace_root = ?`,
    )
...
export function clearWorkspacePermissionStateForUser(userId: number): void {
  getDb()
    .prepare(`DELETE FROM workspace_permission_state WHERE user_id = ?`)
    .run(userId);
}
 ...
 */
export function clearWorkspaceFileState(
  userId: number,
  workspaceRoot: string,
): number {
  let removed = 0;
...
export function countWorkspaceFileGrants(
  userId: number,
  workspaceRoot: string,
): number {
  const row = getDb()
...
export function revokeWorkspaceFileGrants(
  userId: number,
  workspaceRoot: string,
): number {
  const r = getDb()
...
export function revokeAllGrantsForUser(userId: number): number {
  ...
  getDb()
    ...
    .run(userId);
  const r = getDb()
    .prepare(`DELETE FROM permission_grants WHERE user_id = ?`)
    .run(userId);
  return r.changes;
}
...
export function pruneExpiredGrants(now: number = Date.now()): number {
  const r = getDb()
    .prepare(
      `DELETE FROM permission_grants WHERE expires_at IS NOT NULL AND expires_at < ?`,
    )
    .run(now);
  return r.changes;
}
