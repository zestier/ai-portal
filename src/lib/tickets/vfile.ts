...
export type ParseResult = { ok: true; value: TicketVFile } | { ok: false; error: string };
...
export function parseTicketFile(text: string): ParseResult {
	...
	if (firstLf < 0 || text.slice(0, firstLf).trim() !== VFILE_FENCE) {
		return {
			ok: true,
			value: { id: '', title: '', plan: '', body: text, status: 'open', priority: 'P2' }
		};
	}
	...
	try {
		...
		if (e instanceof TomlError) {
			...
			return {
				ok: true,
				value: { id: '', title: '', plan: '', body, status: 'open', priority: 'P2' }
			};
		}
		throw e;
	}
	...
	if (id && !/^[T][1-9][0-9]*$/i.test(id)) {
		return { ok: false, error: `invalid ticket id in file: ${JSON.stringify(id)}` };
	}
	if (title && !title.trim()) {
		return { ok: false, error: 'ticket title cannot be empty' };
	}

	return { ok: true, value: { id, title: title.trim(), plan, body, status, priority } };
}
 ...
 */
export function ticketFileDiff(current: TicketVFile, parsed: TicketVFile): TicketFilePatch {
