<script lang="ts">
  ...
  const lazyState = $derived(
    lazy
      ? lazyFieldState(lazy.conversationId, "file-diff", lazy.fileEditId)
      : null,
  );
  ...
  const tooLarge = $derived(
    resolvedDiff !== null && !isRenderableDiff(resolvedDiff),
  );
...
<div class="diff-set">
    ...
    <div class="diff">
      ...
      <div class="diff-lazy">
        ...
        {:else if lazy}
          ...
          <span class="muted"
            >Not loaded on open, to keep the conversation fast.</span
          >
        {:else}
  ...
  {#if tooLarge && resolvedDiff !== null}
    <div class="diff-too-large">
      Diff is too large to render safely ({resolvedDiff.length.toLocaleString()} characters;
      limit
  ...
  {#each parsedChunks as chunk (chunk.key)}
    {@const key = chunk.key}
    {@const collapsed =
      collapsible && (collapsedFiles[key] ?? collapsedByDefault)}
        ...
        {:else}
          ...
          >
            <div class="rows">
              {#each chunk.parsed as l, i (i)}
                ...
                {:else}
                  {@const loc = commentable
                    ? lineLocation(chunk.path, l)
                    : null}
                  {@const commented =
                    loc != null && commentedKeys
                      ? commentedKeys.has(lineKey(loc))
                      : false}
                  <div class={"line " + l.kind} class:commented role="row">
                    {#if commentable}
                      ...
                      {:else}
                        <span class="comment-add placeholder" aria-hidden="true"
                        ></span>
                      {/if}
                    {/if}
                    {#if showLineNumbers}
                    ...
                    <span class="text" role="cell"
                      ...
                      {@html highlightedDiffLine(key, l, i)}</span
                    >
                  </div>
                {/if}
              {/each}
          ...
          {#if highlightedChunks[key]?.skipped}
            <div class="highlight-note">
              Syntax highlighting skipped for this large diff.
            </div>
          {/if}
        {/if}
      {/if}
