<script lang="ts">
  ...
  async function submitForkEdit(isolated = false) {
    ...
    try {
      const r = await fetch(
        `/api/conversations/${conversationId}/messages/${message.id}/fork`,
        {
          method: "POST",
  ...
  async function submitInlineEdit() {
    ...
    try {
      const r = await fetch(
        `/api/conversations/${conversationId}/messages/${message.id}/edit`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
        ...
        {:else if p.kind === "tool"}
          ...
          {:else}
            <ToolCall
              toolCall={p.tool}
              {conversationId}
              onRerunStarted={onToolRerunStarted}
            />
          {/if}
        {:else if p.kind === "reasoning"}
          <ReasoningBlock
            ...
            lazy={p.block.textTruncated && conversationId
              ? {
                  conversationId,
                  reasoningBlockId: p.block.id,
                  bytes: p.block.textBytes,
                }
              : null}
          />
        {:else}
          <DiffView
            ...
            lazy={p.edit.diffTruncated && conversationId
              ? {
                  conversationId,
                  fileEditId: p.edit.id,
                  bytes: p.edit.diffBytes,
                }
              : null}
          />
        {/if}
      ...
      >
        ...
        <div class="edit-actions">
          <span class="hint muted">
            Fork keeps this thread unchanged. Inline edit discards later
