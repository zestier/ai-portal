...
>
  ...
  <p class="muted small">
    Exactly what the model received for this turn — the auto-injected portal
    prelude, any memory or prior-message context, and your raw message.
  </p>
  ...
  {:else if !input}
    <p class="muted">
      No captured input for this message. Inputs are recorded for turns started
  ...
  {:else}
    ...
    {#if input.initialMessages && input.initialMessages.length > 0}
      ...
      <p class="muted small">
        Sent as conversation history for providers that can't resume a server
        session.
      </p>
      <div class="prior">
