  ...
  async function confirmRevert() {
    ...
    try {
      ...
      const res = await fetch(
        `/api/conversations/${conversationId}/git/changes/revert${query}`,
        {
          method: "POST",
        },
      );
...
<div class="git-status" aria-label="Git status">
  ...
  {:else}
    <div class="row1">
      <span
        class="branch"
        title={head.detached ? "Detached HEAD" : (head.branch ?? "")}
      >
    ...
    <div class="row2 small">
      {#if head.upstream}
        ...
        {#if head.ahead}<span class="ahead" title="Commits ahead of upstream"
            >↑{head.ahead}</span
          >{/if}
        {#if head.behind}<span class="behind" title="Commits behind upstream"
            >↓{head.behind}</span
          >{/if}
        {#if !head.ahead && !head.behind}<span
            class="muted in-sync"
            title="In sync with upstream">·</span
          >{/if}
      {:else}
...
{#if head?.initialized}
  ...
  >
    <p>
      ...
      <strong
        >{head?.dirtyCount ?? 0} uncommitted change{(head?.dirtyCount ?? 0) ===
        1
          ? ""
          : "s"}</strong
      >
      in the working tree: tracked files are reset and untracked files are deleted.
...
<style>
  .git-status {
    ...
    gap: var(--space-1);
    position: relative;
    padding: var(--space-2) calc(var(--space-3) + 5.5rem) var(--space-2)
      var(--space-3);
