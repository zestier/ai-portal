<script lang="ts">
  ...
  const tool = $derived<GrantTool>(
    isGrantTool(request.tool) ? request.tool : "shell",
  );
...
>
  ...
  <div class="body" id={bodyId}>
    ...
    <div class="muted small editor-hint">
      The agent proposed the scope below. Review it and narrow anything before
      saving — what you save here is what gets persisted, not the original
      request.
    </div>
  ...
  <div class="preview muted small" aria-live="polite">
    ...
    {#if denyAllPolicy}
      <Alert kind="warning">
        Default policy is <strong>Deny all</strong>; "Save grant" is disabled so
