<script lang="ts">
  ...
  let appliesTo = $state<"this-conversation" | "all-conversations">(
    "this-conversation",
  );
  ...
  function pickDeny(decision: "deny" | "deny-always") {
    ...
    pick(
      feedback
        ? { kind: "permission", decision, feedback }
        : { kind: "permission", decision },
    );
  }

  function previewShellAlways(
    decision: "allow-always" | "deny-always",
  ): string {
    const verb = decision === "allow-always" ? "Allow" : "Deny";
    const where =
      appliesTo === "all-conversations"
  ...
  function pickAlways(decision: "allow-always" | "deny-always") {
    ...
    const scope = buildPermissionGrantScope(
      request,
...
>
  ...
  <div class="body" id={bodyId}>
    ...
    {#if isShellWithAnalysis && shellAnalysis && shellAnalysis.kind === "parsed"}
      <div class="shell-breakdown" aria-label="Pipeline breakdown">
        <div class="muted small">
          ...
          {shellAnalysis.segments.length === 1 ? "command" : "commands"}) —
          every segment runs if you allow this:
        </div>
    ...
    {:else if isShellUnsafe && shellAnalysis && shellAnalysis.kind === "unsafe"}
      <div class="shell-unsafe" role="note">
        ...
        <div>
          Reason: <code>{shellAnalysis.reason}</code>. Constructs like
          subshells, redirection, command substitution, or variable expansion
          can hide arbitrary commands, so structured "always" grants won't apply
          here. Review the full command above before allowing.
        </div>
      </div>
    {/if}
    ...
    {#if request.escalationReason}
      <div class="shell-unsafe" role="note">
        ...
        <div class="muted small">
          This request can only be allowed once. Persistent decisions are
          disabled.
        </div>
      </div>
    {/if}
      ...
      <div class="git-commit-preview" role="note">
        ...
        <dl>
          ...
          {#if gitCommit.allowConflictMarkers}
            <div>
              ...
              <dd>
                Allowed — files may still contain <code
                  >&lt;&lt;&lt;&lt;&lt;&lt;&lt;</code
                > markers.
              </dd>
            </div>
        ...
        {#if gitCommit.trailers.length > 0}
          <table class="trailers">
            <thead>
              <tr
                ><th class="eyebrow">Trailer</th><th class="eyebrow">Value</th
                ></tr
              >
            </thead>
            <tbody>
              {#each gitCommit.trailers as trailer, i (`${trailer.token}\0${trailer.value}\0${i}`)}
                <tr>
        ...
        {#if templatePreview.merged}
          ...
          {#if merged.prompt.changed}
            <div class="template-prompt-block">
              ...
              <pre class="prompt-before">{merged.prompt.before ??
                  "(none)"}</pre>
              <div class="muted small">
      ...
      <details class="grant-scope">
        ...
        <div class="scope-body">
          {#if isShellWithAnalysis}
            <fieldset class="scope-group">
              ...
              {#if shellOptions.length === 0}
                <div class="muted small">
                  No persistable scopes derivable from this command (all
                  argv[0]s were path-qualified or otherwise unsafe to anchor a
                  grant on).
                </div>
              {:else}
                <div class="muted small">
                  Check every scope you want to remember. Each becomes its own
                  grant and is combined at decision time — covering more
                  invocations than this exact pipeline.
                </div>
          ...
          {:else if isShellUnsafe}
            <div class="muted small">
              Structured grants are unavailable for commands the parser can't
              model. Use Settings → Permissions to add a coarse "any shell from
              this tool" grant if you really want one.
            </div>
          {:else}
            <fieldset class="scope-group">
              <legend>Scope</legend>
              ...
              {:else if scopeChoice === "this-directory" && fsParentDir}
                <div class="muted small">
                  Matches any path resolving inside <code>{fsParentDir}/</code> (symlinks
                  followed).
                </div>
              {/if}
            </fieldset>
          ...
          <label class="expiry">
            ...
            <select
              value={expiryChoice}
              onchange={(e) =>
                (expiryChoice = (e.currentTarget as HTMLSelectElement)
                  .value as ExpiryChoice)}
            >
  ...
  {#if canPersistDecision}
    <div class="preview muted small" aria-live="polite">
      ...
      {#if denyAllPolicy}
        <Alert kind="warning">
          Default policy is <strong>Deny all</strong>; "Allow always" is
          disabled so it doesn't get silently dropped.
        </Alert>
      {/if}
    </div>
  {/if}
