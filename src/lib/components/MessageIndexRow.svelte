<script lang="ts">
	...
	const chips = $derived(entry.records.slice(0, MAX_CHIPS).map((d) => chipLabel(d)));
	...
	function chipLabel(d: TranscriptRecordDescriptor): string {
		switch (d.kind) {
			...
			case 'edit':
				return `✎ ${d.summary}`;
			case 'reasoning':
				return d.reasoningKind === 'content' ? `💬 ${d.summary}` : `🤔 ${d.summary}`;
		}
	}
...
<div class="index-row" data-role={entry.role} data-testid="transcript-index-row">
