<script lang="ts">
  ...
  type WorkspaceFileRequest = InteractiveWorkspaceFileView & {
    requestId: string;
  };
...
<div class="body">
  ...
  <p class="meta">
    ...
    {#if request.acceptedHash !== null}
      &middot; {request.activeGrantCount} grant{request.activeGrantCount === 1
        ? ""
        : "s"} active from the last approved version
    {/if}
  </p>

  {#if request.parseError}
    <Alert kind="error">
      This file is not a valid permissions file and cannot be imported. Fix it
      to review it again:
