<script lang="ts">
  ...
  import type {
    TranscriptEntry,
    TranscriptState,
  } from "$lib/client/transcript-store";
  import * as store from "$lib/client/transcript-store";
  import {
    TRANSCRIPT_BODY_CACHE_MAX,
    TRANSCRIPT_OLDER_PAGE_SIZE,
  ...
  import {
    addInteractive,
    removeInteractive,
  } from "$lib/client/interactive-queue";
  import {
  ...
  import {
    setAwaitingInput,
    clearAwaitingInput,
  } from "$lib/client/awaiting-input";
  ...
  } from "$lib/client/composer-arming";
  ...
  let globalMemoryEnabled = $state<boolean>(
    untrack(() => conversation.globalMemoryEnabled),
  );
  let approvalMode = $state<ApprovalMode>(
    untrack(() => conversation.approvalMode),
  );
  ...
  function attachStream(turnId: string, opts: { replay?: boolean } = {}) {
    ...
    es.onmessage = (msg) => {
      ...
      if (ev.type === "done") {
        ...
        const failed =
          turnErrored || ev.status === "interrupted" || ev.status === "error";
    ...
    es.addEventListener("stream_error", (msg) =>
      es.onmessage?.(msg as MessageEvent),
    );
    es.onerror = () => {
    ...
    switch (ev.type) {
      ...
      case "session.settings": {
        ...
        if (ev.disabledToolGroups !== undefined)
          disabledToolGroups = ev.disabledToolGroups.filter(
            (g): g is PortalToolGroupId =>
  ...
  function addPendingInteractive(
    ...
    if (visibleInteractive.some((p) => p.requestId === request.requestId))
      return;
    ...
    const timer = setTimeout(() => {
      ...
      if (!pendingInteractive.some((p) => p.requestId === request.requestId))
        return;
      visibleInteractive = addInteractive(visibleInteractive, request);
      void scrollToBottom();
  ...
  async function respondInteractive(
    requestId: string,
    ...
    try {
      const r = await fetch(
        `/api/conversations/${conversation.id}/interactive/${requestId}`,
        {
          method: "POST",
  ...
  async function stop() {
    ...
    const stillCurrent = () =>
      stopping && activeTurnId === turnId && conversation.id === convId;
    try {
  ...
  const thinking = $derived.by(() => {
    ...
    const last = lastEntry
      ? transcript.bodies[String(lastEntry.id)]
      : undefined;
    if (!last || last.role !== "assistant") return true;
    const hasContent = last.content.length > 0;
    const hasTools =
      (last.toolCalls?.length ?? 0) > 0 || (last.fileEdits?.length ?? 0) > 0;
  ...
  const latestAssistantMessageId = $derived.by(() => {
    for (let i = transcript.entries.length - 1; i >= 0; i--) {
      if (transcript.entries[i].role === "assistant")
        return transcript.entries[i].id;
    }
    return null;
  });
    ...
    for (let i = 0; i < n; i++) {
      ...
      const body =
        i >= windowStart && i < windowEnd
          ? transcript.bodies[String(entry.id)]
          : undefined;
  ...
  async function fetchHydration(id: string | number): Promise<void> {
    try {
      const r = await fetch(
        `/api/conversations/${conversation.id}/messages/${id}`,
      );
      if (!r.ok) return;
  ...
  function evictBodies() {
    ...
    for (
      ...
      i < Math.min(n, windowEnd + OVERSCAN);
      i++
    ) {
  ...
  $effect(() => {
    ...
    for (
      let i = windowStart;
      i < Math.min(windowEnd, transcript.entries.length);
      i++
    ) {
      const e = transcript.entries[i];
      if (typeof e.id !== "number") continue;
      if (
        transcript.bodies[String(e.id)] !== undefined ||
        hydrationQueue.has(e.id)
      )
        continue;
    ...
    <div class="messages" bind:this={scrollEl} onscroll={onMessagesScroll}>
      ...
      >
        ...
        {#each renderUnits as u, i (u.entry.id)}
          {#if u.kind === "message"}
            <div
              class:highlighted={highlighted.has(u.m.id)}
              use:rowAction={u.entry.id}
            >
              <Message_
                message={u.m}
                conversationId={conversation.id}
                inputMessageId={inputMessageIdByAssistant[String(u.m.id)] ??
                  null}
                ...
                canRetryMemory={!streaming &&
                  u.m.id === latestAssistantMessageId}
                busy={streaming}
          ...
          {:else}
            <div
              class:highlighted={highlighted.has(u.entry.id)}
              use:rowAction={u.entry.id}
            >
              <MessageIndexRow
                entry={u.entry}
