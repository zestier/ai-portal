<script lang="ts">
  ...
  const active = $derived(
    worktrees.find((w) => String(w.id) === selected) ?? null,
  );
  ...
  const selectedMissing = $derived(
    selected !== null && worktrees.length > 0 && active === null,
  );
  ...
  function postMerge(leaseId: string, allowMergeCommit: boolean) {
    return fetch(
      `/api/conversations/${conversationId}/worktrees/${leaseId}/merge`,
      {
        method: "POST",
      ...
      >
        ...
        {#each worktrees as w (w.id)}
          <option value={w.id} disabled={!w.available}>
            {w.label}
            {#if !w.available}(unavailable){:else if w.dirtyCount}({w.dirtyCount}
              changed){/if}
          </option>
        {/each}
      </select>
    ...
    {#if active}
      ...
      {#if active.ahead}
        ...
        >
          {merging
            ? "Merging…"
            : `Merge ${active.ahead} commit${active.ahead === 1 ? "" : "s"}`}
        </button>
