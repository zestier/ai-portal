<script lang="ts">
  ...
  let workspace = $state<LaunchWorkspaceKind>(
    untrack(() => defaults.workspace),
  );
  let conversationMode = $state<string>(
    untrack(() => defaults.conversationMode ?? ""),
  );
...
<Modal {open} onClose={onCancel} ariaLabel={title} width="min(680px, 100%)">
  <div class="review">
    <div>
      <p class="eyebrow">
        {templateTitle
          ? `Launching “${templateTitle}”`
          : "Launching a template"}
      </p>
      <h2>{title}</h2>
    </div>
    <p class="muted small">
      Edit the prompt and the launch settings. Nothing is created until you
      launch.
    </p>
    ...
    <fieldset class="tool-groups-fieldset" disabled={busy}>
      ...
      <p class="muted small">
        Unchecked groups are disabled up front in this chat (a seed — the chat
        can re-enable them). Checked groups stay available. Native CLI tools
        (bash, view, edit…) are always available and unaffected.
      </p>
