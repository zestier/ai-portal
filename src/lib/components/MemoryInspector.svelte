<script lang="ts">
	...
	async function wipe() {
		...
		error = null;
		try {
			const res = await fetch(`/api/conversations/${conversationId}/memory`, { method: 'DELETE' });
			if (!res.ok) throw new Error(`HTTP ${res.status}`);
			const body = (await res.json()) as { memory: unknown };
			memory = body.memory;
		} catch (e) {
	...
	async function mutateItem(kind: string, id: string, method: 'PATCH' | 'DELETE', body?: object) {
		if (busyItem) return;
		busyItem = `${kind}:${id}`;
		error = null;
		try {
	...
	function editablePayload(kind: string, row: unknown): Record<string, unknown> {
		if (!row || typeof row !== 'object') return {};
		const record = row as Record<string, unknown>;
		if (kind === 'entities') {
			return pick(record, ['entityType', 'displayName', 'summary', 'status', 'metadata']);
		}
		if (kind === 'facts') {
							...
							{:else if kind === 'patchItems'}
								{#if (row as { reviewStatus?: string }).reviewStatus !== 'approved'}
									<button
										type="button"
										disabled={busyItem !== null}
										onclick={() => reviewPatchItem(row, 'approve')}>Approve</button
									>
								{/if}
								{#if (row as { reviewStatus?: string }).reviewStatus !== 'rejected'}
									<button
										...
										class="danger"
										disabled={busyItem !== null}
										onclick={() => reviewPatchItem(row, 'reject')}>Reject</button
									>
								{/if}
							{/if}
						</div>
