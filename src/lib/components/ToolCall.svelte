<script lang="ts">
  ...
  const lazyArgs = $derived(
    lazyFieldState(conversationId, "tool-args", toolCall.id),
  );
  ...
  function hydrateTruncated() {
    if (argsTruncated)
      ensureLazyField(conversationId, "tool-args", toolCall.id);
    if (resultTruncated)
  ...
  const summary = $derived(
    summarizeToolCall(toolCall.tool, argsJson) ?? toolCall.summary ?? null,
  );
  const decoded = $derived(decodeToolResult(resultJson));
  const markdownResult = $derived(
    shouldRenderToolResultAsMarkdown(toolCall.tool),
  );
  ...
  const renderedDiffs = $derived(
    toolCall.status === "ok"
      ? synthesizeDiffs({ tool: toolCall.tool, argsJson })
      : [],
  );
  const gitRenderedResult = $derived(
    toolCall.status === "ok"
      ? parseGitToolResult(toolCall.tool, argsJson, resultJson)
      : null,
  );
  const gitDiffText = $derived(
    toolCall.status === "ok" &&
      toolCall.tool === "git_diff" &&
  ...
  const rerunDisabledReason = $derived.by(() => {
    ...
    if (toolCall.parentToolCallId)
      return "Nested sub-agent tool calls cannot be rerun yet.";
    return null;
  });
  ...
  const shellCommand = $derived.by(() => {
    ...
    try {
      ...
      if (a && typeof a === "object" && !Array.isArray(a)) {
        const cmd =
          (a as Record<string, unknown>).command ??
          (a as Record<string, unknown>).cmd;
  ...
  async function rerunWithApproval() {
    ...
    if (argsJson === null) {
      rerunError =
        lazyArgs.error ?? "Could not load the original tool arguments.";
      return;
    }
...
>
  <summary>
    ...
    {:else if argsTruncated && argsJson === null}
      <span class="muted"
        >— {formatFieldBytes(toolCall.argsBytes)} of arguments</span
      >
    {:else}
      <span class="muted">— {toolCall.status}</span>
    {/if}
    {#if pending && toolCall.progressMessage}
      <span class="progress" title={toolCall.progressMessage}
        >· {toolCall.progressMessage}</span
      >
    {/if}
  </summary>
  <div class="content">
      ...
      <div class="rerun">
        <button
          ...
          title={rerunDisabledReason ??
            "Review exact args, grant short-lived approval, and rerun"}
        >
