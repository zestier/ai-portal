<script lang="ts">
  ...
  const linkHref = $derived(
    block.kind === "resource_link" || block.kind === "resource"
      ? safeHref(block.uri)
      : null,
  );
</script>
...
{:else if block.kind === "text" && markdownHtml}
  <div class="markdown-result" use:copyableCodeBlocks use:zoomableImages>
...
{:else if block.kind === "resource_link"}
  ...
  {:else}
    <span class="resource-link">
      {block.name}{block.description ? ` — ${block.description}` : ""} (<code
        >{block.uri}</code
      >)
    </span>
  {/if}
{:else if block.kind === "resource"}
  <div class="resource">
    ...
    {:else}
      <code>{block.uri}</code>
    {/if}
    {#if block.text}<div use:copyableCodeBlocks>
