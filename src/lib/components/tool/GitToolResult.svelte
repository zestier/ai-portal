      ...
      <tbody>
        {#each result.files as file (file.path)}
          <tr>
            <td>
              <code>{file.path}</code>
              {#if file.origPath}<span class="muted">
                  from {file.origPath}</span
                >{/if}
            </td>
...
{:else if result.kind === "commit-created"}
  <div class="git-result">
    <div class="commit-card">
      <div>
        <code title={result.sha}>{result.shortSha}</code>
        {result.subject}
      </div>
      {#if result.diffStat}
        <div class="summary">
    ...
    {#if result.trailers.length > 0}
      <table class="trailers">
        ...
        <tbody>
          {#each result.trailers as trailer, i (`${trailer.token}\0${trailer.value}\0${i}`)}
            <tr>
    ...
    {#if extraStats().length > 0}
      <details class="remaining">
        <summary class="muted"
          >Additional diff stat entries: {extraStats().length}</summary
        >
        <table>
          <thead>
            <tr><th>File</th><th>Added</th><th>Removed</th></tr>
