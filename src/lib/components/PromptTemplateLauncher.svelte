<script lang="ts">
  ...
  const defaultTemplateId = $derived(
    $page.data.defaultPromptTemplateId ?? null,
  );

  const builtIns = $derived(
    templates?.filter((template) => template.source === "builtin") ?? [],
  );
  ...
  function disabledGroupsSummary(
    template: PromptTemplateListItem,
  ): string | null {
    const disabled = template.disabledToolGroups ?? [];
    if (disabled.length === 0) return null;
    const labels = PORTAL_TOOL_GROUPS.filter((g) =>
      disabled.includes(g.id),
    ).map((g) => g.label);
      ...
      if (!res.ok) {
        ...
        reportError(
          message
            ...
            : `Could not create chat (${res.status})`,
        );
        return;
      }
  ...
  async function startLaunch(
    template: PromptTemplateListItem,
    options: TemplateLaunchOptions = templateLaunchDefaults(
      template,
      template.prompt,
    ),
  ): Promise<void> {
...
{#if pickerOpen}
  <Modal open onClose={closePicker} labelledby="template-dialog-title">
    ...
    <p class="muted small">
      Pick a reusable prompt to prefill the composer. You can edit it before
      sending.
    </p>
          ...
          {#each builtIns as template (template.id)}
            ...
            >
              ...
              {#if workspaceModeLabel(template)}
                <span class="tool-groups-tag"
                  >{workspaceModeLabel(template)}</span
                >
              {/if}
            </button>
          {/each}
            ...
            {#each customTemplates as template (template.id)}
              ...
              >
                ...
                {#if disabledGroupsSummary(template)}
                  <span class="tool-groups-tag"
                    >{disabledGroupsSummary(template)}</span
                  >
                {/if}
                {#if workspaceModeLabel(template)}
                  <span class="tool-groups-tag"
...
{#if reviewing}
  <LaunchReviewDialog
    ...
    defaults={reviewingOptions ??
      templateLaunchDefaults(reviewing, reviewing.prompt)}
