<script lang="ts">
  ...
  let shellPositionals = $state<ShellPositionalsKind>(
    initial.fields.shellPositionals,
  );
  ...
  let shellStepOptions = $state<ShellStepOptionInput[]>(
    initial.fields.shellStepOptions,
  );
  let originalShellCommand = $state<ShellCommandStep[] | null>(
    initial.originalShellCommand,
  );
  ...
  const shellCommandTokens = $derived(
    deriveShellCommandTokens({ shellArgv0, shellSubcommands }),
  );

  $effect(() => {
    const next = nextShellStepOptions(
      shellCommandTokens,
      shellStepOptions,
  ...
  function updateShellStepOption(
    index: number,
