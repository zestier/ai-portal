<script lang="ts">
  ...
  async function loadFile(path: string) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/conversations/${conversationId}/fs/file?${params}`,
        {
  ...
  async function loadDiff(path: string) {
    ...
    try {
      ...
      const res = await fetch(
        `/api/conversations/${conversationId}/fs/diff?${params}`,
        {
          signal: controller.signal,
        },
      );
  ...
  <div class="right">
    {#if pane === "commits" && selectedSha}
      <div class="header">
        <div class="title">
          <code class="sha"
            >{commitDetail?.shortSha ?? selectedSha.slice(0, 8)}</code
          >
          <span>{commitDetail?.subject ?? "Loading…"}</span>
        </div>
        {#if commitDetail}
          <div class="muted small">
            {commitDetail.author} · {new Date(
              commitDetail.timestamp,
            ).toLocaleString()}
          </div>
        {/if}
      </div>
      <div class="commit-body">
        {#if commitDetailError}
          <div class="error-wrap">
            <Alert kind="error">{commitDetailError}</Alert>
          </div>
          ...
          <div class="files-grid">
            ...
            <div class="commit-diff">
              ...
              {:else}
                <EmptyState
                  size="sm"
                  description="Select a file to see its diff."
                />
              {/if}
            </div>
          </div>
              ...
              <div class="image-preview">
                ...
                <div class="image-caption">
                  ...
                  {/if}{(fileData as { imageMimeType?: string })
                    ...
                    · {fmtSize(
                      (fileData as { size?: number }).size ?? null,
                    )}{/if}
                </div>
              </div>
            {:else}
              ...
              >
                {#each contentLines as text, idx (idx)}
                  ...
                  <div class="file-line" class:commented role="row">
                    ...
                    <span class="text" role="cell"
                      ><!-- eslint-disable-next-line svelte/no-at-html-tags -->
                      {@html highlightedContentLine(text, idx)}</span
                    >
                  </div>
                {/each}
              </div>
    ...
    {#if draft}
      <div class="review-draft">
        ...
        <div class="review-draft-actions">
          ...
          <button class="btn ghost" type="button" onclick={cancelComment}
            >Cancel</button
          >
          <button
            class="btn primary"
    ...
    {#if reviewComments.length > 0}
      <div class="review-bar">
        ...
        >
          <span class="chevron" class:open={reviewListOpen} aria-hidden="true"
            >▸</span
          >
          {reviewComments.length} review comment{reviewComments.length === 1
            ? ""
            : "s"}
        </button>
        <div class="review-bar-actions">
          <button
            class="btn ghost"
            type="button"
            onclick={() => reviewStore.clear()}>Clear</button
          >
          <button class="btn primary" type="button" onclick={sendReview}
            >Send to chat</button
          >
        </div>
      </div>
