<script lang="ts">
  ...
  import {
    ...
    worktreeStatusRevision,
  } from "$lib/client/worktree-status";
  ...
  async function chooseModel(next: string) {
    ...
    if (!trimmed || trimmed === model || savingModel || modelChangeDisabled)
      return;
    savingModel = true;
    const prev = model;
  ...
  async function chooseHarvesterModel(next: string | null) {
    ...
    if (
      normalized === memoryExtractorModel ||
      savingHarvester ||
      modelChangeDisabled
    )
      return;
  ...
  const disabledToolGroupSet = $derived(
    new Set<PortalToolGroupId>(disabledToolGroups),
  );
  ...
  $effect(() => {
    ...
    if (untrack(() => conversation.workspaceKind) !== "managed-worktree")
      return;
    void loadWorktree(id);
  });
  ...
  async function integrate(allowMergeCommit: boolean) {
    ...
    try {
      const res = await fetch(
        `/api/conversations/${conversation.id}/worktree/merge`,
        {
          method: "POST",
          headers: { "content-type": "application/json" },
      ...
      mergeFlash = merge?.merged
        ? `Merged ${merge.from} into ${merge.into}`
        : "Already up to date";
    ...
    <div class="details-inner">
      <div class="details-body">
        <dl class="header-meta">
          ...
          <dt>Workspace</dt>
          <dd>
            {conversation.workspaceKind === "managed-worktree"
              ? "Managed worktree"
              : "Shared"}
          </dd>
          ...
          {#if worktree?.isLinkedWorktree}
            ...
            <dd>
              {#if worktree.unmerged}
                {unmergedLabel}
              {:else}
                Fully merged{worktree.behind > 0
        ...
        {#if worktree?.isLinkedWorktree}
          <div class="worktree-actions">
            ...
            >
              {merging
                ? "Merging…"
                : `Merge into ${worktree.upstreamBranch ?? "source"}`}
            </button>
            ...
            {/if}
            {#if mergeFlash}<span class="worktree-flash ok">{mergeFlash}</span
              >{/if}
            {#if mergeError}<span class="worktree-flash err">{mergeError}</span
              >{/if}
          </div>
        {/if}
        {#if parent}
        ...
        >
          ...
          <p
            ...
            class:neutral={approvalMode === "ask"}
            role="note"
          >
            {currentApprovalNote}
          </p>
          <div class="tool-groups">
            <div class="tool-groups-head">
              ...
              <span class="tool-groups-sub"
                >Checked = available to the agent this session.</span
              >
            </div>
            ...
            <p class="tool-groups-note" role="note">
              ...
              <strong>Memory</strong> gates the memory tools on top of the memory-mode
              setting. Changes take effect on the next turn.
            </p>
          </div>
        </div>
      </div>
...
>
  <p>
    This auto-approves tool calls in <strong>this conversation only</strong> — the
    agent stops asking you to confirm them here, and your default permission policy
    is bypassed for it. Your other conversations still prompt as usual, and it stays
    on until you turn it off, including across reloads and new sessions.
  </p>
  <p>
    Deny and prompt grants in your permission settings still apply, and some
    high-impact portal actions — commits, worktree merges and removals, template
    changes — always ask. Each auto-approved portal tool is still recorded in
    the audit log as <code>auto-allow</code>.
  </p>
</ConfirmDialog>
