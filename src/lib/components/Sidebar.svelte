<script lang="ts">
  ...
  async function refreshWorktreeStatuses(fresh = false) {
    try {
      ...
      const res = await fetch(
        `/api/worktrees/status${fresh ? "?fresh=1" : ""}`,
      );
      if (!res.ok) return;
      const body = (await res.json()) as {
        worktrees?: WorktreeStatusSummary[];
      };
      worktreeStatuses = new Map(
        (body.worktrees ?? []).map((w) => [w.conversationId, w]),
      );
    } catch {
  ...
  const awaiting = (id: string) =>
    isAwaitingInput(id, serverAwaiting, $awaitingInputOverrides);
  ...
  const selectedActiveCount = $derived(
    active.filter((c) => selected.has(c.id)).length,
  );
  const allActiveSelected = $derived(
    active.length > 0 && selectedActiveCount === active.length,
  );
  ...
  async function deleteConv(id: string) {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        const reason = worktreeBlockReason(
          body?.code,
          body?.detail,
          body?.leases,
        );
        ...
        res = await fetch(`/api/conversations/${id}?forceWorktree=1`, {
          method: "DELETE",
        });
      }
    } catch {
  ...
  async function bulk(action: "archive" | "unarchive" | "delete") {
    ...
    try {
        ...
        ids.map(async (id) => {
          if (action === "delete") {
            ...
            return {
              ...
              ok: response.ok,
              blocked:
                worktreeBlockReason(body?.code, body?.detail, body?.leases) !==
                null,
            };
          }
          const response = await fetch(`/api/conversations/${id}`, {
      ...
      if (
        ...
        const forced = await Promise.all(
          blockedIds.map(async (id) => ({
            id,
            ok: (
              await fetch(`/api/conversations/${id}?forceWorktree=1`, {
                method: "DELETE",
              })
            ).ok,
            blocked: false,
      ...
      if (failed > 0)
        flashError(`${failed} of ${ids.length} ${action} operations failed`);
      await invalidateAll();
      if (action === "delete") {
        ...
        const currentIdNum =
          currentId === undefined
            ? undefined
            : conversationId.tryParse(currentId);
        if (
        ...
        {#if ticketDraftOpen}
          ...
          >
            <input
              ...
              disabled={ticketBusy}
            />
            <button class="btn sm" disabled={ticketBusy || !ticketTitle.trim()}
              >Add</button
            >
          </form>
        {/if}
            ...
            {#each tickets as ticket (ticket.id)}
              <div class="ticket">
                <div class="ticket-row">
                  ...
                  >
                    <span
                      class="caret ticket-caret"
                      class:open={isTicketExpanded(ticket.id)}>▸</span
                    >
                    ...
                    <div class="ticket-details">
                      {#if ticket.body.trim()}
                        {#if mounted}
                          ...
                          <div
                            class="markdown-body ticket-body-md"
    ...
    {#each active as c (c.id)}
      ...
      <div class="conv" class:selected={selected.has(c.id)}>
        ...
        {:else}
          ...
          >
            <div class="title-row">
              <span class="title" class:unseen={indicator?.kind === "unseen"}
                >{c.title}</span
              >
              {#if indicator}
        ...
        {#if isMenu}
          <div class="menu" onclick={stop} onkeydown={stop} role="presentation">
            <button bind:this={firstMenuItem} onclick={() => startRename(c)}
              >Rename</button
            >
            <button onclick={() => setArchived(c.id, true)}>Archive</button>
            <button class="danger" onclick={() => deleteConv(c.id)}
              >Delete</button
            >
          </div>
        {/if}
            ...
            {#if isMenu}
              ...
              >
                <button bind:this={firstMenuItem} onclick={() => startRename(c)}
                  >Rename</button
                >
                <button onclick={() => setArchived(c.id, false)}
                  >Unarchive</button
                >
                <button class="danger" onclick={() => deleteConv(c.id)}
                  >Delete</button
      ...
      <div class="bulk-actions">
        ...
        <button
          ...
          disabled={bulkBusy ||
            selected.size === 0 ||
            [...selected].every(
              (id) => archived.find((c) => c.id === id) == null,
            )}
